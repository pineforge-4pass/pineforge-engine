...
(`src/native_execution_consumer.cpp:9199-9210`) sizes no journal under Window.
`NativeRunSpec::event_retention` (`include/pineforge/native_run_spec.hpp:641`)
defaults to Window, and the Pine adapter declares
`NativeEventRetention::Window` (`src/source/pine_adapter.cpp:2333`). K4 still
...
(`src/source/pine_adapter.cpp:5331`) returns at its receipt gate when no
...
- **PERF-L4's retired reader slots.** `event_high_water_reader_` and
  ...
  (`include/pineforge/source/pine_adapter.hpp:1924-1925`), with their type
