...
`reserve_driver_log` (`src/native_execution_consumer.cpp:2538-2543`) returns
unless the retention is Full, and `presize_logs`
(`src/native_execution_consumer.cpp:8837-8848`) sizes no journal under Window.
`NativeRunSpec::event_retention` (`include/pineforge/native_run_spec.hpp:639`)
defaults to Window, and the Pine adapter declares
`NativeEventRetention::Window` (`src/source/pine_adapter.cpp:2429`). K4 still
...
(`src/source/pine_adapter.cpp:5175`) returns at its receipt gate when no
...
- **PERF-L4's retired reader slots.** `event_high_water_reader_` and
  ...
  (`include/pineforge/source/pine_adapter.hpp:1917-1918`), with their type
