...
|---|---|---|---|---|---|---|
| OL1 | Market / limit / stop / stop-limit / trail triggers (`trigger_for` pine_adapter.cpp:2600-2611; `entry()` pine_adapter.cpp:5197, `order()` pine_adapter.cpp:10476) | **yes** — `Trigger` native_order.hpp:296-315; geometric matching `first_region_entry` native_execution_consumer.cpp:5710-5757; `submit` native_host.hpp:1213 | K | — | F:A1 O:A1 S:O2 | — |
| OL2 | Intents: flatten, reduce by units, reduce owner-opened units, transact, reverse-to (`close_all()` pine_adapter.cpp:8333 → `Flatten{}` pine_adapter.cpp:8362-8363; `Reduce{ExplicitUnits}` pine_adapter.cpp:7449-7458; `ReverseTo` pine_adapter.cpp:6289-6290) | **yes** — `OrderIntent` native_order.hpp:229, reduce sizes `ReductionSize` native_order.hpp:215, `ReverseTo` native_order.hpp:60-62, `OpeningShape` native_order.hpp:910-914, `Flatten` native_order.hpp:36 | K | — | F:A4 F:A8 O:A2 O:A3 S:O1 | — |
...
| TR3 | Zero-distance trail, "ride the best, exit on any adverse tick" (adapter substitutes `tick * 0.5` pine_adapter.cpp:11787-11799) | **no** — the kernel fails the run when `offset <= 0` (`checked_trail_stop` → native_execution_consumer.cpp:4473-4478) **Closed:** a zero offset is legal and means *ride the best* — the level is the running best itself and the exit is the first print strictly past it; negative and nonfinite offsets stay rejected (`tests/test_native_order_ergonomics.cpp`, `tests/test_native_anchored_legs.cpp`). | K+A | L7 | O:B3 F:B3 S:T2 | **Disagree:** O kernel / F, S quirk. R5-1: accepting a zero offset is generic; TV zero-offset *pricing* (`zero_trail_sibling_stop` pine_adapter.cpp:11700-11837) stays A. |
...
| TR5 | *(quirk)* TV trail conventions: half-tick arm threshold against tick-quantized extremes `has_trail_request` pine_adapter.cpp:9588-9593, zero-offset pricing `zero_trail_sibling_stop` pine_adapter.cpp:11700-11837, retained best `retained_trail_best` pine_adapter.hpp:287-289, operand / restart rules `source_trail_points` pine_adapter.cpp:8605-8831, `trail_one_shot` pine_adapter.cpp:9677-9806 | n/a | A | — | F:B3 S:T2 | R5-1 |

### 1.3 Sizing bases and reservations

| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| SZ1 | Explicit units (`Transact{signed}` after a grid floor pine_adapter.cpp:5832-5836, pine_adapter.cpp:5832) | **yes** — `Transact` in `OrderIntent` native_order.hpp:229 | K | — | F:C1 O:C1 S:S1 | — |
| SZ2 | Host override of the resolved price / units (`resolve_terms` pine_adapter.cpp:10989-12193) | **yes** — `HostSized` native_order.hpp:73-78 + `resolve_execution_terms` native_host.hpp:945-949, invoked `resolve_execution_terms` native_execution_consumer.cpp:4958, guarded by `consume_matched_request`'s `try` native_execution_consumer.cpp:4935-4969 | K | — | F:C2 O:H9 S:S2 | **Disagree:** S "partial" (a hook is not a basis). The hook is native; bases are SZ3-SZ5. It stays the custom-policy seam after L3. |
...
| SZ8 | Fractional reduce of the position / a cohort (`quantize_close_units` pine_adapter.cpp:12497-12525, `compute_exit_reservation` pine_adapter.cpp:4289-4408) | **no** — sizes are `ExplicitUnits` / `OwnerOpenedUnits` / `HostSized` only native_order.hpp:60-74 **Closed:** `Reduce{ScopeFraction{fraction, claim, basis}}` takes a fraction in `(0, 1]` of the scope the reduce is bound to — the book, one opening, or a cohort roster (`tests/test_native_sizing_bases.cpp`). | K | L3 | F:A7 O:A12 O:C4 S:S5 | F's "partial" counts the units-reduce of OL2. R5-4 → L3. |
| SZ9 | Exit-quantity reservation against sibling exits (`compute_exit_reservation` pine_adapter.cpp:4341-4660, `reserved_exit_qty` pine_adapter.cpp:8964-9274) | **partial** — `PointBudget` native_order.hpp:379-382 and group `Reduce` native_order.hpp:220 give a generic reservation; no net-of-siblings claim for fraction sizes **Closed:** `ScopeClaim::NetOfSiblings` subtracts the units already claimed by live sibling reduces bound to the same scope, and `ScopeBasis::{AtMatch, AtAcceptance}` chooses when that scope is measured (`tests/test_native_sizing_bases.cpp`). | K+A | L3 | F:A14 S:S5 S:O10 O:A12 | F proposed nothing. R5-4: generic half → L3; the Pine ledger → SZ11, SZ12. |
...
| SZ10a | *(quirk, measured retention)* The money band's affordable price, `sig10(sig10(E) / notional per unit of price)` against the sizing price — at placement (`entry`, `affordable_price` pine_adapter.cpp:6688), in the paired-reversal whole drop (`close`, `affordable_price` pine_adapter.cpp:7894), at the terms boundary (`resolve_terms`, `affordable_price` pine_adapter.cpp:12755), in the precommit money scopes (`validate_precommit`, `affordable_price` pine_adapter.cpp:13131, pine_adapter.cpp:13184) and in the carried-money check (`apply_reversal_gap_bracket_policy`, `affordable_price` pine_adapter.cpp:15517) | n/a — not a units conversion. Every money-to-units conversion in the adapter is the kernel's `native_sized_units` under the source's floors since R5 lane B-ADAPTER: the default at placement and at the fill (`default_sizing_units` pine_adapter.cpp:2458), the typed per-call quantity at placement, at the fill and in the probe (`typed_quantity_units` pine_adapter.cpp:2471, `typed_entry_units` pine_adapter.cpp:2489), and the pending-order fill-quantity probe (`probe_fill_qty` pine_adapter.cpp:18622) | A | — | AUDIT3-opus2 H4 (G2 R2, R3) | **Measured:** the quotient is TradingView's whole-drop PRICE, pinned by 445 famr3 tapes (`tests/test_tv_money_band_l4b.cpp`) and 202 famr tapes (`tests/test_tv_money_precision_l4b.cpp`); on a seven-digit tie (E 9998.99999996, 10 000 lots at tick(close) 0.9999) the rule admits while the kernel's conversion buys 9999.99999996 lots and binary64 gives E/Q 0.999899999996 (`tests/test_adapter_fill_qty_probe.cpp`, section 2). |
| SZ11 | *(quirk)* Same-bar `strategy.close` callsite batching, two-call provenance, entry-id ledger (`enqueue_pooc_fifo_close` pine_adapter.cpp:7157-7507; ledger `close_batch_bar_` pine_adapter.hpp:2039-2050; the kernel's comment-only residue was deleted by R5 lanes E6 and E10) | n/a | A | — | S:O8 F:A15 O:A15 | **Disagree:** S "generic → kernel" (its P2 `ReplaceByKey` / `Fifo`). R5-4: adapter. |
...
| CT3 | Pre-match hook at bar open (`on_bar_open` pine_adapter.cpp:15956) | **yes** — `on_native_bar_open` native_host.hpp:889 runs before the open match native_execution_consumer.cpp:7096-7100; `execute_current` legal native_host.hpp:1104 | K | — | F:E3 O:F2 | hazard → CT4 |
...
| CT11 | *(quirk)* COOF TV specifics: waypoint-only refill (`coof_next_waypoint` pine_adapter.cpp:5687), two-fills-at-open rule, script-state rollback (`PineScheduler` pine_scheduler_native.cpp:52-104; hooks `snapshot_script_state` engine.hpp:1332-1333), the first-open guard `kFirstOpenLoopGuard` pine_scheduler_native.cpp:663, deferral queue of `PendingCoofRequest` pine_adapter.hpp:1638-1645 | n/a | A | — | F:E4 O:F5 S:C2 | R5-5: the adapter keeps its path |
...
| FP2 | Slippage in ticks, limit-clamped | **yes** — `slippage_ticks` native_run_spec.hpp:584 applied once native_execution_consumer.cpp:4620-4621, clamped by `protect_limit` native_execution_consumer.cpp:6289 | K+A | — | F:G1 O:H7 S:F2 | TV's round-then-slip order (`source_bar_fill` pine_adapter.cpp:11954-11963) is A |
...
| FP7 | *(quirk)* TV tick conventions: half-tick threshold `source_bar_fill_tick` pine_adapter.cpp:264-312 (used `source_trigger_threshold` pine_adapter.cpp:9029-9034, `exit_limit_trigger` pine_adapter.cpp:9563-9584), `source_bar_fill_tick` pine_adapter.cpp:202-215, raw-vs-booked spelling `source_same_point` pine_adapter.cpp:221-225, margin-call fill pricing `source_margin_fill_price` pine_adapter.cpp:13808-13814 | n/a — already isolated behind the terms seam (adapter `resolve_terms` pine_adapter.cpp:11679) | A | — | F:G4 O:H5 S:F3 | R5-1, R5-3 |
...
- Under the magnifier on an aggregated chart, the same-bar cohort checks
  ...
  **Closed (R5 lane H-MEASURE):** all three entry-bar tests of a magnified
  ...
  trades exact after). That pass also never ran on an aggregated or magnified run: its
...
3. **A cache must know which rows changed, and nothing records it.**
   ...
   (`has_full_entry_bracket` pine_adapter.cpp:8746). Full-table scans
   ...
   `preserved_by_close_all` pine_adapter.cpp:11488), and
...
- Mechanism (F): a **kernel-originated request**. At each script-bar open and after each applied fill the kernel computes the liquidation level and rests a `Reduce` / `Flatten` with `Stop{level}` bound to the live openings — the shape the adapter builds host-side today (pine_adapter.cpp:13269-13290). New hashed `RequestDefinition::origin { Host, KernelLiquidation, KernelRisk }`, new `CancelReason::Superseded` on re-price, `MarginCallEvent` joins `CommandEvent` (17 → 18, native_order.hpp:1308). O's alternative, a checkpoint on the excursion walk (`apply_excursion` native_execution_consumer.cpp:2622), is not chosen: a reduction mid-path is the entanglement O itself flags (E2).
...
- Interim (F): a C++ host can build all four today from `native_marked_equity` + `on_native_applied`, which is what the adapter does (pine_adapter.cpp:13490-13582); a toolkit helper is an acceptable stand-in until L9.
