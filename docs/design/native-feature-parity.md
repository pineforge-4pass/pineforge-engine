...
| MG4 | Initial-margin opening gate (adapter answers `AdmitWithHostMargin` from frozen signal-time equity pine_adapter.cpp:11678-11743 and leaves the kernel gate unset pine_adapter.cpp:11678-11680) | **yes** — `initial_margin_fraction` native_run_spec.hpp:565; `required = resulting_abs_notional * f` against marked equity native_execution_consumer.cpp:2607-2614. **Conversion instant, R5 follow-up lane E21:** both sites of the gate convert at their own point's rate — the candidate at its cursor, the placement gate at the acceptance point where a `Sized{SizeTime::AtAcceptance}` freezes its units, which `make_command_context` native_execution_consumer.cpp:2460 presents for the freeze and the gate — for the notional, the equity and a percent ticket, and the freeze's equity basis with them (`tests/test_native_margin_fx_clock.cpp` section 8) | K | — | F:D1 O:D1 S:M2 | **Disagree:** F, O partial (they fold per-side in) / S yes. Row split → MG5. |
...
`native_run_spec.hpp:238-301` and native_execution_consumer.cpp:3074-3255
...
- TUs: `native_run_spec.{hpp,cpp}`, `native_execution_consumer.cpp` (`admit_opening_inspect` native_execution_consumer.cpp:4172-4205 per side; scheduling next to `invoke_bar_open_callback` / `drain_after_applied`), `native_order.*`, `engine_state_hash.cpp` + the coverage checker, `c_abi.cpp` + `pineforge.h` (`pf_native_run_spec_v2` or an extension struct; `CLAUDE.md` SOP).
...
- TUs: `native_run_spec.cpp` (validation), `native_execution_consumer.cpp` (resolve path native_execution_consumer.cpp:3666-3692, `execute_current` path native_execution_consumer.cpp:3391-3401), `native_matching.hpp` (threshold form). The dead helpers engine.hpp:1341, engine.hpp:1351, engine.hpp:1391 are the reference arithmetic (§2.ii e).
...
- TUs: `native_run_spec.{hpp,cpp}`, `native_execution_consumer.cpp` (`admit_opening_inspect` native_execution_consumer.cpp:4172-4205 and the per-point walk), `native_order.hpp`, the state hash.
