...
| R5-2 | **Brackets are not a new kernel object.** `WaitForApplied{RequestHandle parent}` (native_order.hpp:385-387) and `waiting_children(parent)` (native_order.hpp:1654, *fresh*) exist; L7 delivers a builder/toolkit over them plus relative anchors and the missing offsets. Size M. |
...
|---|---|---|---|---|---|---|
| OL1 | Market / limit / stop / stop-limit / trail triggers (`trigger_for` pine_adapter.cpp:1841-1852; `entry()` pine_adapter.cpp:3694, `order()` pine_adapter.cpp:8800) | **yes** — `Trigger` native_order.hpp:271-290; geometric matching native_execution_consumer.cpp:3397-3445; `submit` native_host.hpp:1073 | K | — | F:A1 O:A1 S:O2 | — |
| OL2 | Intents: flatten, reduce by units, reduce owner-opened units, transact, reverse-to (`close_all()` pine_adapter.cpp:6696 → `Flatten{}` pine_adapter.cpp:6725-6726; `Reduce{ExplicitUnits}` pine_adapter.cpp:5845-5854; `ReverseTo` pine_adapter.cpp:4672-4673) | **yes** — `OrderIntent` native_order.hpp:229, reduce sizes native_order.hpp:229-236, `ReverseTo` native_order.hpp:56-58, `OpeningShape` native_order.hpp:803-807, `Flatten` native_order.hpp:36 | K | — | F:A4 F:A8 O:A2 O:A3 S:O1 | — |
| OL3 | Amend / cancel one working order (`submit_or_replace` pine_adapter.cpp:1855-2380 → `host.replace` pine_adapter.cpp:1871-1937) | **yes** — `replace` native_host.hpp:1080-1081, `cancel` native_host.hpp:1104; `ReplacedEvent` native_order.hpp:906-916; result and event types native_order.hpp:395-416, native_order.hpp:521-583 | K | — | F:A2 O:A4 S:O3 | id → handle map is host bookkeeping (F) → OL4 |
| OL4 | Cancel all / cancel by label; label → handle book (`cancel(id)` pine_adapter.cpp:3107-3153, `cancel_all` pine_adapter.cpp:16113-16128; `live_by_source_key_` pine_adapter.hpp:1382; predecessor state hand-carried pine_adapter.cpp:2239-2303) | **partial** — single-handle `cancel` native_host.hpp:1104; no enumeration (`WorkingRequestCore::live()` is consumer-internal native_order.hpp:1524); labels exist but are unindexed (`Request::label` native_order.hpp:434) **Closed:** `cancel_all` native_host.hpp:1122, `cancel_where(comment)` native_host.hpp:1127 and `cancel_where(text, NativeRequestField::Label)` native_host.hpp:1127 withdraw by predicate and answer how many left the book; `native_working_requests()` native_host.hpp:1118 is the enumeration; the id → handle book is `native_toolkit::OrderBook<Key>`, host-side and Pine-free (`tests/test_native_order_ergonomics.cpp`, `tests/test_native_anchored_legs.cpp`). | K+A | L7 | F:A3 F:A2 O:A5 O:A14 S:O3 | **Disagree:** F partial / O no (A5) + partial (A14) / S yes (folded into O3). R5-1: cancel-all / by-label is generic → kernel. TV state inheritance across a same-id re-issue stays A. |
| OL5 | Per-point fill capacity, partial fills, residual working quantity (Pine uses only `ImmediateRemaining` pine_adapter.cpp:2591; residual closes pine_adapter.cpp:5221-5572) | **yes** — `Capacity` / `PointBudget` native_order.hpp:328-332, `Remaining` native_order.hpp:506-528, allowances native_order.hpp:506-519 | K | — | O:A16 S:O4 | — |
| OL6 | Ownership: legs bound to a parent / opening(s) / cohort; close the lots of a given entry, FIFO or ANY (`owner_for_close` pine_adapter.cpp:3857-3892, `cohort_for` pine_adapter.cpp:1648; FIFO prefix computed adapter-side pine_adapter.cpp:5519-5536) | **yes** — `Owner` native_order.hpp:413-428, `Authority` native_order.hpp:569-570, `CohortClose` native_order.hpp:566-568, `SelectedExposure` native_order.hpp:574-578, `cohort_open` native_host.hpp:1137, `CancelReason::OwnerGone` native_order.hpp:844 | K | — | F:A6 O:A8 O:A9 S:O5 | FIFO *ordering* is host-computed (O) |
| OL7 | OCA cancel / reduce groups (`group_for` pine_adapter.cpp:1850-1856) | **yes** — `Group` / `GroupEffect` native_order.hpp:415-422, `CancelReason::Group` native_order.hpp:422-427, `ReservationReducedEvent` native_order.hpp:1061-1071 (deferred form native_order.hpp:1061-1071) | K | — | F:A12 O:A6 O:A7 S:O7 | — |
...
| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| TR1 | Trail arm → track → trigger, with a state query | **yes** — `Trail{offset, arm_price}` native_order.hpp:266-269; states native_order.hpp:280-288; arm / track / fire native_execution_consumer.cpp:3409-3432; `NativeTrailState` native_host.hpp:611-616, `trail_state` native_host.hpp:993 | K | — | F:B1 O:B1 S:T1 | — |
...
| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| SZ1 | Explicit units (`Transact{signed}` after a grid floor pine_adapter.cpp:4261-4265, pine_adapter.cpp:4261) | **yes** — `Transact` in `OrderIntent` native_order.hpp:229 | K | — | F:C1 O:C1 S:S1 | — |
...
| SZ7 | Quantity step | **yes** — `quantity_grid` admission native_run_spec.hpp:559, native_order.hpp:1282-1299; host-sized terms snap (`ExecutionGridPolicy` native_order.hpp:71-85); exact grid validation native_execution_consumer.cpp:2768-2771 | K | — | F:C4 O:C5 S:S1 | **Disagree:** F, O partial / S yes. R5-3: the quantity grid is native already. L3's `Sized` intent reuses the snap policy. |
...
| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| MG1 | Allowed opening directions (pine_adapter.cpp:1471) | **yes** — `allowed_open_directions` native_run_spec.hpp:564 → `MatchRejectReason::OpeningDirection` native_execution_consumer.cpp:2550-2558 | K | — | F:D4 O:D7 S:M1 | the one risk rule the adapter delegates (O) |
| MG2 | Max absolute position size | **yes** — `max_abs_units` native_run_spec.hpp:562 tests the resulting book native_execution_consumer.cpp:2557-2560 | K+A | — | F:D5 O:D5 S:M1 S:M5 | **Disagree:** O, S:M5 partial. R5-1: the generic cap is native; the adapter's live-position `>= held` variant (pine_adapter.cpp:10808-10811) is TV. |
| MG3 | Open-lot cap (pyramiding) | **yes** — `max_open_lots` native_run_spec.hpp:563 → `MaxOpenLots` native_execution_consumer.cpp:2562-2565 | K+A | — | F:A5 O:A13 O:D6 S:M1 | **Disagree:** F, O:A13 partial / O:D6, S yes. R5-1: Pine's per-cycle entry count (pine_adapter.cpp:4235-4281, cap left unset pine_adapter.cpp:1468-1470) is TV. |
| MG4 | Initial-margin opening gate (adapter answers `AdmitWithHostMargin` from frozen signal-time equity pine_adapter.cpp:11568-11633 and leaves the kernel gate unset pine_adapter.cpp:11568-11570) | **yes** — `initial_margin_fraction` native_run_spec.hpp:565; `required = resulting_abs_notional * f` against marked equity native_execution_consumer.cpp:2006-2013 | K | — | F:D1 O:D1 S:M2 | **Disagree:** F, O partial (they fold per-side in) / S yes. Row split → MG5. |
...
| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| CT1 | Calculate at script-bar close | **yes** — `on_native_bar` native_host.hpp:825, dispatched native_execution_consumer.cpp:6237-6259 (call site native_execution_consumer.cpp:6237) | K | — | F:E1 O:F1 | — |
...
| CT3 | Pre-match hook at bar open (`on_bar_open` pine_adapter.cpp:14351) | **yes** — `on_native_bar_open` native_host.hpp:821 runs before the open match native_execution_consumer.cpp:6083-6087; `execute_current` legal native_host.hpp:1005 | K | — | F:E3 O:F2 | hazard → CT4 |
...
| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| FP1 | Trigger geometry on the modeled path; fill at the level or at the gap price; market-if-touched | **yes** — native_execution_consumer.cpp:3397-3445, native_matching.hpp:59-83; kind native_execution_consumer.cpp:3689-3697; `Limit::fill_through` native_order.hpp:239-244 enforced native_execution_consumer.cpp:4429-4441, re-checked after host terms native_execution_consumer.cpp:4429-4448 | K | — | F:G3 O:H1 O:H2 O:H3 | — |
| FP2 | Slippage in ticks, limit-clamped | **yes** — `slippage_ticks` native_run_spec.hpp:551 applied once native_execution_consumer.cpp:4040-4041, clamp native_execution_consumer.cpp:4040 | K+A | — | F:G1 O:H7 S:F2 | TV's round-then-slip order (pine_adapter.cpp:9553-9562) is A |
...
| RP8 | Continuation hash | **yes** — `native_continuation_hash` native_host.hpp:1199, native_execution_consumer.cpp:9044; the spec is folded by `hash_spec` native_execution_consumer.cpp:121-142 | K | — | F:I1 O:J2 S:X3 | — |
...
- Mechanism (F): a **kernel-originated request**. At each script-bar open and after each applied fill the kernel computes the liquidation level and rests a `Reduce` / `Flatten` with `Stop{level}` bound to the live openings — the shape the adapter builds host-side today (pine_adapter.cpp:11426-11447). New hashed `RequestDefinition::origin { Host, KernelLiquidation, KernelRisk }`, new `CancelReason::Superseded` on re-price, `MarginCallEvent` joins `CommandEvent` (17 → 18, native_order.hpp:1169). O's alternative, a checkpoint on the excursion walk (native_execution_consumer.cpp:1769-1782), is not chosen: a reduction mid-path is the entanglement O itself flags (E2).
...
- A leg with owner `WaitForApplied` / `BindOpening` and a non-absolute anchor gets its level when the owner's fill arms it (`ArmedEvent` native_order.hpp:1105-1112 already exists) (O). Fallback with no kernel change: the toolkit materializes relative levels from the host's `on_native_applied` (F).
