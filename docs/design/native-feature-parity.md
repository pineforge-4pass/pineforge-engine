...
|---|---|---|---|---|---|---|
| OL1 | Market / limit / stop / stop-limit / trail triggers (`trigger_for` pine_adapter.cpp:1885-1896; `entry()` pine_adapter.cpp:3745, `order()` pine_adapter.cpp:8886) | **yes** — `Trigger` native_order.hpp:296-315; geometric matching native_execution_consumer.cpp:3397-3445; `submit` native_host.hpp:1073 | K | — | F:A1 O:A1 S:O2 | — |
...
| OL8 | Immediate execution at the current point (`immediately=true`) | **yes** — `inspect_current_execution` / `execute_current` native_host.hpp:1005-1006 | K | — | F:A9 | (single-source row; O:F2 cites the same call) |
...
| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| TR1 | Trail arm → track → trigger, with a state query | **yes** — `Trail{offset, arm_price}` native_order.hpp:290-293; states native_order.hpp:280-288; arm / track / fire native_execution_consumer.cpp:3409-3432; `NativeTrailState` native_host.hpp:611-616, `trail_state` native_host.hpp:993 | K | — | F:B1 O:B1 S:T1 | — |
...
| SZ2 | Host override of the resolved price / units (`resolve_terms` pine_adapter.cpp:9310-10487) | **yes** — `HostSized` native_order.hpp:73-78 + `resolve_execution_terms` native_host.hpp:878-882, invoked native_execution_consumer.cpp:4346-4364, guarded native_execution_consumer.cpp:4346-4380 | K | — | F:C2 O:H9 S:S2 | **Disagree:** S "partial" (a hook is not a basis). The hook is native; bases are SZ3-SZ5. It stays the custom-policy seam after L3. |
...
| MG2 | Max absolute position size | **yes** — `max_abs_units` native_run_spec.hpp:562 tests the resulting book native_execution_consumer.cpp:2569-2572 | K+A | — | F:D5 O:D5 S:M1 S:M5 | **Disagree:** O, S:M5 partial. R5-1: the generic cap is native; the adapter's live-position `>= held` variant (pine_adapter.cpp:10808-10811) is TV. |
| MG3 | Open-lot cap (pyramiding) | **yes** — `max_open_lots` native_run_spec.hpp:563 → `MaxOpenLots` native_execution_consumer.cpp:2574-2577 | K+A | — | F:A5 O:A13 O:D6 S:M1 | **Disagree:** F, O:A13 partial / O:D6, S yes. R5-1: Pine's per-cycle entry count (pine_adapter.cpp:4235-4281, cap left unset pine_adapter.cpp:1468-1470) is TV. |
...
| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| CT1 | Calculate at script-bar close | **yes** — `on_native_bar` native_host.hpp:825, dispatched native_execution_consumer.cpp:6299-6321 (call site native_execution_consumer.cpp:6299) | K | — | F:E1 O:F1 | — |
...
| HT6 | Self-aggregation recipe (interim) | **yes** — `TimeframeAggregator` is public and engine-free timeframe.hpp:288-486, fed from `on_native_input` native_host.hpp:808; Pine uses the same class pine_scheduler_native.cpp:117-128 | K | L0 (document) | O:G-4 F:F1 | R5-6 |
...
| FP2 | Slippage in ticks, limit-clamped | **yes** — `slippage_ticks` native_run_spec.hpp:551 applied once native_execution_consumer.cpp:4102-4103, clamp native_execution_consumer.cpp:4102 | K+A | — | F:G1 O:H7 S:F2 | TV's round-then-slip order (pine_adapter.cpp:9553-9562) is A |
...
| RP8 | Continuation hash | **yes** — `native_continuation_hash` native_host.hpp:1199, native_execution_consumer.cpp:9125; the spec is folded by `hash_spec` native_execution_consumer.cpp:121-142 | K | — | F:I1 O:J2 S:X3 | — |
...
| Engine reuse | none: generalize the consumer's one script bucket (`ScriptBucket` native_execution_consumer.hpp:254-268, `contribute_input` native_execution_consumer.cpp:6876, `seal_script` native_execution_consumer.cpp:6847) to N; rejects exposing the `request.security` path as Pine-shaped end to end | promote the existing evaluator + feed machinery; edits `engine_security.cpp`, `engine_aux_security.cpp`, `engine_lower_tf.cpp` | same TUs + `native_calendar.cpp` | The existing feed machinery, **called, not edited**; `engine_security.cpp` untouched |
| Pump | consumer seal logic | accepted-input path (`invoke_input_callback` native_execution_consumer.cpp:6177), only for natively registered series | copy at Ready, evaluators created before the first input | O's registration-gated pump + S's copy-at-Ready |
...
5. **Interim, documented in L0:** `on_native_input` + `TimeframeAggregator` (HT6). Doc notes (O): `set_native_security_feed` is public but inert without evaluators and throws in-run (`guard_native_mutation` engine_aux_security.cpp:78, which latches `UnsupportedSource` native_execution_consumer.cpp:1378); `set_aux_security_feed` returns false in the kernel engine_consumer.cpp:158-160.
...
|---|---|---|
| the page's opening, and its "close-only" claims | callbacks are close-only | The kernel has a bar-open hook (`on_native_bar_open` native_host.hpp:821), a tick hook (`on_native_tick` native_host.hpp:811), a post-fill hook (`on_native_applied` native_host.hpp:869), a recalculation hook (`on_native_recalculate` native_host.hpp:844) and a sub-bar hook (`on_native_sub_bar` native_host.hpp:859). The page's opening now tables all eleven, and `on_native_applied` is documented as the calculate-on-fill hook with suffix eligibility |
...
| added | — | the self-aggregation recipe beside the subscriptions it is an alternative to; the open-bar lookahead warning with `current_partial_bar` native_host.hpp:974; and `set_native_security_feed` documented as the pre-run door for a series' authoritative bars, refused in-run |
...
| Tier | Lanes | Why |
|---|---|---|
| Day-one blockers (O) | L1, L2, L3, L10 | Without L2 `fill_report` silently returns an empty curve and degenerate equity metrics (engine_metrics.cpp:3) and drops an open final position. Without L3 every non-unit order needs a hand-written `resolve_execution_terms`, and a mistake is a run-terminating `TermsUnresolved` (native_execution_consumer.cpp:4375-4377). Without L1 "kernel" is a claim, not an artifact. L10 is the minimum buildable proof. |
...
| Q4 | `SizeTime::AtAcceptance` needs a current execution point (native_host.hpp:1162); between realtime inputs there is none — reject or fall back to `AtMatch`? | F: reject with `InvalidQuantityBasis` |
...
- `KernelRecorded`: the consumer calls the existing `update_equity_extremes()` + `record_equity_point(script_open_ms)` (engine.hpp:1466, engine.hpp:1466) once per script calculation (after `invoke_callback` native_execution_consumer.cpp:6275, native_execution_consumer.cpp:6275, native_execution_consumer.cpp:6275) and after the `AfterCalculation` close; appends the per-bar broker hash when recording is on; synthesizes the open-position row at run end / `stream_end` with the existing `build_close_trade_with_costs` (engine_orders.cpp:84).
- Publish read-only `closed_trade_*` accessors (RP6).
- Alternative: F's virtual `owns_equity_recording()` (the A48 ownership pattern of `owns_lot_excursions` native_host.hpp:953). Same semantics, but it costs a host-epoch bump, so the spec form was chosen; the virtual was never added, and `NativeReportPolicy` is what landed (§3.4 row 3).
...
4. `current_partial_bar()` derives from the driver points already recorded (`record_driver` native_execution_consumer.cpp:2278); valid in bar-open, applied and tick callbacks.
...
- Breach: `MatchRejectReason::RiskLimit` for openings (O; F: `RiskHalted`), an optional kernel-issued flatten through A.3's kernel-originated request (`origin = KernelRisk`, F), and a `NativeRiskEvent` in `native_events` (native_host.hpp:1181) (O) or a callback `on_native_risk_event` (S — a virtual, so only inside an epoch batch). The day key uses the existing calendar (native_calendar.hpp:363-366), not a chart clock.
