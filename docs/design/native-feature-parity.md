...
| R5-2 | **Brackets are not a new kernel object.** `WaitForApplied{RequestHandle parent}` (native_order.hpp:436-438) and `waiting_children(parent)` (native_order.hpp:1895, *fresh*) exist; L7 delivers a builder/toolkit over them plus relative anchors and the missing offsets. Size M. |
...
|---|---|---|---|---|---|---|
| OL1 | Market / limit / stop / stop-limit / trail triggers (`trigger_for` pine_adapter.cpp:2600-2611; `entry()` pine_adapter.cpp:5325, `order()` pine_adapter.cpp:10598) | **yes** — `Trigger` native_order.hpp:296-315; geometric matching `first_region_entry` native_execution_consumer.cpp:5710-5757; `submit` native_host.hpp:1213 | K | — | F:A1 O:A1 S:O2 | — |
| OL2 | Intents: flatten, reduce by units, reduce owner-opened units, transact, reverse-to (`close_all()` pine_adapter.cpp:8363 → `Flatten{}` pine_adapter.cpp:8392-8393; `Reduce{ExplicitUnits}` pine_adapter.cpp:7479-7488; `ReverseTo` pine_adapter.cpp:6319-6320) | **yes** — `OrderIntent` native_order.hpp:229, reduce sizes `ReductionSize` native_order.hpp:215, `ReverseTo` native_order.hpp:60-62, `OpeningShape` native_order.hpp:910-914, `Flatten` native_order.hpp:36 | K | — | F:A4 F:A8 O:A2 O:A3 S:O1 | — |
| OL3 | Amend / cancel one working order (`submit_or_replace` pine_adapter.cpp:2570-3777 → `host.replace` pine_adapter.cpp:2586-2666) | **yes** — `replace` native_host.hpp:1218-1220, `cancel` native_host.hpp:1249; `ReplacedEvent` native_order.hpp:1017-1031; result and event types `SubmitStatus` native_order.hpp:835-861, `ReplacedEvent` native_order.hpp:1001-1067 | K | — | F:A2 O:A4 S:O3 | id → handle map is host bookkeeping (F) → OL4 |
| OL4 | Cancel all / cancel by label; label → handle book (`cancel(id)` pine_adapter.cpp:4596-4642, `cancel_all` pine_adapter.cpp:18238-18253; `live_by_source_key_` pine_adapter.hpp:2000; predecessor state hand-carried `projection_predecessor` pine_adapter.cpp:3927-3992) | **partial** — single-handle `cancel` native_host.hpp:1249; no enumeration (`WorkingRequestCore::live()` is consumer-internal native_order.hpp:1703); labels exist but are unindexed (`Request::label` native_order.hpp:485) **Closed:** `cancel_all` native_host.hpp:1267, `cancel_where(comment)` native_host.hpp:1272 and `cancel_where(text, NativeRequestField::Label)` native_host.hpp:1272 withdraw by predicate and answer how many left the book; `native_working_requests()` native_host.hpp:1263 is the enumeration; the id → handle book is `native_toolkit::OrderBook<Key>`, host-side and Pine-free (`tests/test_native_order_ergonomics.cpp`, `tests/test_native_anchored_legs.cpp`). | K+A | L7 | F:A3 F:A2 O:A5 O:A14 S:O3 | **Disagree:** F partial / O no (A5) + partial (A14) / S yes (folded into O3). R5-1: cancel-all / by-label is generic → kernel. TV state inheritance across a same-id re-issue stays A. The adapter's `strategy.cancel` / `strategy.cancel_all` keep their per-handle loops, measured by R5 lane F7 (audit M27): TradingView cancels in the SOURCE book, which holds commands of the same script call that have not reached the kernel yet (and a live `strategy.close` is labelled `__close__<id>` there), so at the same point `cancel_where(id, NativeRequestField::Label)` and `cancel_all()` withdraw 0 — the close flattens at the next open, the reversal lands — where the source cancels withdraw them (`tests/test_adapter_brackets_relower.cpp`, M27 rows); each withdrawn source handle is also retired one by one. |
| OL5 | Per-point fill capacity, partial fills, residual working quantity (Pine uses only `ImmediateRemaining` pine_adapter.cpp:4052; residual closes `enqueue_pooc_fifo_close` pine_adapter.cpp:7151-7501) | **yes** — `Capacity` / `PointBudget` native_order.hpp:379-383, `Remaining` native_order.hpp:557-579, allowances `Allowance` native_order.hpp:662-675 | K | — | O:A16 S:O4 | — |
| OL6 | Ownership: legs bound to a parent / opening(s) / cohort; close the lots of a given entry, FIFO or ANY (`owner_for_close` pine_adapter.cpp:5451-5486, `cohort_for` pine_adapter.cpp:2512; FIFO prefix computed adapter-side `source_fifo_prefix_openings` pine_adapter.cpp:7550-7567) | **yes** — `Owner` native_order.hpp:464-479, `Authority` native_order.hpp:620-621, `CohortClose` native_order.hpp:617-619, `SelectedExposure` native_order.hpp:625-629, `cohort_open` native_host.hpp:1282, `CancelReason::OwnerGone` native_order.hpp:949 | K | — | F:A6 O:A8 O:A9 S:O5 | FIFO *ordering* is host-computed (O) |
| OL7 | OCA cancel / reduce groups (`group_for` pine_adapter.cpp:2757-2763) | **yes** — `Group` / `GroupEffect` native_order.hpp:466-473, `CancelReason::Group` native_order.hpp:948-953, `ReservationReducedEvent` native_order.hpp:1191-1201 (deferred form `DeferredGroupAdjustmentEvent` native_order.hpp:1211-1221) | K | — | F:A12 O:A6 O:A7 S:O7 | — |
| OL8 | Immediate execution at the current point (`immediately=true`) | **yes** — `inspect_current_execution` / `execute_current` native_host.hpp:1104-1105 | K | — | F:A9 | (single-source row; O:F2 cites the same call) |
| OL9 | Birth eligibility: an order accepted at bar N cannot fill on N's already-delivered points | **yes** — `point_eligible` native_order.hpp:786-790, the generic birth record `OrderBirth` include/pineforge/order_birth.hpp:59, contract `docs/pages/native-engine.md` ("Acceptance is not a fill") | K+A | — | F:A13 O:A10 S:R4 | **Disagree:** F yes / O partial / S "no, TV quirk". R5-1: the generic core (birth cursor) is native; TV's bar-granular first-bar rule is the A half → OL14. |
| OL10 | TP/SL bracket per entry, armed before the entry fills, OCA between legs (`exit()` pine_adapter.cpp:13134; lowering `submit_one` pine_adapter.cpp:9158-9170, `BindCohort` pine_adapter.cpp:9659-9665, `submit_leg` pine_adapter.cpp:9683-9707; lifecycle `select_exit_suspension` exit_lifecycle.cpp:10-66) | **partial** — every primitive is native (`WaitForApplied{parent}` native_order.hpp:436-438, `Reduce{OwnerOpenedUnits}` native_order.hpp:220-224, `Member{…, Cancel}` native_order.hpp:468-475, `waiting_children` native_order.hpp:1895); no builder **Closed:** the builder is `native_toolkit::submit_bracket(host, BracketSpec)` — take-profit, stop-loss and trail as one parent's `WaitForApplied` children in one OCA group, with `anchor_rounding` and `visibility` knobs — and it adds no kernel behaviour (`tests/test_native_toolkit_bracket.cpp`). | K (toolkit) | L7 | F:A10 O:A8 S:O6 | **Disagree:** F "yes (primitives)" + toolkit / S partial + new kernel bracket object, lane size L and prerequisite for everything. R5-2: builder over the existing primitives, size M. |
| OL11 | Bracket levels relative to the parent's fill (profit / loss ticks; derived `profit_ticks` pine_adapter.cpp:8731-8734; `materialize_relative_exits` pine_adapter.cpp:17528-17569, pine_adapter.hpp:1813-1825) | **partial** — composable: the host computes levels in `on_native_applied` and submits; suffix eligibility makes that same-bar correct `born_on_remaining_path` native_execution_consumer.cpp:5636-5670. `Trigger` itself is absolute-only native_order.hpp:296 **Closed:** `Request::anchor = FromOwnerFill{offset, ticks, rounding}` (`TriggerAnchor` native_order.hpp:336) defers the level to the owner's fill, the kernel materializes it once at the arm under `NativeAnchorRounding`, and `resolve_anchored_level` native_host.hpp:1012 is where a source language's own level arithmetic lives (`tests/test_native_anchored_legs.cpp`, `tests/test_native_arm_options.cpp`). | K | L7 | F:A11 O:A11 S:O6 | **Disagree:** F partial (toolkit is enough) / O no (kernel `TriggerAnchor`). R5-2: L7 delivers relative anchors; kernel anchor primary, toolkit materialization is the zero-kernel fallback. |
| OL12 | Working-order view for the host and outside observers (`strategy_pending_order_*`; `PendingIntentView` pine_adapter.hpp:1231-1247; the POD is filled only by the adapter `pine_birth_reach` pine_adapter.cpp:19301-19330) | **no** — base returns 0 / -1 `observe_last_bar_dual_entry_path_v1` engine_consumer.cpp:124-133; the POD is TV-shaped (`tv_carry_qty` pending_order_mirror.hpp:73, `pine_birth_reach` pending_order_mirror.hpp:135) **Closed for a bare host:** `native_working_requests()` native_host.hpp:1263 returns one owning `NativeWorkingRequest` native_host.hpp:681 per live request — definition, remaining, trigger state — with the C spelling `strategy_native_working_len_v1` / `_get_v1`. The TV-shaped POD stays frozen and stays the source layer's projection (`tests/test_native_limit_fill_through.cpp`, `tests/test_native_arm_options.cpp`, `tests/test_native_c_api.c`). | K+A | L7 (C++), L13 (C) | F:J5 O:J6 S:X4 | **Disagree:** O "partial + TV-shaped" / F, S no. A bare host reads zeros → no. R5-1: neutral view → kernel; POD shape and Pine journal retention stay A (POD frozen, §2.ii j). |
| OL13 | *(quirk)* Same-bar command batching and the seven deferred queues (`pending_bracket_legs_` pine_adapter.hpp:2001-2019); KI-62 open-order priority (`select` order_priority.cpp:13-60, selected `update_l4c_priority` pine_adapter.cpp:975-1054); per-bar priced-entry throttle, BUY-before-SELL batch, short-seed plan / collision (`maybe_activate_short_seed_plan` pine_adapter.cpp:3331-3498, `materialize_snapshot` pine_adapter.cpp:10998-11022), reversal-gap bracket policy (`ShortSeedPlan` pine_adapter.hpp:1151-1178, `DeferredOpenMarketableSell` pine_adapter.hpp:1563-1586, `CloseCallsiteState` pine_adapter.hpp:1657-1672; `apply_reversal_gap_bracket_policy` pine_adapter.cpp:15761, `reaccept_gapped_bracket_behind_same_id_add` pine_adapter.cpp:15995) | n/a — native priority is acceptance / incarnation order `native_events` native-engine.md:667-669 | A | — | F:A15 O:A15 S:O9 S:R3 | R5-1 names KI-62. S: the kernel exposes deterministic insertion order only. |
| OL14 | *(quirk)* Exit activation / suspension / revival barriers, historical birth reach (`select_exit_activation` exit_activation.cpp:42-88, enforced `leg_activation` pine_adapter.cpp:869-874; `select_exit_activation` exit_activation.cpp:18-88, `select_historical_birth_reach` order_birth.cpp:5-14, `capture_order_birth` pine_adapter.cpp:714-746) | n/a | A | — | S:R4 O:A10 | TV half of OL9. **V19D-P1 (R5 lanes B-ADAPTER, V19-FIX):** a margin-call revival never restores an exit the script cancelled: `strategy.cancel` / `cancel_all` retire every leg of each exit row they name — the live ones they take off the book (caused at the Cancelled event, after anything the same callback did to the leg), the dormant ones, and the ones the adapter took off the book itself — and drop a gapped stop a declined reversal parked for the next margin-call slice (`retire_cancelled_exits` pine_adapter.cpp:1677); the revival skips a withdrawn exit (`revive_brackets_after_margin` pine_adapter.cpp:1751). What `tests/test_adapter_margin_revival_cancel.cpp` replays on the NYSE:F 15m chart of seven lab tv tapes: badapter-v19dp1-control (no cancel: the margin call closes 928 through X) and v19fix-gapped-stop-control (X's stop at 10.15, gapped by the 05-01 open and parked: the same close), and badapter-v19dp1-cancel, v19fix-cancel-before-pair, v19fix-cancel-at-1000, v19fix-gapped-stop-cancel and v19fix-gapped-stop-cancel-after-pair (a cancel on 05-01 09:30 while X is dormant, on 04-30 15:45 before the reversal pair and on 04-30 10:00 while X is live, and on 05-01 09:30 and after the pair while X is parked; one tape file: margin calls 12 @10.39 and 92 @10.57, 836 open). Untaped, the same row holds the tapes' trades for `strategy.cancel_all` on 05-01 and before the pair and for `strategy.cancel` after the pair, and on the same bars with no quantity grid — where the pair holds X in the cancel's own callback — every spelling books what the cancel on the dormant exit books |

### 1.2 Trailing

| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| TR1 | Trail arm → track → trigger, with a state query | **yes** — `Trail{offset, arm_price}` native_order.hpp:290-293; states `TrailWaitArm` native_order.hpp:642-660; arm / track / fire `TrailWaitArm` native_execution_consumer.cpp:5722-5744; `NativeTrailState` native_host.hpp:669-674, `trail_state` native_host.hpp:1092 | K | — | F:B1 O:B1 S:T1 | — |
| TR2 | Trail activation / offset in ticks or points, relative to the entry fill (`trail_points_to_ticks` include/pineforge/compat/pine/trail_ticks.hpp:39 + grid snap exit_lifecycle.cpp:22-27; `trail_activation_level` pine_adapter.hpp:284-286) | **partial** — absolute prices and a raw price distance only **Closed:** `TrailTicks{n}` (`Trail::ticks` native_order.hpp:293) is resolved against `NativeRunSpec::price_tick` at acceptance, so a tick-spelled trail and its price-spelled equal behave identically bar for bar; a relative arm threshold is the `FromOwnerFill` anchor of OL11 (`tests/test_native_order_ergonomics.cpp`, `tests/test_native_anchored_legs.cpp`). | K | L7 | F:B2 O:B2 S:T2 | **Disagree:** S classes all trail operand conversion as TV quirk. R5-1: tick / relative spelling is generic. |
| TR3 | Zero-distance trail, "ride the best, exit on any adverse tick" (adapter substitutes `tick * 0.5` pine_adapter.cpp:11909-11921) | **no** — the kernel fails the run when `offset <= 0` (`checked_trail_stop` → native_execution_consumer.cpp:4473-4478) **Closed:** a zero offset is legal and means *ride the best* — the level is the running best itself and the exit is the first print strictly past it; negative and nonfinite offsets stay rejected (`tests/test_native_order_ergonomics.cpp`, `tests/test_native_anchored_legs.cpp`). | K+A | L7 | O:B3 F:B3 S:T2 | **Disagree:** O kernel / F, S quirk. R5-1: accepting a zero offset is generic; TV zero-offset *pricing* (`zero_trail_sibling_stop` pine_adapter.cpp:11822-11959) stays A. |
| TR4 | Re-price a trail without losing the running extreme (adapter skips `replace` and reads `trail_state()` pine_adapter.cpp:3884-3930) | **partial** — the read exists `trail_state` native_host.hpp:1092; `replace` always rebuilds trigger state **Closed:** `replace(handle, request, ReplaceOptions{/*retain_trigger_state=*/true})` carries the predecessor's live trigger state — a tracking trail's best, an already active stop — into the successor, refusing the replacement when the retained best leaves no representable level (`tests/test_native_order_ergonomics.cpp`, `tests/test_native_anchored_legs.cpp`). | K+A | L7 | O:B4 F:B3 | Status is single-source (O). R5-1: retain-trail replace is generic; TV's "retained best across re-issue" convention (`retained_trail_best` pine_adapter.hpp:287-289) is A. |
| TR5 | *(quirk)* TV trail conventions: half-tick arm threshold against tick-quantized extremes `has_trail_request` pine_adapter.cpp:9709-9714, zero-offset pricing `zero_trail_sibling_stop` pine_adapter.cpp:11822-11959, retained best `retained_trail_best` pine_adapter.hpp:287-289, operand / restart rules `source_trail_points` pine_adapter.cpp:8701-8927, `trail_one_shot` pine_adapter.cpp:9798-9927 | n/a | A | — | F:B3 S:T2 | R5-1 |

### 1.3 Sizing bases and reservations

| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| SZ1 | Explicit units (`Transact{signed}` after a grid floor pine_adapter.cpp:5937-5941, pine_adapter.cpp:5937) | **yes** — `Transact` in `OrderIntent` native_order.hpp:229 | K | — | F:C1 O:C1 S:S1 | — |
| SZ2 | Host override of the resolved price / units (`resolve_terms` pine_adapter.cpp:11019-12223) | **yes** — `HostSized` native_order.hpp:73-78 + `resolve_execution_terms` native_host.hpp:945-949, invoked `resolve_execution_terms` native_execution_consumer.cpp:4958, guarded by `consume_matched_request`'s `try` native_execution_consumer.cpp:4935-4969 | K | — | F:C2 O:H9 S:S2 | **Disagree:** S "partial" (a hook is not a basis). The hook is native; bases are SZ3-SZ5. It stays the custom-policy seam after L3. |
| SZ3 | Percent-of-equity sizing (`default_sizing_units` pine_adapter.cpp:2458-2513) | **no** — `HostSized` without host units is `TermsUnresolved` native_execution_consumer.cpp:4987-4989 **Closed:** `Sized{Side, EquityFraction{f}}` (`OrderIntent` native_order.hpp:229) is the sixth intent — the kernel resolves the units from a fraction of marked equity at the sizing point, so a bare host needs no terms override (`tests/test_native_sizing_bases.cpp`). | K+A | L3 | F:C3 O:C2 S:S3 | TV parts stay A: rounded equity `source_money_round` pine_adapter.cpp:2558, money floor-to-lot `source_money_floor_lot` pine_adapter.cpp:463 (→ SZ10) |
| SZ4 | Cash-value sizing (`default_qty_type` pine_adapter.cpp:2567-2569) | **no** — same **Closed:** `Sized{Side, CashValue{cash}}`, the same resolution over an account-currency amount (`tests/test_native_sizing_bases.cpp`). | K | L3 | F:C2 O:C3 S:S4 | A per-call `qty_type` (cash or percent of equity on one `strategy.entry`) is not Pine — TradingView's compiler refuses it, so only generated or C++ callers reach it — and since R5 lane B-ADAPTER it converts through the kernel's `native_sized_units` exactly as the declared default does (`typed_quantity_units` pine_adapter.cpp:2619, `typed_entry_units` pine_adapter.cpp:2637; `tests/test_adapter_typed_entry_admission.cpp`) |
| SZ5 | Fee-net sizing (commission reserve `/(1+c)`, asked of the kernel as `reserve_percent_fee` pine_adapter.cpp:2450; `percent_commission_live_equity` pine_adapter.cpp:4191-4232) | **no** — only a dead primitive, `reserve_percent_commission` (since deleted), with no caller in `src/` **Closed:** `Sized::reserve_percent_fee` divides the sizing cash by `1 + fee_value / 100` under `NativeFeeKind::Percent` and is an exact no-op for every other fee kind; the dead `reserve_percent_commission` primitive is gone (`tests/test_native_sizing_bases.cpp`, `tests/test_native_sizing_price_rule.cpp`). | K | L3 | F:C3 O:C6 S:S3 | **Disagree:** F lists the reserve under quirk yet offers a knob / O kernel, "partial" / S knob. R5-1: fee-net option is generic. TradingView's cash-commission reserve stays adapter policy over the kernel's `CashValue`: a percent-of-equity default quantity under a cash-per-order or cash-per-contract fee takes its percentage of `strategy.equity` (`PineSizingSnapshot::strategy_equity`, the open entries' cash fees charged) and leaves out the fee its own order pays (`default_sizing_cash`, R5 lane PAR-CASHFEE, §3.10; `tests/test_cash_fee_sizing_tapes.cpp` on the tapes in `tests/fixtures/cash_fee_sizing`) |
| SZ6 | Placement-time vs fill-time sizing (`sizing_snapshot` pine_adapter.cpp:2347-2362, `PineSizingSnapshot` pine_adapter.hpp:147-159; fill-time exceptions `default_sized` pine_adapter.cpp:6722-6748) | **no** **Closed:** `SizeTime::AtMatch` (default) resolves at the matching candidate and `SizeTime::AtAcceptance` freezes the units against the command point, where the run's own opening admission also runs against the frozen quantity (`RequestRejectReason::PlacementAdmission`). Which price the basis converts at is `SizePrice::{Resolved, Signal, SignalOnTick}` (`tests/test_native_sizing_bases.cpp`, `tests/test_native_sizing_price_rule.cpp`). | K+A | L3 | F:C3 O:C7 S:S3 | **Disagree:** O adapter-only / F, S generic timing knob. R5-11 (L3 scope) + R5-1: the knob is kernel; TV's frozen tuple stays A through `HostSized`. |
| SZ7 | Quantity step | **yes** — `quantity_grid` admission native_run_spec.hpp:592, native_order.hpp:1421-1438; host-sized terms snap (`ExecutionGridPolicy` native_order.hpp:71-85); exact grid validation `execution_terms_grid_representable` native_execution_consumer.cpp:5004-5007; the book's own quantities are on the grid -- a whole-scope `ScopeFraction` and a `Reduce` of a FIFO boundary of its scope (R5 lane K-ULP4) | K | — | F:C4 O:C5 S:S1 | **Disagree:** F, O partial / S yes. R5-3: the quantity grid is native already. L3's `Sized` intent reuses the snap policy. |
| SZ8 | Fractional reduce of the position / a cohort (`quantize_close_units` pine_adapter.cpp:12608-12636, `compute_exit_reservation` pine_adapter.cpp:4289-4408) | **no** — sizes are `ExplicitUnits` / `OwnerOpenedUnits` / `HostSized` only native_order.hpp:60-74 **Closed:** `Reduce{ScopeFraction{fraction, claim, basis}}` takes a fraction in `(0, 1]` of the scope the reduce is bound to — the book, one opening, or a cohort roster (`tests/test_native_sizing_bases.cpp`). | K | L3 | F:A7 O:A12 O:C4 S:S5 | F's "partial" counts the units-reduce of OL2. R5-4 → L3. |
| SZ9 | Exit-quantity reservation against sibling exits (`compute_exit_reservation` pine_adapter.cpp:4341-4660, `reserved_exit_qty` pine_adapter.cpp:9075-9395) | **partial** — `PointBudget` native_order.hpp:379-382 and group `Reduce` native_order.hpp:220 give a generic reservation; no net-of-siblings claim for fraction sizes **Closed:** `ScopeClaim::NetOfSiblings` subtracts the units already claimed by live sibling reduces bound to the same scope, and `ScopeBasis::{AtMatch, AtAcceptance}` chooses when that scope is measured (`tests/test_native_sizing_bases.cpp`). | K+A | L3 | F:A14 S:S5 S:O10 O:A12 | F proposed nothing. R5-4: generic half → L3; the Pine ledger → SZ11, SZ12. |
| SZ10 | *(quirk)* Ten-significant-digit money rule (`source_money_round` pine_adapter.cpp:369-376, `source_money_floor_lot` pine_adapter.cpp:463; twin `tv_money_round` pine_policy_support.hpp:9-15); exact percent-exit rounding (`quantize_percent_exit_units` pine_adapter.cpp:4321-4336) | n/a — kernel money is plain binary64 `marked_equity` engine_execution.cpp:1638-1657 | A | — | O:C8 F:C3 S:M4 S:S5 | R5-1 |
| SZ10a | *(quirk, measured retention)* The money band's affordable price, `sig10(sig10(E) / notional per unit of price)` against the sizing price — at placement (`entry`, `affordable_price` pine_adapter.cpp:6783), in the paired-reversal whole drop (`close`, `affordable_price` pine_adapter.cpp:7990), at the terms boundary (`resolve_terms`, `affordable_price` pine_adapter.cpp:12865), in the precommit money scopes (`validate_precommit`, `affordable_price` pine_adapter.cpp:13461, pine_adapter.cpp:13514) and in the carried-money check (`apply_reversal_gap_bracket_policy`, `affordable_price` pine_adapter.cpp:15809) | n/a — not a units conversion. Every money-to-units conversion in the adapter is the kernel's `native_sized_units` under the source's floors since R5 lane B-ADAPTER: the default at placement and at the fill (`default_sizing_units` pine_adapter.cpp:2606), the typed per-call quantity at placement, at the fill and in the probe (`typed_quantity_units` pine_adapter.cpp:2619, `typed_entry_units` pine_adapter.cpp:2637), and the pending-order fill-quantity probe (`probe_fill_qty` pine_adapter.cpp:18896) | A | — | AUDIT3-opus2 H4 (G2 R2, R3) | **Measured:** the quotient is TradingView's whole-drop PRICE, pinned by 445 famr3 tapes (`tests/test_tv_money_band_l4b.cpp`) and 202 famr tapes (`tests/test_tv_money_precision_l4b.cpp`); on a seven-digit tie (E 9998.99999996, 10 000 lots at tick(close) 0.9999) the rule admits while the kernel's conversion buys 9999.99999996 lots and binary64 gives E/Q 0.999899999996 (`tests/test_adapter_fill_qty_probe.cpp`, section 2). |
| SZ11 | *(quirk)* Same-bar `strategy.close` callsite batching, two-call provenance, entry-id ledger (`enqueue_pooc_fifo_close` pine_adapter.cpp:7252-7603; ledger `close_batch_bar_` pine_adapter.hpp:2042-2053; the kernel's comment-only residue was deleted by R5 lanes E6 and E10) | n/a | A | — | S:O8 F:A15 O:A15 | **Disagree:** S "generic → kernel" (its P2 `ReplaceByKey` / `Fifo`). R5-4: adapter. |
| SZ12 | *(quirk)* POOC reservation-growth market-entry population predicate (`select_reservation_growth_sources` src/compat/pine/reservation_expansion.cpp:9-34, called from the adapter, `select_reservation_growth_sources` pine_adapter.cpp:933) | n/a | A | — | S:O10 | (single-source) R5-4 confirms S's own split: concept → SZ9, predicate A. |

### 1.4 Margin, admission, liquidation, risk

| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| MG1 | Allowed opening directions (`allowed_open_directions` pine_adapter.cpp:2372) | **yes** — `allowed_open_directions` native_run_spec.hpp:597 → `MatchRejectReason::OpeningDirection` native_execution_consumer.cpp:2956-2964 | K | — | F:D4 O:D7 S:M1 | the one risk rule the adapter delegates (O) |
| MG2 | Max absolute position size | **yes** — `max_abs_units` native_run_spec.hpp:595 tests the resulting book native_execution_consumer.cpp:2970-2973 | K+A | — | F:D5 O:D5 S:M1 S:M5 | **Disagree:** O, S:M5 partial. R5-1: the generic cap is native; the adapter's live-position `>= held` variant (`max_position_size` pine_adapter.cpp:13356-13359) is TV. |
| MG3 | Open-lot cap (pyramiding) | **yes** — `max_open_lots` native_run_spec.hpp:596 → `MaxOpenLots` native_execution_consumer.cpp:2975-2978 | K+A | — | F:A5 O:A13 O:D6 S:M1 | **Disagree:** F, O:A13 partial / O:D6, S yes. R5-1: Pine's per-cycle entry count (`accepted_in_cycle` pine_adapter.cpp:6167-6216, cap left unset `max_open_lots` pine_adapter.cpp:2194-2196) is TV. **Measured (R5 lane H-MEASURE), and not TradingView's rule:** on 15 scenarios of three `lab tv` tapes (`tests/test_pyramiding_count_differential.cpp`) TradingView checks an entry once, at its first eligible point, against the trades then open; the kernel's `max_open_lots` books 14 as TradingView does, the adapter's count 8 (it counts a resting entry, admits three market entries on a flat bar, keeps a slot another entry's exit drained). The fifteenth, a resting entry accepted below the cap and filled at it, needs a first-eligible-point rule neither side has. A recorded divergence, retained pending a re-lowering ruling. **Re-lowering tried by R5 lane H-THIN and not taken:** on the kernel's cap four corpus probes moved away from TradingView, for want of that first-eligible-point rule, so the count stays. The lane fixed the reservation divergence H-MEASURE measured beside it (P10): a default `strategy.exit(from_entry)` under FIFO reserved the whole book (200 in the adapter, 100 on TradingView's tape); it now reserves the entry's own quantity, the oldest lots first (`fifo_named_entry` pine_adapter.cpp:8918-8924). |
| MG4 | Initial-margin opening gate (adapter answers `AdmitWithHostMargin` from frozen signal-time equity pine_adapter.cpp:13109-13175 and leaves the kernel gate unset pine_adapter.cpp:13354-13356) | **yes** — `initial_margin_fraction` native_run_spec.hpp:598; the requirement `resulting_abs_notional * fraction` native_execution_consumer.cpp:2993 against marked equity. **Conversion instant, R5 follow-up lane E21:** both sites of the gate convert at their own point's rate — the candidate at its cursor, the placement gate at the acceptance point where a `Sized{SizeTime::AtAcceptance}` freezes its units — for the notional, the equity and a percent ticket, and the freeze's equity basis with them. The rate is threaded, never the clock: `make_command_context` native_execution_consumer.cpp:2839 hands the point's rate to the freeze, to `marked_equity_at` and to the engine's rate-explicit settlement inspection `inspect_native_settlement_scoped_at` engine.hpp:649, and the engine's presented clock is never written (`tests/test_native_margin_fx_clock.cpp` sections 8, 10 and 11) | K | — | F:D1 O:D1 S:M2 | **Disagree:** F, O partial (they fold per-side in) / S yes. Row split → MG5. **Measured (R5 lane H-MEASURE, M11):** the two gates part both ways on the same bars — the kernel's declines a 1× long whose entry fee the adapter admits, and admits a gap-up add the adapter refuses against its signal-time equity; `ab9714be` books the adapter's answer both times (`tests/test_adapter_margin_schedule_differential.cpp`). |
| MG5 | Per-side initial margin (`margin_pct` pine_adapter.cpp:14065-14066, `margin_pct` pine_adapter.cpp:13473) | **no** — one `std::optional<double>` `initial_margin_fraction` native_run_spec.hpp:598 **Closed:** `NativeMarginModel::initial_long` / `initial_short` native_run_spec.hpp:242, opt-in and mutually exclusive with `initial_margin_fraction`; `initial_<side> == 0.0` is the maintenance-only spelling that leaves opening admission to the host at both of its gates (`tests/test_native_margin_model.cpp`). | K | L4 | F:D1 O:D2 | R5-1 |
| MG6 | Maintenance margin + forced liquidation (`submit_margin_call_slice` pine_adapter.cpp:14165-14343, `submit_margin_call_units` pine_adapter.cpp:14304-14350, `schedule_margin_call_path` pine_adapter.cpp:18391-18468) | **no** — "no maintenance liquidation" `initial_margin_fraction` native_run_spec.hpp:598-599; margin is checked only when `inspect.would_open` native_execution_consumer.cpp:2952 **Closed:** `maintenance_long` / `maintenance_short` with `NativeLiquidationCheck::{PathAdverseExtreme, PathAdverseExtremeMark, CalculationOnly}`; the kernel solves the level, rests its own `Reduce`/`Flatten` bound to the live book, re-prices it with `CancelReason::Superseded` and books a `MarginCallEvent` (`tests/test_native_margin_model.cpp`, `tests/test_native_margin_hooks.cpp`). | K+A | L4 | F:D2 O:D3 S:M3 | TV parts → MG15 |
| MG7 | Liquidation slice sizing (grid floor, ×4 restore, one-contract fallback `source_margin_units` pine_adapter.cpp:13790-13826) | **no** **Closed:** `NativeLiquidationSizing::{RestoreMinimum, ShortfallMultiple, Flatten}` with `shortfall_multiple` and `liquidation_min_units`, capped at the held quantity and floored onto `quantity_grid`; `resolve_margin_call_units` native_host.hpp:995 gives a host the last word, clamped into `(0, held]` (`tests/test_native_margin_model.cpp`, `tests/test_native_margin_hooks.cpp`). | K+A | L4 | O:D4 F:D2 S:M4 | **Disagree:** F generic knob `ShortfallMultiple` / O "×4 is A" / S quirk, yet a 4.0 default inside its generic model. R5-1: policy knob in the kernel, plain default (restore-minimum). **Decided by R5 lane F7 (the D2 question):** the adapter selects NO kernel knob — `project()` leaves `sizing` / `shortfall_multiple` / `liquidation_min_units` unset and its `resolve_margin_call_units` answers every call with TradingView's slice (the restore floored onto the lot grid BEFORE the ×4, floored again, the one-contract band, the dust gate). Measured: on a one-unit grid, 20 long at 100 on 1000 at 50 % reaching 93.9, TradingView books 4 lots and the kernel's own `ShortfallMultiple 4` books 5 (`tests/test_native_margin_hooks_twin.cpp` MG-F3, an executed run, no longer a comment); without a grid the two agree bit for bit (MG-F/H, 4.2105263157894735), but declaring the knob for those runs alone folds `NativeMarginModel::sizing` into every margin run's identity — an epoch decision F7 does not take. Since R5 lane H-THIN the pre-open admission slice (`schedule_preopen_margin_slice`) sizes through the same `source_margin_units`, where it kept its own copy of the ×4 without the band or the dust gates (AUDIT4 §6 G2-12; `tests/test_adapter_margin_schedule_differential.cpp` M10: band and fudge now 1 and 1, the kernel's and `ab9714be`'s). |
| MG8 | Liquidation price query (`margin_liquidation_price` pine_strategy_host.cpp:114-130) | **no** **Closed:** `native_liquidation_price()` native_host.hpp:1320 answers the exact solved level, `nullopt` where none exists; `NativeLiquidationLevelBase` chooses whether the solve answers for the open entries' fees (`tests/test_native_margin_model.cpp`, `tests/test_native_open_lots.cpp`). | K+A | L4 | F:D3 S:R5 | **Disagree:** F generic / S adapter-only. R5-1: liquidation price is generic; the Pine spelling stays A. **Lowered by R5 lane H-THIN:** the Pine host's second solve (`compute_liquidation_price`) is gone; `strategy.margin_liquidation_price` reads `native_liquidation_price()`, which the adapter configures with `NativeLiquidationLevelBase::RealizedOnly` (the Pine formula's closed-money solve, bit for bit at every bar of the lane's 1,021 seeded margin-call runs), and keeps only TradingView's tick spelling: floored for a long, ceiled for a short. A run whose margin calls are switched off declares no margin model, so it reads na. |
| MG9 | FX revaluation of margin (`apply_fx_open_margin_slice` pine_adapter.cpp:16413-16429, `apply_fx_opening_margin_slice` pine_adapter.cpp:18469-18497) | **no** — staged FX curve `configure_native_fx_curve` native_host.hpp:1202 but no margin re-check. **Closed by audit lane N6:** `NativeMarginCheckKind::FxRoll` — a step of the declared curve is a check point of its own, measured at the unchanged price before the first point the new rate converts (`tests/test_native_margin_fx_roll.cpp`). The adapter refuses the point by kind, for a measured reason since R5 lane F7: TradingView's rollover is a broker-open checkpoint at the open price, now at any positive margin (`apply_fx_open_margin_slice`, on the one TradingView money / units rule every other checkpoint uses), while the roll point measures at the remaining path's adverse mark and rests the slice there — a carried 1x long on a step bar O 100 L 97 books 0.4116 @ 97 on the adapter's run shape and 0.4036 @ 99, measured a bar early, on a strict calendar, against TradingView's 0.3996 @ 100 (`tests/test_native_margin_hooks_twin.cpp` MG-FX). Both N6 follow-ups are closed (§4.4). **Completed by R5 follow-up lane E3:** the conversion INSTANT is now generic too — every check kind converts at its own cursor's rate (`account_currency_fx_at(cursor.point.effective_time_ms)`) for the requirement, the equity and the level, in batch and stream, so a step landing between a fill's cursor and the clock the engine presents no longer breaches at the fill (`tests/test_native_margin_fx_clock.cpp`; N6's local clock swap inside `fx_roll_margin_check` is removed as redundant). **Extended by R5 follow-up lane E21** to the opening gate's placement site (MG4), and the rate-explicit equity E3 restated beside the engine's accessor is now that accessor's one implementation, `BacktestEngine::marked_equity_at` engine.hpp:729 | K | L4 → N6 → E3 → E21 | O:D8 | (single-source) |
| MG10 | Max drawdown halt (`update_risk_state` pine_adapter.cpp:16980-17019) | **no** — observation field only `drawdown_limit` include/pineforge/source/market_admission.hpp:50; composable today from `native_marked_equity` native_host.hpp:1301 **Closed:** `NativeRiskLimits::max_drawdown` native_run_spec.hpp:311, a `NativeLossLimit{value, percent}` measured against the running peak equity at three points of every script bar, blocking openings for the rest of the run (`tests/test_native_risk_limits.cpp`). | K | L9 | F:D6 O:E1 S:M6 | R5-9 |
| MG11 | Max consecutive losing days (`SourceDayLedger` pine_adapter.hpp:1197-1205; gate `max_cons_loss_days` pine_adapter.cpp:13445-13448) | **no** — observation only `loss_days_limit` include/pineforge/source/market_admission.hpp:49 **Closed:** `NativeRiskLimits::max_consecutive_loss_days`, settled from each day's realized result at the next day's open (`tests/test_native_risk_limits.cpp`). | K | L9 | F:D6 O:E2 S:M6 | R5-9 |
| MG12 | Max intraday loss → flatten + block (`intraday_loss_breached` pine_adapter.cpp:14026-14040, forced flatten `submit_intraday_loss_close` pine_adapter.cpp:15052-15082) | **no** — observation only `intraday_loss_limit` include/pineforge/source/market_admission.hpp:51 **Closed:** `NativeRiskLimits::max_intraday_loss` with `NativeRiskAction::FlattenAndBlock`, under which the kernel issues one `Flatten` of its own at the breach point (`RequestOrigin::KernelRisk`) and latches the block before that flatten settles (`tests/test_native_risk_limits.cpp`). | K | L9 | F:D7 O:E3 S:M6 | R5-9 |
| MG13 | Max filled orders per day (`IntradayCap` intraday_cap.hpp:83-241, `IntradayOrderBudget` intraday_order_budget.hpp:27-92; `execute_due_cap_close` pine_adapter.cpp:15095-15329) | **no** **Closed:** `NativeRiskLimits::max_fills_per_day` counts every applied execution of the day as it settles, whatever issued it (`tests/test_native_risk_limits.cpp`). | K+A | L9 | F:D8 O:E4 S:M7 | Count cap K; quota transfer, POOC deferral, TV close price A |
| MG14 | Risk-day boundary (`chart_day_key` pine_adapter.cpp:13836-13915) | **no** — the calendar exists `session_day_ordinal` native_calendar.hpp:416-419, there is no risk-day ledger **Closed:** `NativeRiskDay::{SessionDay, CalendarDayInTimezone}` keys the risk day, and `native_risk_state()` native_host.hpp:1329 reads the whole ledger back — block and reason, day, fills counted, loss-day streak, peak equity, day-opening equity (`tests/test_native_risk_limits.cpp`). | K+A | L9 | O:E5 S:M7 | The chart-day shortcut stays A; the kernel keys on session / calendar day |
| MG15 | *(quirk)* TV margin call: money rounding `source_margin_money` pine_adapter.cpp:13999, 4× shortfall `4.0 * minimum` pine_adapter.cpp:14058, POOC chronology exceptions `competing_entry` pine_adapter.cpp:14553-14593, 1×-long money call `submit_tv_money_long_margin_call` pine_adapter.cpp:14077-14204, fill pricing (the slice books at its waypoint's mark, `submit_tv_money_long_margin_call` pine_adapter.cpp:14328) | n/a | A | — | F:D2 F:G4 O:D4 S:M4 | R5-1. **Measured (R5 lane H-MEASURE)** against the kernel's own points on the same bars and orders, the kernel's hooks answering TradingView's money and slice (`tests/test_adapter_margin_schedule_differential.cpp`): a gap-open breach executes at the open, sized on the open's money, where the kernel's mark check rests at the path's adverse extreme and `CalculationOnly` executes at the calculation (M8); the pre-open slice parts from the kernel's `AfterApplied` point, and from `ab9714be`, on the one-contract band and the restore's `+1e-6` floor (M10; the frozen units it also parted on are TradingView's book since R5 lane PAR-MARGIN froze a default-percent stop above 100 % as eight `lab tv` tapes, `tests/fixtures/margin_entry_bar/pm-m10-*`, show TradingView does); the entry bar of a leveraged opening, where four `lab tv` tapes (`tests/fixtures/margin_entry_bar`) and the kernel's `AfterApplied` point book the margin call at that bar's low, is admitted by `on_applied` since R5 lane PAR-MARGIN on the run shapes `ab9714be` checked it on (the pre-open slice's own opening keeps that slice's verdict), and the adapter books the tapes' rows (M7; H-MEASURE had recorded it as a divergence); R5 lane PAR-MARGIN-2 extended it to limit openings, `process_orders_on_close`, `calc_on_order_fills` and adds to a carried book, and made the kernel's post-fill point reach the low a limit was filled on the way down to, or a short's high on its way up (twenty-eight `lab tv` tapes, `tests/fixtures/margin_entry_bar/pm2-*`). Under the magnifier the kernel's `IntrabarSample` point (R5 lane PAR-MARGIN) is admitted on a leveraged long, plain, under `process_orders_on_close` or `calc_on_order_fills`, and on a short outside `process_orders_on_close` (PAR-MARGIN-2), where TradingView books its call at the first lower-timeframe low that crosses -- at its own 2-minute intrabars on a 15-minute chart, which a model reproduces on all 20 magnified tapes (`tests/fixtures/intrabar_margin`); six tapes part on the one-minute samples the host feeds, a recorded divergence of the magnifier's sampling. A margin call books the nearest tick of its print, as TradingView's market fills do (12.105 books 12.11; `tests/fixtures/half_tick_rounding`, PAR-MARGIN-2). |
| MG16 | *(quirk)* Historical market-admission review / retention journal (`explicit_pair_scope` src/compat/pine/market_admission.cpp:33 and the review fold it feeds (`original_pair_call` src/compat/pine/market_admission.cpp:47); `admission_allocation` pine_adapter.cpp:3680-3747, `record_market_review` pine_adapter.cpp:15333-15391) | n/a | A | — | S:R2 | (single-source) |

### 1.5 Calculation timing

| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| CT1 | Calculate at script-bar close | **yes** — `on_native_bar` native_host.hpp:893, dispatched by `invoke_callback` native_execution_consumer.cpp:7222-7255 (call site `on_native_recalculate` native_execution_consumer.cpp:7247) | K | — | F:E1 O:F1 | — |
| CT2 | Process orders on close (`close_execution` pine_adapter.cpp:2152-2153) | **yes** — `NativeCloseExecution::AfterCalculation` native_run_spec.hpp:35-38; `AfterCalculation` native_execution_consumer.cpp:7632-7636 | K | — | F:E2 O:F6 S:C1 | — |
| CT3 | Pre-match hook at bar open (`on_bar_open` pine_adapter.cpp:16248) | **yes** — `on_native_bar_open` native_host.hpp:889 runs before the open match native_execution_consumer.cpp:7096-7100; `execute_current` legal native_host.hpp:1104 | K | — | F:E3 O:F2 | hazard → CT4 |
| CT4 | Partial-bar view / open-bar lookahead guard | **no** — the open hook receives the *complete* script bar: `invoke_bar_open_callback(engine, bar, point)` native_execution_consumer.cpp:7039 with `engine.current_bar_ = open_view` native_execution_consumer.cpp:7090 (the complete bar unless `OpenOnly`) **Closed:** `current_partial_bar()` native_host.hpp:1073 is the lookahead-free bar so far, through the last path point consumed (a fill inside a segment reads it at the segment's origin), and `NativeOpenBarView::OpenOnly` native_run_spec.hpp:109 masks `on_native_bar_open`'s bar to `H = L = C = open` as presentation only — the complete bar is restored before the open match (`tests/test_native_calc_timing.cpp`). | K | L5 | F:E3 | (single-source) The adapter needs the full bar, so the guard is opt-in. |
| CT5 | Raw input observation before aggregation | **yes** — `on_native_input` native_host.hpp:875, `NativeInputContext` native_host.hpp:773-778 | K | — | O:F3 | (single-source row) |
| CT6 | Calculate on order fills (scheduler `PineScheduler::recalculate` pine_scheduler_native.cpp:579, `coof_recalculation_due` pine_scheduler_native.cpp:554, the first-open chain `kFirstOpenLoopGuard` pine_scheduler_native.cpp:663; `begin_coof_recalc` pine_adapter.cpp:5548, `flush_coof_tail` pine_adapter.cpp:16391, `end_coof_recalc` pine_adapter.cpp:5571) | **partial** — `on_native_applied` native_host.hpp:938 fires mid-path after each fill native_execution_consumer.cpp:6945, native_execution_consumer.cpp:6945-6975; commands are legal there `commands_allowed` native_execution_consumer.cpp:1407-1415; a request born there is eligible on the unconsumed rest of the bar `born_on_remaining_path` native_execution_consumer.cpp:5636-5670, `remaining_path_delivery` native_order.cpp:2061-2069. Missing: calculation re-entry, a documented chronology, a bar-so-far view **Closed:** `NativeCalculationTrigger::BarCloseAndFills` native_run_spec.hpp:96 drives one `OrderFill` recalculation per applied execution at that event's own cursor, delivered as `on_native_recalculate(..., OrderFill, cause)` and bounded by `max_recalculations_per_point` (`tests/test_native_calc_timing.cpp`). | K+A | L5 | F:E4 O:F5 S:C2 | **Disagree:** F partial / O, S no; F, S v1 / O deferrable. R5-5: v1, sub-bar hook + chronology contract, adapter path unchanged. TV parts → CT11. |
| CT7 | Calculate on every tick | **partial** — `on_native_tick` native_host.hpp:878 fires per accepted realtime print native_execution_consumer.cpp:7178-7182; no per-tick calculation in batch; the guide pinned "close-only" (`docs/pages/native-engine.md`, "Batch OHLCV vs ticks vs quiet") **Closed:** `NativeCalculationTrigger::EveryModeledPoint` native_run_spec.hpp:97 adds one `Tick` recalculation at every modeled point of the delivered path and at every observed print, in batch and in a stream alike (`tests/test_native_calc_timing.cpp`). | K | L5 | F:E5 O:F4 S:C3 | **Disagree:** F yes / O partial / S no. R5-5: v1. Not an adapter feature either: absent from `PineStrategyConfig` pine_adapter.hpp:62-76, the runner rejects it (`calc_on_every_tick` main.cpp:213), the Pine stream route refuses `calc_on_order_fills` pine_strategy_host.cpp:203-205. |
| CT8 | Historical intrabar calculation (per lower-TF sub-bar / magnifier sample) | **no** — with an `IntrabarPath` the callback still fires once per script bar native_execution_consumer.cpp:7626-7743 **Closed:** `EveryModeledPoint` recalculates at each intrabar sample, and `on_native_sub_bar` native_host.hpp:928 fires once after each retained lower-timeframe sub-bar's whole matching path (`tests/test_native_calc_timing.cpp`). | K | L5 | F:E6 O:F8 S:C4 | — |
| CT9 | Intrabar (magnifier) matching path + path-order policy (`volume_weighted_cap` pine_adapter.cpp:2281-2337) | **yes** — `IntrabarPath` native_run_spec.hpp:362-403, delivery `deliver_confirmed_script` native_execution_consumer.cpp:7475-7719; `NativePathOrder` native_run_spec.hpp:335-339 | K+A | — | F:E7 F:G5 O:F7 S:C4 | **Disagree:** S partial (it wants callback cadence → CT8). TV sampler choice is A: TradingView's own intrabars at its table's timeframe, each owned by the chart bar of its last minute, built from the finer feed (`tradingview_magnifier_bars` magnifier_intrabars.hpp:44, R5 lane MAG-INTRABAR). |
| CT10 | Batch → warmup → realtime forward execution | **yes** — `stream_begin` engine.hpp:1755, `stream_push_bar` engine.hpp:1761, `stream_end` engine.hpp:1765, `NativeRunPhase` native_host.hpp:41, C ABI `strategy_stream_begin` c_abi.cpp:613 | K+A | — | F:E8 O:J3 S:C5 | **Disagree:** S partial. R5-1: the lifecycle is native; Pine trade-start suppression and realtime-tail / probe rules (`trading_window_active` pine_strategy_commands.cpp:15-21, `prepare_native_begin` pine_strategy_host.cpp:152-238) are A. S's "neutral tail + trade-window policy" is single-source and unassigned (§4 Q8). |
| CT11 | *(quirk)* COOF TV specifics: waypoint-only refill (`coof_next_waypoint` pine_adapter.cpp:5717), two-fills-at-open rule, script-state rollback (`PineScheduler` pine_scheduler_native.cpp:52-104; hooks `snapshot_script_state` engine.hpp:1332-1333), the first-open guard `kFirstOpenLoopGuard` pine_scheduler_native.cpp:663, deferral queue of `PendingCoofRequest` pine_adapter.hpp:1638-1645 | n/a | A | — | F:E4 O:F5 S:C2 | R5-5: the adapter keeps its path |

### 1.6 Higher timeframes and auxiliary feeds

| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| HT1 | HTF series of the same symbol (`request.security`): evaluators registered `scheduler_configure_security_evaluators` pine_strategy_host.cpp:1489-1492, pumped from the scheduler `scheduler_prepare_security_sequence` pine_strategy_host.cpp:1014-1170; state lives in `BacktestEngine` engine.hpp:356-558, engine.hpp:356-402, engine_security.cpp:27 | **no** sanctioned path — `configure_security_evaluators` is an empty virtual engine.hpp:1322 whose only caller is pine_strategy_host.cpp:1490 **Closed:** `NativeRunSpec::subscriptions` (`NativeTimeframeSubscription` native_run_spec.hpp:486), or `declare_timeframe_subscriptions` native_host.hpp:1136 from inside `on_native_run_begin`, with `on_native_timeframe_bar` native_host.hpp:885 and `native_series_bar(index)` native_host.hpp:1119 reading them back; the kernel registers the declared series after that callback returns, so a host's own evaluators survive (`tests/test_native_htf_subscriptions.cpp`, `tests/test_native_htf_subscriptions_stream.cpp`). | K | L6 | F:F1 O:G-1 S:H1 | **Disagree:** S partial (it counts kernel-side storage → HT2); F, S v1 / O deferrable. R5-6: v1. |
| HT2 | Authoritative (exchange) HTF bars substituted into a series (`set_native_security_feed` engine.hpp:1730, engine_aux_security.cpp:78-129, C ABI `strategy_set_native_security_feed` c_abi.cpp:842) | **partial (inert)** — pre-run staging is accepted `refuse_source_mutation` native_execution_consumer.cpp:1438-1454 but never consumed: `prepare_native_security_feeds` is protected engine.hpp:1639, its sole call `prepare_native_security_feeds` pine_strategy_host.cpp:1139; in-run it latches `UnsupportedSource` native_execution_consumer.cpp:1456-1461 **Closed:** a bare host's subscriptions consume authoritative bars: `NativeTimeframeSubscription::authoritative_bars` native_run_spec.hpp:488 is staged into the feed store by the kernel's own `prepare_native_security_feeds` native_execution_consumer.cpp:8355 call at begin, so a declared series is the venue's own bars (`tests/test_native_htf_subscriptions.cpp`). | K | L6 | F:F2 O:G-2 S:H1 | F says "no" on the same facts. "native" in these names means TradingView's native-timeframe bars, not the native host (the feed store's comment `never from the chart's intraday prints` `sha256:3cc5174fa9a60fc0e2ca7870ef1cccd875abaf5d8e11248e633922612c18d9d8` engine_aux_security.cpp:46-56). |
| HT3 | Lower-timeframe arrays (`request.security_lower_tf`; `register_security_lower_tf_eval` include/pineforge/source/pine_strategy_host.hpp:640) | **no** — `IntrabarPath::lower_tf` drives matching only **Closed:** `on_native_sub_bar` native_host.hpp:928 delivers every retained `lower_tf` sub-bar after its matching path, so a host that put the lower bars in the path reads them bar by bar (`tests/test_native_calc_timing.cpp`); the series form is the auxiliary feed of HT4 (`tests/test_native_auxiliary_feed.cpp`). | K | L5 (sub-bar hook); series form optional in L6 | F:F4 O:G-3 S:H2 | **Disagree on lane:** F via `on_native_sub_bar` (the host already owns the lower bars it put in the path) / O, S via the series API. v1 = the L5 hook (R5-5), landed; the array form Pine returns stays the adapter's. |
| HT4 | Auxiliary finer feed driving `request.security` (`set_aux_security_feed` pine_aux_security.cpp:21-230, `feed_aux_security_for_chart_bar` pine_aux_security.cpp:251-423; the kernel base returns false `set_aux_security_feed` engine_consumer.cpp:158-161) | **no** **Closed:** `NativeRunSpec::auxiliary_feed` (`NativeAuxiliaryFeed` native_run_spec.hpp:515) plus `NativeSeriesSource::AuxiliaryFeed` build a series from the host's finer bars, routed by time alone, with `declare_auxiliary_feed` at begin and `append_auxiliary_bars` on a realtime stream (`tests/test_native_auxiliary_feed.cpp`, `tests/test_native_auxiliary_feed_stream.cpp`). | K+A | L6 | F:F3 S:H2 | **Disagree:** F quirk (TV split-feed harness construct) / S generic. R5-1 + R5-6: aux feed registration is generic; chart-slice mapping and deferred first-bucket publication stay A. |
| HT5 | Publication mode: lookahead, gaps, session / early-close boundaries (`prepare_historical_security_lookahead_projections` pine_scheduler.cpp:42-132, `calling_bar_complete` pine_aux_security.cpp:265-346) | **no** for a bare host **Closed for a bare host:** `NativeTimeframeSubscription::lookahead` and `::gaps` are the two delivery rules, and the calendar boundaries — session close, early close by instrument class, the lazy seal of a clipped script interval — are the kernel's own (`tests/test_native_htf_subscriptions.cpp`). | K+A | L6 | S:H3 F:F1 O:G-1 | **Disagree:** F "no `lookahead_on`, by construction" / O `lookahead = false` default, available / S `lookahead_on` field, status partial. R5-6: user question (§4 U1); the exact TV projection stays A. |
| HT6 | Self-aggregation recipe (interim) | **yes** — `TimeframeAggregator` is public and engine-free timeframe.hpp:288-486, fed from `on_native_input` native_host.hpp:875; the Pine scheduler asks the kernel's `script_bucket_completions` native_execution_consumer.cpp:7932-7949, which feeds the same class `TimeframeAggregator` native_execution_consumer.cpp:7939, how its input span buckets (R5 lane B-ADAPTER) | K | L0 (document) | O:G-4 F:F1 | R5-6 |

### 1.7 Fill price, slippage, commission, FX

| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| FP1 | Trigger geometry on the modeled path; fill at the level or at the gap price; market-if-touched | **yes** — `native_matching` native_execution_consumer.cpp:5698-5745, `first_region_entry` native_matching.hpp:229-257; kind `NativeCandidatePriceKind` native_execution_consumer.cpp:6293-6301; `Limit::fill_through` native_order.hpp:239-244 enforced where the matched price is limit-protected (`protect_limit` native_execution_consumer.cpp:6281-6291) and re-checked after host terms (`InvalidTerms` native_execution_consumer.cpp:5103-5121) | K | — | F:G3 O:H1 O:H2 O:H3 | — |
| FP2 | Slippage in ticks, limit-clamped | **yes** — `slippage_ticks` native_run_spec.hpp:584 applied once native_execution_consumer.cpp:4620-4621, clamped by `protect_limit` native_execution_consumer.cpp:6289 | K+A | — | F:G1 O:H7 S:F2 | TV's round-then-slip order (`source_bar_fill` pine_adapter.cpp:11984-11993) is A |
| FP3 | Commission: percent / cash per unit / cash per execution (`fee_kind_for` pine_adapter.cpp:388-395) | **yes** — `NativeFeeKind` native_run_spec.hpp:19-23; `quote_execution_commissions` engine_execution.cpp:1575-1611 | K | — | F:G2 O:H8 S:F1 | — |
| FP4 | Booked fill quantized to the instrument tick, nearest or directional (`nearest_tick` pine_adapter.cpp:181-184, `source_bar_fill` pine_adapter.cpp:12092-12101, `directional_tick` pine_adapter.cpp:226-230) | **no** — `price_tick` is only a slippage multiplier native_execution_consumer.cpp:4082; `bar_fill_price` engine.hpp:771 is reachable only through `NativeCurrentPriceRule::NearestTick` native_execution_consumer.cpp:6375-6376; the reference helpers were dead (since deleted) **Closed:** `NativeRunSpec::price_grid` (`NativePriceGrid` native_run_spec.hpp:118) with `grid_rounding` (`NativeGridRounding` native_run_spec.hpp:128) rounds the fill basis onto `price_tick` before slippage, `HalfUp` or `Directional` (`tests/test_native_price_grid.cpp`). | K | L8 | O:H4 O:H6 F:G4 S:F3 | **Disagree:** O kernel / F, S "stays adapter, no promotion". R5-3: own kernel lane. |
| FP5 | Trigger tested against the tick-quantized bar | **no** — the kernel tests raw prices against raw levels **Closed:** `NativePriceGrid::QuantizeFillsAndTriggers` additionally tests and re-validates every activation against the tick-quantized path, one rule for every trigger kind (`tests/test_native_price_grid.cpp`). | K+A | L8 | O:H5 F:G4 S:F3 | R5-3: the optional quantized trigger test is K; TV's exact half-tick rule (`source_trigger_threshold` pine_adapter.cpp:257-305) is A on top. |
...
| FP6 | Account-currency FX | **partial** — scalar `account_fx` + immutable `NativeFxCurve` native_host.hpp:1202 in batch; the stream then refused a non-empty curve (today only tick-driven input is refused under one, `refuse_fx_curve_tick_input` native_execution_consumer.cpp:8761). **Closed by audit lane N6:** the declared curve is the run's immutable FX epoch and a confirmed-bar stream runs under it exactly as a batch does (stream twin in `tests/test_native_margin_fx_roll.cpp`); no realtime rate ingestion and no tick-driven input under a curve are stated as permanent limits with their reasons (native-engine.md, "Stream FX"). **Re-examined by R5 follow-up lane E3 and KEPT:** the tick refusal was never only about the margin check — it guards the SCRIPT-VISIBLE conversions (`open_trade_profit()`, `marked_equity()`, the equity curve), which read `active_account_currency_fx()`, i.e. the presented clock the observation hook has not yet moved (re-measured: a step at T+90s left the T+100s and T+110s prints converting at the pre-step rate). E3's cursor rule reaches the margin model only, so the refusal stands and is pinned in `tests/test_native_margin_fx_clock.cpp` | K | unassigned → N6 (E3: refusal kept) | F:D9 S:F4 | **Disagree:** F out of scope ("O7 clock") / S its P10, a stream-safe FX epoch. No R5 lane → §4 U4 (answered by N6: S's "same curve in batch and stream", without a new clock or callback). |
| FP7 | *(quirk)* TV tick conventions: half-tick threshold `source_bar_fill_tick` pine_adapter.cpp:264-312 (used `source_trigger_threshold` pine_adapter.cpp:9059-9064, `exit_limit_trigger` pine_adapter.cpp:9593-9614), `source_bar_fill_tick` pine_adapter.cpp:202-215, raw-vs-booked spelling `source_same_point` pine_adapter.cpp:221-225, margin-call fill pricing `source_margin_fill_price` pine_adapter.cpp:13838-13844 | n/a — already isolated behind the terms seam (adapter `resolve_terms` pine_adapter.cpp:11709) | A | — | F:G4 O:H5 S:F3 | R5-1, R5-3 |

### 1.8 Excursion, metrics, report, hash

| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| RP1 | Per-lot MFE / MAE, kernel-sampled or host-owned | **yes** — sampler `apply_excursion` native_execution_consumer.cpp:2622-2635, closing-row fold `max_runup` engine_orders.cpp:157-171; `owns_lot_excursions` native_host.hpp:1021-1025 | K+A | — | F:H1 O:I1 O:I2 S:X1 | **Disagree:** S partial + its P8 excursion-mode enum (single-source, not adopted, Appendix A.9). **Lowered by R5 lane H-THIN:** the Pine host keeps the kernel's `owns_lot_excursions()` default, so the sampler is the Pine excursion; the host model, its entry-bar masks and its caches are deleted, and the source layer's fold no longer carries them. **Measured against TradingView first (R5 lane H-MEASURE, `tests/test_e19_excursion_tape.cpp`):** the host-owned model was `ab9714be`'s, not TradingView's — where it and the kernel sampler differ the kernel's number is TradingView's (entry-bar over-mask, exit-bar over-fold after an at-open exit, a `calc_on_order_fills` scratch TradingView reports as 0 / 0); both miss a bar's open print ahead of a same-bar stop fill. On the corpus the two models differ on 341 of 431 390 trades, and of the 326 of those a TradingView export identifies, the kernel's number is TradingView's on 315 and the host's on none. The lowering moved the cells of 335 trades in 19 corpus probes: 324 now equal TradingView's, 11 moved closer, none farther; the all-in residual's own entry-bar call is tape `tests/fixtures/e19_allin_trim`. |
| RP2 | Closed-trade list, trade statistics, report struct | **yes** — `fill_report` engine_report.cpp:39-66, `get_trade` engine.hpp:1791-1828, `compute_trade_stats` engine_metrics.cpp:122 | K | — | F:H2 O:I3 S:X2 | — |
| RP3 | Equity curve | **no** — `update_equity_extremes` engine.hpp:1362 / `record_equity_point` engine.hpp:1380 are protected and were called only by the adapter **Closed:** `NativeReportPolicy::KernelRecorded` native_run_spec.hpp:63 has the consumer fold the extremes and append one point per script calculation, labelled with the script interval's open; `KernelRecordedAtHostMarks` records the same series at the host's own marks (`tests/test_native_report_truth.cpp`). | K | L2 | F:H3 O:I4 S:X2 | S's "partial" folds RP2 in |
| RP4 | Equity metrics (drawdown, run-up, Sharpe / Sortino, CAGR, time in market) | **no** — `compute_equity_stats` exits early on an empty curve engine_metrics.cpp:203; `fill_metrics_section` engine_report.cpp:119-139 **Closed:** with a recorded curve the drawdown / run-up walk is finite and every equity metric is computed over a real series; `report_open_position_at_end` adds the still-open position as a mark-to-market row (`tests/test_native_report_truth.cpp`). | K | L2 | F:H3 O:I5 | silent degenerate metrics, no error (O) |
| RP5 | Range-end row for a position still open at the end (`scheduler_record_range_end` pine_strategy_host.cpp:1306) | **yes, shared** — one generic producer `NativeExecutionConsumer::append_open_position_report_rows`, called by the kernel's run-end `record_open_position_report_rows` (`KernelRecorded` + `report_open_position_at_end`) and by the adapter at TradingView's mark; `report_trade_count()` engine.hpp:1815 includes the rows | K + A report shape | L2, Q6 | F:H4 O:I6 S:X2 | **Disagree:** S says kernel range-end rows exist (storage only). The ROWS are now the kernel's (§3.7 part 1); TV report shape — the equity re-mark, the extreme re-fold, the same-bar re-sort, three marks — stays A, measured in §3.7 part 2. The short-seed swaps (`project_short_seed_report_rows` pine_strategy_host.cpp:1031) stay A as well: a projection of two rows' `entry_incarnation` onto `ab9714be`'s numbering, which TradingView does not export; measured against the kernel's own lot identities by `tests/test_short_seed_report_swap.cpp` (R5 lane H-MEASURE), and pinned by `test_native_oracle_short_seed_percent_full_l2`, whose two incarnation checks fail when the swap is skipped. The swap runs only when a later close of the seed id closes a remnant; a flat collision keeps the kernel's numbering. |
| RP6 | Per-trade accessors | **partial** — `get_trade` is public; the `closed_trade_*` family is protected engine.hpp:1391 **Closed:** `closed_trade_count()` / `closed_trade(i)` and `report_trade_count()` / `get_report_trade(i)` are public, and `closed_trade_close_cause(i)` engine.hpp:1833 answers the typed `execution::CloseCause` the closer recorded (`tests/test_native_report_truth.cpp`). | K | L2 | F:H6 | (single-source) |
| RP7 | Live order-action stream (runner ledger / webhook) | **yes** — `stream_observe_actions_` engine_execution.cpp:1293, enabled `stream_observe_actions_` native_execution_consumer.cpp:9335, C ABI `strategy_stream_order_actions_len` c_abi.cpp:652-669 | K | — | F:H5 | (single-source) |
| RP8 | Continuation hash | **yes** — `native_continuation_hash` native_host.hpp:1376, native_execution_consumer.cpp:10737; the spec is folded by `hash_spec` native_execution_consumer.cpp:238-260 | K | — | F:I1 O:J2 S:X3 | — |
| RP9 | Broker-state hash: scalar + per-bar recording | **partial** — scalar `broker_state_hash` engine_state_hash.cpp:22-118, C ABI `strategy_broker_state_hash` c_abi.cpp:527; per-bar rows are appended only by the source scheduler `scheduler_record_broker_hash` pine_strategy_host.cpp:1552 **Closed:** under `KernelRecorded` with `set_broker_state_hash_recording(true)` the consumer appends one `broker_state_hash()` row after each report point, so `broker_state_hash_len == equity_curve_len == script_bars_processed` on a completed run in batch and across a stream's warmup and realtime legs. A Failed or Aborted run can expose a shorter prefix. A row is the run's continuation identity at that bar, per driving mode — the broker half alone is mode-invariant (`tests/test_native_report_truth.cpp`). | K | L2 | F:I2 O:J1 S:X3 | O's "yes" covers the scalar only. §3.9 records the old quadratic adapter fold and closed-row kernel walk as history; V19-E and V19-A replaced them with running digests. |
| RP10 | Fold the host's own durable state into the hash | **partial** — the protected virtual `hash_source_extension` engine.hpp:411 is overridable but source-named; the kernel default folds `"source:none"` engine_state_hash.cpp:19-21 **Closed:** `hash_host_extension(BrokerStateHashSink&)` engine.hpp:405 is the generic seam — protected virtual on `BacktestEngine`, called once per hash, last — and `hash_source_extension` engine.hpp:411 is its deprecated spelling, which the generic default forwards to (`tests/test_native_host_hash_extension.cpp`). | K | L2 (the virtual rides the shared bump) | F:I3 O:J1 S:X3 | — |

### 1.9 Other runtime surface

| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| OT1 | Key / value run parameters (`set_input` engine.hpp:1856, refused in-run by `guard_native_mutation` engine.hpp:1860) | **partial** — staging allowed pre-run, getters protected | K (low) | — | F:J1 | (single-source) Nothing for v1: native hosts take constructor parameters. |
| OT2 | Symbol info, sessions, calendars, timezones (`timezone` pine_adapter.cpp:2088-2089, `chart_timezone` pine_adapter.cpp:2306-2308) | **yes** — spec fields `ticker` native_run_spec.hpp:568-578; `parse_session` native_calendar.hpp:180-467; intervals on every decision context `input_interval` market_driver.hpp:123-124 | K | — | F:J2 O:G-5 | — |
...
- **Before L1:** one static library always compiled the source layer and installed every header, `include/pineforge/source/**` and `include/pineforge/compat/pine/**` included, and `src/compat/pine/market_admission.cpp` sat in the *main* list for historical reasons (added by `3b13183`, before the source-layer list existed — F). **Since L1:** the source layer is its own list (`PINEFORGE_SOURCE_LAYER_SOURCES` CMakeLists.txt:91, `market_admission.cpp` at :94), appended to `pineforge` only when `PINEFORGE_BUILD_SOURCE_LAYER` is ON (`add_library(pineforge STATIC` CMakeLists.txt:164-167), beside the kernel-only archive (`pineforge_kernel` CMakeLists.txt:154), whose header install excludes `source` and `compat` (`PATTERN` CMakeLists.txt:426-431).
...
| g | Legacy path resolver | **Done, and then deleted.** `src/engine_path_resolve.cpp` is 118 lines with five functions: `bar_path_uses_high_first`, both `first_touch_position` overloads, `fill_bar_path_points_ordered`, and `BacktestEngine::declare_opened_lot_entry_bar_mask`. Ten test files still mention `resolve_exit_path_fill` in historical comments; the resolver body and test-only oracle are gone. | NEUTRAL (pure move; verify with the link gate) | Keep the generic path helpers in the kernel TU; the old source resolver is not a live counterpart | F (single-source) | <!-- verified HEAD --> |
...
*History: the plan L13 worked from. The surface shipped as
`<pineforge/native_c_api.h>`, R5 lane F4 routed every policy hook, and what the
1.0 C surface still does not expose is the 1.0 C boundary table of
`docs/pages/native-engine.md`. "Today" below is the tree L13 read.*

Today a C host can configure (`strategy_configure_native_v1` c_abi.cpp:850-915), stream (`strategy_stream_*` c_abi.cpp:578-687) and read results, but strategy logic is C++-only and the batch entry points (`strategy_create`, `run_backtest*`) are per-strategy symbols a native author hand-writes. Design retained from F (most complete), with O's event polling and S's hardening rules:

```c
typedef struct pf_native_decision_v1 { uint64_t ordinal; int32_t interval_index; /* SCRIPT-bar index */ int64_t effective_time_ms,
...
| Point | Decision | Src |
|---|---|---|
| Implementation | One new kernel TU (F `src/native_c_host.cpp`, O `c_native_host.cpp`, S `native_c_api.cpp`): `class CCallbackHost final : public NativeStrategyHost` forwarding each virtual to the table. Streaming reuses the existing `strategy_stream_*` symbols unchanged (they take any `BacktestEngine*`) | F O S |
| Errors | A non-zero callback return maps to `NativeFailureCode::CallbackException`; C callbacks must not unwind. The C++ path already latches exceptions as `CallbackException` native_execution_consumer.cpp:7254 | F O |
| Legality | The existing rule (`commands_allowed` native_execution_consumer.cpp:1407-1414): inside a callback, or between realtime inputs | F |
...
| SOP | Every symbol goes into `src/c_abi.cpp`, `pineforge.h`, `EXPECTED_RUNTIME` + both counts (check_c_abi_runtime.py:19-79), the README table and the Python harnesses, or the `source-guard-c-abi` stage fails, in preflight and every `ci_verify` profile (`CLAUDE.md`) | F O S |
| Not in v1 of the C API | `resolve_execution_terms` / `validate_execution_precommit` are the adapter's seam; a C host that needs sizing uses `Sized` (F). O offers an optional `resolve_terms` callback: deferred. *History: R5 lane F4 routed both (`on_execution_terms`, `on_precommit`); the terms hook's units answer outside a host-sized close is still C++-only (the 1.0 C boundary table)* | F O |
| Open | Naming: F and O use `pf_native_*`; S uses `strategy_native_*`, which matches the existing runtime symbols. The v1 C spec `pf_native_run_spec_v1` pineforge.h:510 omitted `intrabar`, `path_order` and `abort_reporting`; the extension's policy tail carries all three now (`pf_native_abort_reporting_e` native_c_api.h:911) | F S |
...
| Engine reuse | none: generalize the consumer's one script bucket (`ScriptBucket` native_execution_consumer.hpp:661-676, `contribute_input` native_execution_consumer.cpp:8068, `seal_script` native_execution_consumer.cpp:8033) to N; rejects exposing the `request.security` path as Pine-shaped end to end | promote the existing evaluator + feed machinery; edits `engine_security.cpp`, `engine_aux_security.cpp`, `engine_lower_tf.cpp` | same TUs + `native_calendar.cpp` | The existing feed machinery, **called, not edited**; `engine_security.cpp` untouched |
| Pump | consumer seal logic | accepted-input path (`invoke_input_callback` native_execution_consumer.cpp:7128), only for natively registered series | copy at Ready, evaluators created before the first input | O's registration-gated pump + S's copy-at-Ready |
...
|---|---|---|---|---|
| 1 | Orders are "Market / Limit / Stop" (ADR L72-75) | Also StopLimit and Trail; five intents, five owner kinds, groups, `PointBudget` | `Trigger` native_order.hpp:296, `OrderIntent` native_order.hpp:229, `Owner` native_order.hpp:464, `Group` native_order.hpp:473, `PointBudget` native_order.hpp:379 | F |
| 2 | "The kernel matches and fills … with sizing, slippage, fees, margin and settlement" (ADR L74-75) | "with slippage, fees, opening admission and settlement; sizing is resolved by the host". `NativeRunSpec` owns fees, FX, close timing, direction / caps and intrabar paths; it does not own percent / cash sizing, maintenance margin, calc-on-fill, every-tick calculation or bare-host security registration | at `73817c1` (overtaken; ADR-0001, "Current state vs. target") | O S |
| 3 | "Read results via `metrics.hpp`" (ADR L76-80) | Trade stats yes. Equity curve and equity metrics are empty for a bare host, silently, and an open final position is missing from the report | at `73817c1` (overtaken; ADR-0001, "Current state vs. target") | F O |
| 4 | HTF "comes from the kernel feed `set_native_security_feed` / `prepare_native_security_feeds`" (ADR L84-89) | Not usable by a bare host: a partial boundary, not an absent kernel. `prepare_native_security_feeds` is protected with one caller; `configure_security_evaluators` has one caller; the public setter is inert pre-run and refused in-run | `prepare_native_security_feeds` engine.hpp:1639, `configure_security_evaluators` engine.hpp:1322, `set_native_security_feed` engine.hpp:1730, `guard_native_mutation` engine_aux_security.cpp:78 | F O S |
| 5 | "`engine_aux_security.cpp` includes `source/pine_strategy_host.hpp`" (ADR L129-132) | False at `73817c1`: its includes are `engine_internal.hpp`, `ta.hpp` and std headers (*fresh* re-read). No kernel TU includes a source header | `sha256:4c80b38f7193fcf8d72f5b10d04d2c468a21b6f771c682d8adc8dddcd99a73f2` engine_aux_security.cpp:6-13 | F |
| 6 | "Kernel TUs reference adapter types (`source::StrategyOverrides` …)" (ADR L129-131) | True only as a forward-declared opaque pointer the kernel never dereferences; the independence checker whitelists exactly that symbol | at `73817c1`; overtaken by `3f4fff5b` (§2.ii a), after which the rich begin carries `overrides_opaque` native_host.hpp:758 and the independence checker whitelists nothing | F O |
| 7 | `engine.hpp` hosts "the ten-significant-digit money rule, KI-62, the `strategy.close` batching + entry-id ledger, a TradingView margin-call toggle" (ADR L131-134) | Two of four survive as code. Money rule: a comment block; the arithmetic is in the adapter. KI-62: orphaned comments + one live, hashed lot flag. Close batching + id ledger: gone from the kernel (the ledger is in the adapter; the old names survive only in comments). Live: `margin_call_enabled_`, the KI-62 flag and — unmentioned by the ADR — string-sentinel decoding of adapter comments | the money-rule block was lines 927-961 of `engine.hpp` and R5 lane E6 deleted it (lines 73-180 were and are live code), the arithmetic `source_money_round` pine_adapter.cpp:370-376 and its twin `tv_money_round` pine_policy_support.hpp:9-15; `skip_entry_bar_high` engine.hpp:152, `skip_entry_bar_low` engine_state_hash.cpp:66, `declare_opened_lot_entry_bar_mask` engine_path_resolve.cpp:95; the ledger `close_logical_units_` pine_adapter.hpp:2028-2039; `closed_trade_close_cause` engine_trade_accessors.cpp:158. **Overtaken:** the margin-call toggle and the string decoding are both gone — see ADR-0001, "Current state vs. target" | F O |
| 8 | "No kernel-level C API. `c_abi.cpp` exports only version/descriptor symbols" (ADR L136-138) | 58 runtime `PF_API` symbols: native configure, the 12-symbol stream family, pending-order views, broker-state hash, security feeds, FX curve, reports. The 58 compiled-strategy runtime symbols in `c_abi.cpp` are distinct from the native C host API in `native_c_host.cpp`: the latter exposes submit, replace, cancel and a callback table to a C host. | `sha256:13933f70987f6bfe37d3015ae065a8d5ee5937c37431d48e2ecd1f17a4c29958` c_abi.cpp:278-918, `sha256:4b7e398c78b276b34c4247e8271242b48aa195cea29126f4e809073f2665e1ea` check_c_abi_runtime.py:20-80, c_abi.cpp:851, `sha256:a9d7348a5a060e57c33fe9f537b9e7c21a18417c5d15569074bca299ea72f504` c_abi.cpp:579-698, `sha256:db54076d46a71efb0c7556c97494317e7fca443278b6963c3edf79ce918d72a1` c_abi.cpp:504-556, `sha256:f18d46ac323234f7e3082367d30840b704b8a7548f7ae95ee8cff8714f29fd50` c_abi.cpp:483-500, `sha256:dab6c5091fb32549abf866e5d71ed05f722ab4715defa4f0eca524c8f9238a2f` c_abi.cpp:791-822, c_abi.cpp:918 | F O S |
| 9 | "No examples. `PINEFORGE_BUILD_EXAMPLES` is 'none yet'" (ADR L139) | The option is inert, but two Pine-free `NativeStrategyHost` examples exist, are compiled by the independence checker and tested through the live runner, only under `PINEFORGE_BUILD_LIVE_RUNNER`. Wording: "no top-level examples target" | at `73817c1` (overtaken; ADR-0001, "Current state vs. target") | F O S |
...
|---|---|---|
| the page's opening, and its "close-only" claims | callbacks are close-only | The kernel has a bar-open hook (`on_native_bar_open` native_host.hpp:889), a tick hook (`on_native_tick` native_host.hpp:878), a post-fill hook (`on_native_applied` native_host.hpp:938), a recalculation hook (`on_native_recalculate` native_host.hpp:913) and a sub-bar hook (`on_native_sub_bar` native_host.hpp:928). The page's opening now tables all eleven, and `on_native_applied` is documented as the calculate-on-fill hook with suffix eligibility |
| the request-epoch sentence | `native_order_v4` | `native_order_v7` native_order.hpp:25 | <!-- verified HEAD -->
| the two host-epoch sentences | host epoch v16 | `engine_script_run_v19` native_host.hpp:20 |
| "What still requires Pine compatibility to build" | `BacktestEngine` still holds `CapAttachment` / `OrderPriority` / `IntradayCap` | `engine.hpp` has zero references to them; the cap type is `CapAttachment` intraday_cap.hpp:18 and the constructor is `PineStrategyHost`'s pine_strategy_host.hpp:259 |
| the rich-`run` paragraph | the rich `run` overload fails a native host | `run_rich` native_execution_consumer.cpp:9258-9298 shows no such refusal (still unpinned by a test; the page says so) |
| added | — | the self-aggregation recipe beside the subscriptions it is an alternative to; the open-bar lookahead warning with `current_partial_bar` native_host.hpp:1073; and `set_native_security_feed` documented as the pre-run door for a series' authoritative bars, refused in-run |
...
| **L3** | Sizing bases (percent-of-equity, cash, fee-net), placement-vs-fill timing, fractional reduce, sibling reservation | M/L | yes | — (∥ L2) | opens `native_order_v6` | L3 / L3 / C + B (reservations) | <!-- verified HEAD -->
...
| **L5** | Calc-timing contract: partial-bar view, open-bar guard, sub-bar hook, calc-on-fill re-entry, every-tick | M/L | yes | runs after L7 in the R5-11 order; ∥ L6 | `engine_script_run_v18`, `native_run_spec` | L5 / L7 / E | <!-- verified HEAD -->
| **L6** | Native HTF / aux feed registration for bare hosts (§2.iv) | L | yes | L1; user decision U1; ∥ L5 | `engine_script_run_v18`, `native_run_spec` | L6 / L8 / F | <!-- verified HEAD -->
| **L7** | Order ergonomics + bracket / trailing completeness: builder, relative anchors, zero / tick offsets, retain-trail replace, cancel-all / by-label, working view | M | yes | L3 (shared `native_order` epoch) | `native_order_v6` | L7 / L9 / B | <!-- verified HEAD -->
...
| **L11** | Neutral coupling extraction, small PRs (§2.ii a-i) | M | post-L1, any time (a, d wanted for v1 — F) | L1 (link gate) | a, f ride `engine_script_run_v18`; the rest none | L11 / L11(a) / I (part) | <!-- verified HEAD -->
...
| L5 | `NativeCalculationTrigger`, `on_native_recalculate(…, reason, cause)`, `on_native_sub_bar`, `current_partial_bar()`, `NativeOpenBarView` → A.4 | the adapter keeps `BarClose` + its own cascade; `born_on_remaining_path` (native_execution_consumer.cpp:5641-5645) and `drain_applied_notifications` (native_execution_consumer.cpp:6302-6315) are frozen; accessors are inert | COOF probe re-expressed natively: same fills where the TV waypoint rule is inactive (F); a host that flips on its own fill produces the adapter's cascade in shape (O); callback reason, cursor, newborn eligibility and event ordinals compared (S) |
| L6 | `NativeTimeframeSubscription`, `on_native_timeframe_bar`, `native_series_bar` → A.5, §2.iv | `subscriptions` empty for the adapter; registration-gated pump; `engine_security.cpp` and pine_strategy_host.cpp:927-1011 untouched; corpus diff before / after (O: highest parity risk of any lane) | HTF-filter probe on a 24x7 symbol: exact (F); `"D"` over a 15m feed equals a `TimeframeAggregator` baseline bar for bar (O); installed-feed and no-feed cases (S) |
...
| L9 | `NativeRiskLimits`, `NativeRiskEvent`, `MatchRejectReason::RiskLimit` → A.8 | `risk` unset for the adapter; its ledger (`SourceDayLedger` pine_adapter.hpp:1200) untouched — **retained after measurement, audit lane N12; native-only by ruling, audit lane P6: §3.6.1** | risk probes: same halt bar (F); each limit + the day-boundary basis (O); breach, forced close, cancellation, next-day reset (S) |
...
| 4 | R5-11 shares one bump between L5 and L6 | L4, earlier, also adds host virtuals | `engine_script_run_v18` opens at L4 and closes after L5 ∥ L6, or L4's hook ships with that batch — supervisor call (§4 Q1) | <!-- verified HEAD -->
...
| Tier | Lanes | Why |
|---|---|---|
| Day-one blockers (O) | L1, L2, L3, L10 | Without L2 `fill_report` silently returns an empty curve and degenerate equity metrics and drops an open final position. Without L3 every non-unit order needs a hand-written `resolve_execution_terms`, and a mistake is a run-terminating `TermsUnresolved` (native_execution_consumer.cpp:4987-4989). Without L1 "kernel" is a claim, not an artifact. L10 is the minimum buildable proof. |
...
(pine_strategy_host.cpp:575-597) — after `prepare_native_begin` has already
projected the spec (pine_strategy_host.cpp:163) and `configure_native` has
...
begin-time declaration (`native_run_spec.hpp:609-616,504`, folded at
native_execution_consumer.cpp:236-237); a statement-time limit has nowhere to
...
`NativeRiskLimits` native_run_spec.hpp:310 and `risk_evaluate` native_execution_consumer.cpp:3692
...
stands (`project()` at pine_strategy_host.cpp:242, `configure_native` at :243,
the `set_pine_risk_*` setters (`set_pine_risk_direction` pine_strategy_host.cpp:585-608), and corpus 442d497
...
taped it, lane PAR-ORDERS fixed it).** A trailing `strategy.exit` issued while
its entry is still pending was materialized as the kernel's anchored child at
the fill (`relative_leg_shapes`, M17), which names no `best_seed`: its ride
started where the kernel arms it. Six `lab tv` tapes of that shape (NYSE:F 15m,
the exit written on the entry's signal bar; `tests/fixtures/pending_entry_trail`,
replayed by `tests/test_pending_entry_trail_tapes.cpp`) settle both halves. The
arm: on 28 trades whose arming bar's raw extreme stops inside the activation's
tick cell (16 market entries, 12 limit entries filling mid-bar), TradingView
exits on that bar at the activation ∓ 1 tick 28 of 28 (E5's quantized arm
holds for a pending entry). The running best: on 13 trades whose prints after
the arm fall between the two stops a best can give, TradingView's best starts
AT the activation 13 of 13, as E14 measured for a filled entry; the anchored
child's started at the raw arm print, and the adapter exited 1 to 24 bars later
on all 13, because the fill point's adoption compare left `best_seed` out and
adopted the unseeded child where `exit()` would have submitted a seeded leg.
Lane PAR-ORDERS makes a trailing offset of a tick or more unanchorable
(`anchorable_relative_exit`): the fill point submits the seeded leg, and the
row books TradingView's bar and price on all 41 trades.
...
(pine_adapter.cpp:343), `source_level_on_price_grid` (:329) and the
`nearest_tick` (pine_adapter.cpp:182) / `source_bar_fill_tick` (:203) /
`directional_tick` (:227) spellings are a *different* rule — a half-tick threshold per order kind with
...
(`PF_NATIVE_SPEC_EXT_PRICE_GRID`, translated at native_c_host.cpp:3162-3166)
...
| `max_open_lots` | adapter-policy: Pine pyramiding is a per-cycle entry count (measured against TradingView: not its rule, row MG3; R5 lane H-THIN's lowering trial moved four corpus probes away) | row MG3; the contract comment in `project()`; `tests/test_pyramiding_count_differential.cpp` |
...
mark_time_ms, interval_index, account_fx)`: one `open_at_end` row per open
physical lot through the same non-mutating builder a full close uses, at the
account-currency rate the caller names, returning the summed NET row P&L. It
is policy-free — the caller decides when to mark, at which rate, what to clear
first and what it re-derives from the rows — so the kernel's run-end producer
and a host with a different report shape share it instead of each restating
it. The adapter's own loop is gone; `scheduler_record_range_end` calls the
producer at TradingView's mark and hands it the rate of that mark's instant
(`account_currency_fx_at` of the curve's last point); since R5 lane B-ADAPTER
it no longer presents the mark as the engine's clock so the rows convert
there. Ruling: **a mark-to-market row per open lot is a generic capability;
when to mark and what to re-derive from the mark is not.**
...
(engine.hpp:545) is initialised NaN, reset to NaN at run start
(`fold_exit_trail_peak_` engine_run.cpp:109) and **never assigned a finite value anywhere in the
tree**, so all four `if (!std::isnan(fold_exit_trail_peak_))` carries
(engine_execution.cpp:1028/:493, the former adapter loop, the shared producer)
...
no baseline rows. Since R5 lane H-MEASURE a CTest row does:
`tests/test_aggregated_entry_bar_index_tape.cpp` replays TradingView's own
tapes of a `process_orders_on_close` script whose every exit reads
...
**Ruling (supervisor, ADR-0001 rule 2):** Option A. The kernel owns the
public coordinate in script-bar space for every host. `NativeCoordinate::interval_index`
and every lot, trade-row and metric index are sequential SCRIPT-bar indices;
`NativeCoordinate::input_interval_index` keeps the input slot beside them for
a host that needs input cadence. The Pine adapter's index re-stamp is removed;
its chart-bar sampling remains an explicit source-layer policy. The coordinate
and its named input companion are folded by the v19 continuation recipe, so
this lane re-pins the affected v19 witnesses once.

| ID | What TradingView and `ab9714be` do | What R4 slice C did | Owner and fix | Proof |
|---|---|---|---|---|
| AG1 | A trade row counts **chart** bars: its entry and exit bar are the `bar_index` the script read there, so `bar_index - strategy.opentrades.entry_bar_index(0)` is the bars held, and a tape row one chart bar long says "Duration (bars)" 1 | The pre-K-IDX kernel used the first contributing INPUT slot as its public interval index on an aggregated run. That made a two-script-bar trade report input indices and made `avg_bars_in_trade` count input slots. | **Kernel (Option A).** `NativeExecutionConsumer::ScriptBucket::script_index` assigns the sequential script index when a bucket seals. `NativeCoordinate::interval_index`, `PyramidEntry::entry_bar_index`, `Trade::exit_bar_index`, open-lot rows and the metrics calculation all consume that script index. `NativeCoordinate::input_interval_index` preserves the first input slot. `PineStrategyHost::on_native_applied` no longer rewrites lots or rows; Pine's cohort sampling still passes its explicit chart source index to the source-layer sampler (its excursion sampling is the kernel's since R5 lane H-THIN). | `kc_a310` / `kc_a310_c` in the K-IDX scratch: batch, stream and both magnified modes report `0/1/2`, entry/exit `1/2`, avg `2`; `tests/test_aggregated_path_regressions.cpp` and `tests/test_pooc_fill_stamp_aggregation.cpp` remain green. TradingView: `tests/test_aggregated_entry_bar_index_tape.cpp` (R5 lane H-MEASURE), a tape whose exits read the indices, booked field for field on all three paths. |
| AG2 | Every fill of a chart bar is dated at the bar's **open**: the e25-f-islastbar tape dates all 510 of its `process_orders_on_close` rows there, and `ab9714be`'s aggregated rows equal its chart rows | The kernel dates a close point (the calculation, the close after it, a modeled bar's close leg) at `calculation_time()`, the bucket's close, and a fill the host executes at the current point (a `calc_on_order_fills` first-open chain, a same-bar close-then-entry, the KI-62 cover) at its decision floor, which on the no-path route the bucket's inputs have already carried to the close. The chart timeframe's zero-width interval hides both, since every point of a bar sits at its label | **Adapter, at projection time.** On a chart whose script bars the kernel gathers from the input (`native_aggregates_input_bars()`, the kernel's `pairing_aggregates` of the resolved pairing, since R5 lane H-THIN; the adapter's own literal test parted from it on one-bucket pairings such as `1D` on `D`), `on_native_applied` dates the lot a fill opened (`PyramidEntry::time`, hence `Trade::entry_time`) and the rows it closed (`Trade::exit_time`) at the chart bar's open: on an aggregated chart every point of a confirmed bar's modeled path (its open, the segments, the calculation, the close after it), where the chart timeframe's own points all sit, and under the magnifier every sub-bar point too (each sub-bar's open, path and `ModeledOHLCClose`; lane E27 had cut it to the close points, R5 lane PAR-ORDERS widened it, TradingView's dating). A realtime print keeps its own instant, as the chart-timeframe stream keeps it, and so does a stream's quiet carried interval | the audit's pooc probe rows (`ab9714be`'s 19:45Z / 13:30Z on all three paths), row 4, the KI-62 row and the stream row (a chart-timeframe stream and the same stream fed 1m agree on every row, a 10:30:01 print fill and an 11:00 carried fill included); E27's stamp rows flipped. Measured: the chart path unchanged byte for byte, the magnified run moves only its POOC close-fill instants. TradingView itself dates a magnified fill at its chart bar's open too (218 of 218 fills on sixteen `lab tv` tapes, R5 lane H-MEASURE); the magnified run kept a sub-bar fill's own instant (71 of the 152 instants `tests/test_magnified_aggregated_tape.cpp` replays sat inside their bar) until R5 lane PAR-ORDERS, and the row now asserts every instant exactly |
| AG3 | A `calc_on_order_fills` recalculation runs the script **on the bar the fill is on** and reads that bar's `session.ismarket` / `isfirstbar` / `islastbar`; `ab9714be` set them before any fill of the bar | The scheduler set the flags in the close callback only, so a recalculation published the PREVIOUS bar's: a fill at the next session day's 09:30 read `islastbar`. On the chart timeframe the close then read its lookahead from a count the recalculation had already advanced, one retained bar too far: a fill on 15:30 flagged 15:30 `islastbar` and, through E26's dual, 15:45 `isfirstbar` | **Adapter** (Pine language state, OT7). `scheduler_update_session_state` stays the one writer and runs once per script bar, at the bar's first publication: `PineScheduler::recalculate` asks before the script runs, `PineScheduler::bar` pine_scheduler_native.cpp:493 only when no recalculation published the bar. Since R5 lane F5 it selects the kernel's session-day facts of that bar, which every callback of the bar carries alike, so the scheduler's lookahead is gone. E26's session-day rule and its `isfirstbar` dual are unchanged | the audit's coof_session probes and the 15:30 case in `tests/test_aggregated_path_regressions.cpp`; section 7 of `tests/test_session_islastbar_aggregation.cpp` (TradingView's NYSE:F days with `calc_on_order_fills`, chart, aggregated and magnified: every callback of a bar reads the tape's flags) |

**Recorded, not changed here.**

- Under the magnifier on an aggregated chart, the same-bar cohort checks
  still compare against the explicit chart source index selected by the Pine
  sampler (the entry-bar masks went with the host's excursion model, R5 lane
  H-THIN). They no longer mutate the kernel's script-bar
  lot index; the K-IDX probes cover the kernel index and input companion.
  **Closed (R5 lane H-MEASURE):** all three entry-bar tests of a magnified
  aggregated run (the slippage mask, the same-bar excursion test, the
  `calc_on_order_fills` extreme sample) now compare script-bar indices with
  script-bar indices — instrumented, 0 of 5 170 comparisons out of space,
  against 4 996 with 3 974 wrong verdicts on `91d65ad6` — and K-IDX moved
  two excursions onto the chart path's values and nothing else. Lane F1's
  "12 vs 9 rows" is settled by sixteen `lab tv` tapes of its differential
  script: TradingView books the script's 12 rows as this engine does on
  every path, and `ab9714be`'s 9 were its own error
  (`tests/test_magnified_aggregated_tape.cpp`, the variants the magnified
  run books exactly). **A `process_orders_on_close` stop entry the placing
  close already reached is closed too (R5 lane PAR-ORDERS):** TradingView
  fills it at that close -- the close's tick, slipped like any stop fill --
  on every path and with or without `calc_on_order_fills`, five entries on
  each `-v1` / `-v3` / `-v5` / `-v7` tape, where the resting stop filled a
  bar late at the next open. The close pass that fills it
  (`flush_pooc_marketable_limit_entry_fills`) now takes a pure stop entry
  that is the bar's lone entry marketable at the close; two such entries keep
  the next open's both-marketable arbitration, which no tape orders at a
  close. Since R5 lane PAR-ORDERS-2 the pass (`fill_pooc_close_entries`)
  also takes such a stop entry placed while the book holds the other side:
  TradingView fills it at that close, sized as the reversal it is, and after
  the bar's own `strategy.close` of the held side when one fills there
  (`tests/test_pooc_reversing_stop_tapes.cpp`, chart and magnified; corpus
  `order-deferred-flip-pooc-cross-bar-01`: 57 of TradingView's 792 entries
  at its bar and price before, 734 a bar late and one trade missing; all 792
  trades exact after). It tests a pure limit entry on the close's tick too,
  as it tests a stop, with or without `calc_on_order_fills` (R5 lane
  PAR-ORDERS-3, finding 3): TradingView fills a sell limit at 11.76 over a raw
  11.755 close at that close, and leaves a buy limit at 11.65 over a raw
  11.655 close (tick 11.66) resting whatever the bar's range reached
  (`tests/test_pooc_limit_close_tapes.cpp`), where the pass compared the raw
  close and skipped every limit under `calc_on_order_fills`. A stop entry the
  pass fills there can be the same id's
  entry again, placed at the close of the bar whose bracket limit closed the
  previous one, with a fresh `strategy.exit` of the same exit id. When that
  limit was a retained leg the pre-script drain settled, the adapter reads its
  terminal receipt only at the next open, and the sibling cancel it runs
  (`cancel_bracket_siblings`: the working legs of that exit id, `from_entry`
  and origin) took the fresh bracket too, so the re-entry never exited; since
  R5 lane INT26 it skips a leg of a later `strategy.exit` call that the kernel
  accepted after that fill. A leg of the filled leg's own call still goes,
  however late it was accepted: the drain submits a bracket's legs one at a
  time, executing each
  (`tests/test_pooc_stop_reentry_bracket_tapes.cpp`, with a control; the gate
  sweep's BTCUSDT probe, 820 of TradingView's 821 trades before, 821 after). Under the bar
  magnifier the same script's short still differs, recorded in the row: the
  drain leaves that retained limit to the flush below the body, whose book
  still holds the short, so the re-entry is never placed.
  That pass also never ran on an aggregated or magnified run: its
  quiet-bar gate tested the script-bar `interval_index` where the pass tests
  the input slot (`projection_bar_index`), so since K-IDX every such run
  skipped it, limit entries and exit legs included.
  **The pyramiding-cap bypass is closed (R5 lane PAR-ORDERS).** Under
  `process_orders_on_close` a pending opposite entry deferred the cap to the
  close pass even when that pass would not fill it first -- a priced entry
  resting from an earlier bar -- and nothing checked a fixed-size add at its
  fill, so under the magnifier with `calc_on_order_fills` every fill's
  recalculation added a lot: 86 at once with `pyramiding=1`. Such an add is
  now held to the lots open (`entry()`); the row asserts the cap on all
  eight variants and three paths, the v3 magnified run booking TradingView's
  12 rows and ending flat.
  **A market order a recalculation places on the bar's second extreme is
  closed too (R5 lane PAR-ORDERS-2, H-MEASURE Finding 6d).** When the
  matcher's fill of a resting request at its own level ends the leg to the
  bar's second extreme, the `calc_on_order_fills` recalculation's market
  entry and the exits it places are live from that extreme, as TradingView
  books them (row 5 of `hm-chart-diff-v2` / `-v3` / `-v7`, the
  `pa2-i2-exit-w2-*` tapes of `tests/test_second_extreme_order_tapes.cpp`):
  the entry used to advance to the close point, which admits no cascade
  order, and so to the next open. A fill the adapter itself forced onto that
  extreme -- an order an earlier recalculation of the bar placed, gap-filled
  there (`coof_fill_forced_`) -- starts the next leg instead, and its
  recalculation's market orders still advance, as TradingView books them too
  (the `pa2-i2-point-w2-*` tapes; corpus `bracket-rivet-calc-on-fill-01`,
  trades 350 and 516, byte-identical). A `strategy.close` the same
  recalculation calls fills at that extreme too, as TradingView books it
  (`pa2-i4-close-w2-*`), where it used to wait for the next open, or under
  `process_orders_on_close` for the bar's close.
  **An exit a recalculation places on a fill AT an extreme reads that fill
  (R5 lane PAR-ORDERS-3, findings 1 and 2).** The matcher's fill at a resting
  request's own level on the extreme ends its leg, so none of that leg is
  left in flight: the exit's limit is live at its own level on the next leg,
  as TradingView books it (`pa2-i2-level-w1-*`), where `exit()` forced it
  onto the next extreme. A fill forced onto the extreme starts the next leg,
  read from the extreme itself even when its booked tick is off-grid
  (`pa2-i2-point-w2-long*`: 11.425 booked 11.43), where that tick read as a
  fill short of the low.
  **The same holds on a bar's first extreme (R5 lane PAR-ORDERS-3, finding
  4).** After the matcher's fill at a resting request's own level ON a bar's
  first extreme, the recalculation's orders act from that extreme: a market
  entry, `strategy.close`, `close_all` and a market `strategy.order` fill AT
  it, an exit stop already through fills AT it, and a stop entry between the
  extremes fills at its own level (`pa3-f4-level-w1-*`, `pa3-f4-stop-w1-mkt-*`,
  `pa3-f4-xlimit-w1-reentry{,-tp}-*`), where `ab9714be` advanced every one of
  them to the second extreme or the next open (`coof_next_waypoint`); the
  frozen rows that pinned it moved with their notes (`test_calc_on_order_fills`
  and its twin, oracle row R1). A forced fill there still starts the next leg
  (`pa3-f4-point-w1-mkt-*`). The same reading lets a stop born on the second
  extreme and reached on the leg to the close fill at its own level
  (`pa2-i2-exit-w2-*` rows 5 and 7 / 1), where it was forced onto the close's
  tick.
- The per-bar broker hash of an aggregated run moves because the kernel now
  folds its script coordinate and named input coordinate. Those v19 witnesses
  are re-pinned once in `tests/test_native_point_checks.cpp`; no pre-release
  epoch outside v19 is changed.

---

### 3.9 Audit lane F9 — recording and continuation costs (E24's STOP bucket)
...
| **B — historical** | the pre-v19 kernel recorder's closed-row walk: O(closed rows) per row | ×2.61 rising to ×3.64 per doubling with trades (0.80 s at 24,000 bars and 1,200 closed rows) against ×1.94 to ×2.08 without | **historical ruling, superseded** by the v19 value epoch (R5 lane V19-A): each closed row now folds once, into a running digest |
...
`PineExecutionAdapter::hash_state` pine_state_hash.cpp:242 folded, at every
...
pine_state_hash.cpp:292) — one per accepted request, cleared only by
`reset_for_run` pine_adapter.cpp:2110 — and reflected the whole admission
journal (since replaced by a running digest of its events, `admission_journal`
pine_state_hash.cpp:580). `PineScheduler::hash_state` pine_state_hash.cpp:618
re-folded the consumed source prefix too (`consumed`
pine_state_hash.cpp:625-647). Measured with recording on and off, in two
shapes:
...
1. **No epoch is needed.** The sink is FNV-1a 64 (`BrokerStateHashSink`
   engine.hpp:350), and FNV-1a over a fixed byte segment S is an affine map
...
2. **The transforms need a home.** A row's transform, or a composed
   ...
   pine_adapter.hpp:366, `admission::Journal` market_admission.hpp:188 or
...
3. **A cache must know which rows changed, and nothing records it.**
   ...
   pine_adapter.hpp:502, the non-const `at` pine_adapter.hpp:518 and its
   ...
   (`has_full_entry_bracket` pine_adapter.cpp:8842). Full-table scans
   rewrite rows as well (`suspend_brackets_for_reversal` pine_adapter.cpp:1697,
   `revive_brackets_after_margin` pine_adapter.cpp:1919,
   `preserved_by_close_all` pine_adapter.cpp:11610), and
...
hash: an epoch decision (`pineforge-source-adapter/v3` to v4), not a neutral <!-- verified HEAD -->
change.

**Resolved by R5 lane V19-E, through the epoch.** V19-E took neither the
barrier nor per-row transforms: it made the retained history go away. At each
bar open, after `observe_terminal_receipts` has consumed every command event
up to the high water, `PineExecutionAdapter::erase_retired_rows` erases the
placement row of every request the adapter and the kernel no longer work,
unless a root still names it (an opened origin, an open lot, a queued
command's snapshot or request, an anchored leg, a short-seed plan, an event
above the receipt cursor) or a whole-table scan could still accept it (the
pins K1 to K8 in that function's comment). What the later scans read of an
erased row stays as a compact record: the per-id sequence minima and the first
non-margin ids in `PlacementTable`, the consumed legs and the settled members
of a bracket family (`BracketRoster`), the sides of a cohort's erased openings
(`CohortFacts::erased_opening_sides`) and the chain origin a row inherits
(`PlacementSnapshot::chain_origin_sequence`). A tombstone build
(`-DPINEFORGE_PLACEMENT_AUDIT=1`) keeps every erased row aside and records any
lookup or scan predicate that reaches one: zero hits over the full ctest
suite, the equivalence battery and the 100-slot fingerprint sweep with the
magnifier off and on. The v4 fold then reads the retained rows in full, in
incarnation order, with the table's high water, erased-row count and a running
digest of the erased identities; the admission journal's events fold into a
running digest (a `retain` or `reset` below the folded prefix refolds it); and
the scheduler's consumed prefix is immutable within a run, so it folds once
per bar. The rewrites item 3 lists stay exact because they only reach rows the
table still holds, and those fold in full at every read. Measured by
`tests/test_adapter_live_state_scaling.cpp` on macOS arm64: the re-issue replay
with recording on costs ×4.05 for four times the bars (1,000 to 4,000 bars:
0.032 s to 0.131 s) and retains 5 rows, where the base (`be372243`) cost ×15.64
(2.42 s to 37.86 s) and retained 1,999 and 7,999. Values moved once; trades did
not (`tests/test_adapter_live_state_equivalence.cpp` holds each script's
...
**Completed by R5 lane V19-FIX (audit X2, X9), inside the v19 epoch.** Two
reads still cost what the run had done (macOS arm64, Release, process CPU).
K1 held a leg lifecycle of the current position cycle for the margin revival,
and V19-D released one only once a re-issue at another stop superseded it, so
an exit the script cancelled and re-placed every bar at ONE level kept a row
per bar for the whole cycle, each walked by the sweep at every bar open: on
`91d65ad6` the audit's `probe_recancel_constant` cost ×4.90, ×4.79, ×4.65 per
doubling from 4,000 to 32,000 bars (11.2 s at 32,000), and
`strategy.cancel_all()` with an exit at `position_avg_price * 0.95` ×3.87 to
×4.26 up to 16,000. The same hold let the revival read a cancelled exit as a
standing one (OL14), and a gapped stop a declined reversal had parked for the
next margin-call slice came back after a cancel too. A script cancel now
retires every leg of each exit row it names and drops such a parked stop
(`retire_cancelled_exits` pine_adapter.cpp:1845), the revival skips a withdrawn
exit, and K1 holds a withdrawn leg only while its origin is live or opened -- the one reader left is `origin_leg_consumed`,
and it asks about such origins alone (`erase_retired_rows`
pine_adapter.cpp:2941). Both shapes now cost ×1.89 to ×2.12 per doubling up
to 64,000 bars and retain 3 rows. Row D was the kernel's half: the adapter
enrolled every opening in its id's kernel roster (`cohort_add`) and never took
one off, and `hash_cohorts` folds every member at every read. An origin whose
request chain no longer works and that holds no open lot can never be bound
again, so at every bar open that follows a command event the adapter takes
it off (`release_closed_cohort_origins` pine_adapter.cpp:3372); a lot opened
...
instructions at the median and +1.57 % at most (slots 007 and 030, the latter
with 23,164 trades). The
dead `PineExecutionAdapter::path_order_`,
written and folded but never read, left the fold in the same step.
`tests/test_adapter_live_state_scaling.cpp` holds all three shapes at a small
leg calibrated to at least 0.1 s CPU and four times its bars, below ×5 and
within 64 retained rows and 8 roster members; on the base they fail at ×25.6
(the constant level, 16,000 to 64,000 bars without recording), ×27.4 (the
cancel_all shape) and ×6.9 with 8,000 roster members (the flip), and three
mutants of the product fail it too (K1 holding a withdrawn leg again: ×25.9
and 32,000 rows; no roster removal: ×6.96 and 8,000 members; the removal kept
but a walk over every origin the run accepted at every bar: ×12.0).
...
script ABI (then `engine_script_run_v18`, since advanced), which is an epoch decision, while a <!-- verified HEAD -->
member of the heap-owned execution consumer (`ExecutionConsumerSlot`
engine.hpp:1666) is not. The other is a contract that the six folded fields of
...
`exit_comment` and `entry_incarnation` only; its
writes are `close_cause` pine_strategy_host.cpp:362, `exit_comment` :470, `close_cause` :484-489, `exit_from_bracket` :870 and `entry_incarnation` :1051-1059.
But `trades_` engine.hpp:546 is a protected member any host can write, so a
...
pine_strategy_host.cpp:103-111). `capture_script_continuation_hash`
pine_strategy_host.cpp:277-285 takes that snapshot at the last batch bar
(`last_batch` pine_strategy_host.cpp:336-346), and taking it is one full
...
Pine adapter keep no mechanism that duplicates a kernel capability, that every
mechanism it does keep have a row here or in the ADR and a witness, and that
the size target be met or re-set by a measured ruling. The lane took R5 lane
H-MEASURE's measurements as its inputs.

**X15, item by item.** "Byte-identical" means the corpus (312 probes), the
identity battery with its hash columns, the 100 public slots and the Pine
witness all print the same bytes before and after.
...
| f | `aggregates_input_bars`, read from the two timeframe literals | gone: the kernel's `NativeStrategyHost::native_aggregates_input_bars()` (ADR-0001, H-THIN rule 2 rulings; row AG2) | byte-identical on the corpus and every witness; `tests/test_aggregates_input_bars_literals.cpp` counts the two classes where the copy parted from the kernel |

Beside X15 the lane lowered the host-owned excursion model onto the kernel's
sampler (row RP1, E19), tried the pyramiding count on `max_open_lots` and kept
it (row MG3, M13), and fixed the one reservation divergence measured beside it
(P10). E19 and P10 are the lane's only moves: its one hash step re-pins every
witness they move, and the corpus baseline moves on the 19 probes whose
excursion cells moved toward TradingView.

**AUDIT4 §6, re-derived** (MR measured retention, RU ruled unmeasured, UF
unfinished, RM removed).

| rows | mechanism | AUDIT4 | now |
|---|---|---|---|
| 01, 02, 05, 07, 08, 11, 14, 16-20, 24-26, 30 | sizing floors and freezing, the FX step, the liquidation slice, margin money, the FX broker-open slice, `max_position_size`, the risk rules, the price grid, trail conventions, the second bracket path, the range-end shape, the auxiliary drive, `calc_on_order_fills`, percent-exit reservation, cancel loops | MR | MR (unchanged) |
| 03/04/06, 29 | sizing quotients and the fill-quantity probe, preview aggregation | RM | RM |
| 09, 10, 13 | margin-check scheduling, checkpoints as market executions, `AdmitWithHostMargin` | RU | MR (H-MEASURE M7, M8, M11) |
| 12 | the pre-open slice and its own ×4 | RU + copy | MR; the copy RM (X15 d) |
| 15 | pyramiding as a per-cycle count | RU | MR (H-MEASURE's tapes; H-THIN's lowering trial, row MG3); P10 fixed |
| 21 | short-seed report swaps | RU + GAP | MR (H-MEASURE's tape) |
| 22 | host-owned excursions | MR (partial) | RM (E19, row RP1) |
| 23 | `request.security` exclusions | RU + THIN | MR (H-MEASURE, the ADR `subscriptions` row) |
| 27 | aggregated re-stamp and re-dating | MR | MR; its predicate is the kernel's (X15 f, row AG2) |
| 28 | the dead hashed `path_order_` | RM / UF | RM: `fac8389a` deleted it and its fold |
| 31 | the second liquidation-price solve | UF | RM (X15 a) |
| 32 | the `1e-10` dust sweep | RU + GAP | MR (H-MEASURE's tape; the census below) |
| 33 | the entry-fee ledger | UF | MR (X15 b) |
| 34 | begin-bar validation | RU | RU: the kernel's ingress validation, ruled by E12 |
| 35 | the presented clock and index writes | UF | ruled: Pine presentation, not a kernel cursor copy -- `bar_index` counts script bars with the temporary index of a not-yet-sealed callback, and `last_bar_index` / `last_bar_time` freeze at the live tail's horizon, neither of which the kernel's `interval_index` models |
| 36 | `session.ismarket` through the legacy predicate | UF | RM on the kernel side (H-MEASURE's six tapes: `in_session` is TradingView's); the generated predicate is codegen's to route |
| 37 | the per-origin unit ledger | UF | MR (X15 c) |
| 38 | civil-date copies | RU | RM (X15 e) |
| 39 | `aggregates_input_bars` | UF | RM (X15 f) |

**The quantity-epsilon census (A4-DUST).** Every quantity comparison with an
epsilon in `pine_strategy_host.cpp` and `pine_adapter.cpp`, 85 of them at 57
sites, was measured three ways in a scratch copy of the tree that evaluates
each in its epsilon form and its exact form: over three seeded witness
families (5,500 runs of decimal quantities, dust-sized closes, pyramiding,
reversals, POOC, calc-on-fills, margin at 5-100 %, lot grids 1 to 0.001, fees,
multi-command bars, paired entries, sticky and trailing exits), over the 610
CTest rows that link the library, and with each deciding comparison made exact
on its own. None is removed or moved: the kernel's opt-in `quantity_tolerance`
charges a snapped reduce its request and books every dust lot it closes as a
row, where these rules do neither (ADR-0001, its row). Three local copies of
the two shared constants were folded into them, byte-identical. The census
reads, per site (witness and CTest evaluations; "band" = evaluations where the
two forms disagree; runs moved of 5,500 and CTest rows failing with the exact
form):

| sites | what the epsilon decides | evaluations | band | exact form |
|---|---|---|---|---|
| A1 `on_native_applied` sweep | a lot of at most `1e-10` is erased, without a row | 1,615,742 | 1,167 | 30 runs; fails `test_pine_dust_sweep_paired` (tape `hm-g232-decimal-dust`) |
| A2 flat reset; A3-A5 dust lots in the fee sum, the POOC FIFO view, the KI-62 cover | a dust book or lot is treated as none | 2,015,064 | 17 | no run, no row |
| B1, B2 POOC close flush | a flat book, and a close within `1e-10` of it as a `Flatten` | 3,110 | 4 | 4 runs |
| B3 `close` whole-id | a close within `1e-10` of the id's basis, or of 100 %, is whole | 190,291 | 4,648 | 365 runs |
| B4 `resolve_terms` scope cover | fill-time units within `1e-10` of the live scope are the scope | 367,513 | 284 | 214 runs |
| B5-B13 exit and reservation classes | full versus partial exits, re-issues, reservation expansion, the ANY-rule percent | 169,586,206 | 1,868 | no run, no row |
| C1 `source_fifo_prefix_openings`, EQ `enqueue_pooc_fifo_close` | a POOC FIFO close's whole-lot prefix and its batching | 3,978 | 4 | no run, no row |
| D1 `floor_quantity_grid` | the lot floor's `+1e-6` of a step | 227,717 | 14,182 | 1,437 runs; fails 14 rows |
| D2 `quantize_close_units` | a full close is the basis snapped to the grid | 383,394 | 11,172 | 349 runs |
| D3 `quantize_percent_exit_units` | the percent-exit floor's `+1e-6` of a step | 19,749 | 1,084 | 511 runs; fails 5 rows |
| D4-D9, D11-D15 reservation arithmetic | dust capacity, full families, re-issued brackets | 81,360,970 | 270 | D7: 54 runs; the rest none |
| D10 `close` all-in percent | an all-in close's reversal handling | 36,778 | 1,571 | 3 runs |
| E1, E2 cohort drain | a cohort origin at most `1e-10` is drained | 608,401 | 895 | 16 runs |
| M1-M4 `source_margin_units` | a dust restore, the second floor's `+1e-6`, the one-contract band, a dust slice | 56,680 | 395 | M2: 74 runs, fails 3 rows (`test_tv_money_band_l4b` among them); the rest none |
| M5-M12 margin-call carries and revivals | the one-unit call, deferred short checks, resting whole closes | 69,554 | 1,256 | no run, no row; M9 and M10's second test unreached |
| Q1-Q6, O1-O2 paired-market and short-seed identities | own versus gross frozen units, exact quantity identities | 1,099,295 | 0 | none |

Of the 85, 32 decide somewhere and 50 never do; three (B12's explicit-quantity
test, M9, M10's explicit-quantity test) are reached by no seeded run and no
CTest row. Every site keeps its reason at the line (an `ab9714be` citation or
a TradingView rule); the four whose exact form fails a pinned row are the
sweep and the three `+1e-6` floors.

**Size (AUDIT4's metric: the direct children of `src/source`,
`include/pineforge/source`, `src/compat/pine` and
`include/pineforge/compat/pine`, `.cpp` and `.hpp`).** 29,701 lines at the
audit's tree (`91d65ad6`), 29,920 at this lane's base (`0d76a099`), 29,059
after it: 861 fewer, below the owner's ruling (at most 29,693, aiming at
about 29,350 once the measured duplicates went). The earlier target of 27,008
stays out of reach for the reason H-MEASURE measured: the growth since round
3 is V19 state lifetime and the performance lanes, not duplicated mechanism.

**Findings, recorded and not changed here.**

1. TradingView sizes a percent-of-equity entry under a cash-per-order fee as
   `floor((pct × E − fee) / close)` on the tick-printed quantity grid, where
   `E` is the equity with every open lot's pro-rata entry fee charged: the
   tape `hthin-x15b-fifo-fee-basis` fits that rule on all six of its sized
   entries, and the corpus probe `order-percent-equity-cash-commission-01`
   on all 366 of its trades. The engine, before and after this lane, restores
   the open lots' cash fees and reserves no fee for the new order, so that
   probe's quantities differ from TradingView's in the fourth decimal. R5 lane
   PAR-CASHFEE measured the rule on twelve more tapes and fixed it (§3.10).
2. A `calc_on_order_fills` run of the seeded witnesses can stop with the
   kernel's "native current evaluated allowance mismatch": 117 of the second
   family's 2,000 runs, every one with calc-on-fills. The base tree stops 111
   of those seeds, 108 of them the same runs; the lane's E19 and P10 moves
   shift which runs reach the failing state, not the failure.
...
- cash per contract: `units = floor(pct × E / (price × pointvalue + fee))`,
  the money covering the units' notional and their fee together.

`price` is the price the quantity is sized at: the signal close for a market
`strategy.entry` and for `strategy.order`, the level for a limit or stop
entry. A reversing entry sizes its new position the same way from the equity
that still carries the reversed position (its open profit and its entry fee
share), and its one order pays one fee, split over the closed and the opened
units. Before this lane the adapter took the percentage of an equity that
restored the open entries' cash fees (`percent_commission_live_equity`) and
reserved nothing for the order, so every such quantity came out too large.

**Evidence: thirteen `lab tv` tapes** (`tests/fixtures/cash_fee_sizing`,
ws-report-v1, rangeProof covered; BINANCE:ETHUSDT.P 15 over 2025-04 and
CME_MINI:ES1! 15 over 2025-06). A tape's `E` is read off the tape itself --
the initial capital, the net profit closed before the signal, and each open
piece marked at the signal close less its pro-rata entry fee.

| tape | fee | default % | margin | scenarios | trades | sized entries the rule fits |
|---|---|---|---|---|---|---|
| `pcf-order-p50-m25` | per order 1000 | 50 | 25 | flat, pyramided, partial close, reversals | 15 | 14 / 14 |
| `pcf-order-p100-m25` | per order 1000 | 100 | 25 | the same | 15 | 14 / 14 |
| `pcf-order-p200-m25` | per order 1000 | 200 | 25 | flat, reversals | 6 | 6 / 6 |
| `pcf-order-priced-m25` | per order 1000 | 50 | 25 | limit, stop, `strategy.order` | 7 | 7 / 7 |
| `pcf-order-p100-m100` | per order 1000 | 100 | 100 | flat, reversal | 11 | 6 / 6 |
| `pcf-es-order-p50-m25` | per order 1 000 000 | 50 | 25 | ES1! (point value 50), pyramided, reversal | 7 | 7 / 7 |
| `hthin-x15b-fifo-fee-basis` | per order 1000 | 50 | 100 | FIFO exit of another id (lane H-THIN) | 11 | 6 / 6 |
| `pcf-contract-p50-m25` | per contract 20 | 50 | 25 | flat, pyramided, partial close, reversals | 15 | 14 / 14 |
| `pcf-contract-p100-m25` | per contract 20 | 100 | 25 | the same | 15 | 14 / 14 |
| `pcf-contract-p200-m25` | per contract 20 | 200 | 25 | flat, reversals | 6 | 6 / 6 |
| `pcf-contract-priced-m25` | per contract 20 | 50 | 25 | limit, stop, `strategy.order` | 7 | 7 / 7 |
| `pcf-contract-p100-m100` | per contract 20 | 100 | 100 | flat, reversal | 9 | 6 / 6 |
| `pcf-es-contract-p50-m25` | per contract 1000 | 50 | 25 | ES1! (point value 50), pyramided, reversal | 7 | 7 / 7 |

That is 60 of 60 entries per order and 54 of 54 per contract, and all 366
trades of the corpus probe `order-percent-equity-cash-commission-01` (25 %,
1.5 per order). The restored basis fits only the flat entries; no fee
reserve, or a per-contract fee not scaled by the point value, fits none. A
priced entry sized at the signal close instead of its level fits none of the
eight.

**The fix is adapter policy** (`default_sizing_cash`,
`default_sizing_reserves_cash_fee`): the sizing snapshot records
`strategy.equity` beside the restored equity (`PineSizingSnapshot::strategy_equity`,
set by `mark_sizing_equity` only where the run declares a percent-of-equity
default quantity and a positive cash commission, and folded only there), and
the money it hands the kernel's `CashValue` is the percentage of it less the
per-order fee, or scaled by `unit / (unit + fee)` per contract. The money
gates and the margin money keep the restored equity; a per-call `qty_type`
(which TradingView's compiler refuses) keeps its conversion.

**Replay** (`tests/test_cash_fee_sizing_tapes.cpp`, the tapes' scripts through
the Pine adapter on their bars): on the eleven tapes without an entry-bar
margin call the adapter books 0 of 111 trades as TradingView does before the
fix and 111 of 111 after it (ids, sides, times, prices and quantities exact,
commission to 1e-9, net profit to 1e-7 of |profit| + |commission|).

**Corpus.** `order-percent-equity-cash-commission-01` is the corpus's only
probe with a percent-of-equity default quantity and a cash commission, and the
only one that moves. The corpus harness gives it no quantity grid, so its
quantities are the raw quotient; before → after: quantities inside
TradingView's 0.0001 cell 0 → 355 of 366 (the other 11 lie 1e-7 to 3e-6 below
the cell, where the harness's unfloored quantities left the equity a few cents
under TradingView's), largest |Δ quantity| 0.00106 → 0.0000985, 90th
percentile |Δ net profit| 0.029 → 0.0023; every one of the 366 quantities and
net profits is closer.

**Hash step.** Only runs that record the new snapshot value move:
`tests/test_adapter_quiet_bar_pinned.inc` 80 values over 24 rows, the six
random scenarios with a percent-of-equity default quantity and a cash-per-order
fee (Random05, 06, 20, 35, 38, 46), no count; and
`tests/test_publication_witness_pinned.inc` 34 of 156 digests -- exactly the
Config runs whose configuration sweep passes through such a declaration, by a
census of the battery -- six of them closing a different number of trades.
`tests/test_l4b_policy_regressions.cpp` pinned the old rule (20 units where
the rule gives 9.9 + 9.89).

**Findings, recorded and not changed here.**

1. At margin 100 TradingView margin-calls a default 100 % entry on its own
   entry bar when the order's cash fee leaves the equity under the position
   (`pcf-order-p100-m100`: a long filled 0.01 above its signal close loses
   0.0008 at the open and 2.2032 more on the bar; a short loses 0.4592 at the
   bar's high), and every margin-call order pays its own per-order fee. The
   adapter's margin equity restores a cash entry fee
   (`NativeMarginEquityBasis::MarkedEquityBeforeOpenCommission`) and books no
   such call, so the two tapes part from their second scenario on; the test
   pins the adapter's rows as a recorded divergence. The quantity sized is
   TradingView's: the first scenario is TradingView's trade and the second's
   quantity the sum of TradingView's pieces.
2. The rule converts at the kernel's `price × pointvalue × fx`; no tape has an
   account currency other than the symbol's, so the FX case is unmeasured.
...
| P4 | **Hash compatibility.** The source hash covers hundreds of adapter and scheduler fields (`PineExecutionAdapter::hash_state` pine_state_hash.cpp:242, `PineScheduler::hash_state` pine_state_hash.cpp:618); L4 / L9 add hashed state, L12 removes some | A hash mismatch is a diagnostic until the trade diff is known, never a reason to relax parity (S). Open: does the campaign gate compare broker-state hashes across releases, or only trades (O)? → Q3 | O S |
...
| Q4 | `SizeTime::AtAcceptance` needs a current execution point (`current_execution_point` native_execution_consumer.cpp:2876); between realtime inputs there is none — reject or fall back to `AtMatch`? | F: reject with `InvalidQuantityBasis` |
...
- TUs: `native_execution_consumer.cpp` (seal path native_execution_consumer.cpp:5085-5127), `engine_run.cpp`, `engine_report.cpp`, `engine_state_hash.cpp`, `native_run_spec.{hpp,cpp}`.

### A.2 L3 — sizing, fractional reduce, sibling reservation (F P1, P7 + O G2 + S P2, P3)

```cpp
// native_order.hpp — native_order_v6; OrderIntent grows 5 -> 6 (static_assert native_order.hpp:1388)
...
struct Sized {                                         // O's name; F: SizedOpen
    ...
    ExecutionGridPolicy grid_policy = ExecutionGridPolicy::SnapToGrid;      // existing enum, `ExecutionGridPolicy` native_order.hpp:82
...
- Resolution: `units = cash / (price * point_value * fx)`, `cash` = the value or `fraction * marked_equity(price)`, `price` = the candidate's `default_resolved_price`. It plugs in where `HostSized` units resolve today (native_execution_consumer.cpp:693; terms resolution `resolve_execution_terms` native_execution_consumer.cpp:4958). A host override of `resolve_execution_terms` keeps the last word. Non-representable → `MatchRejectReason::TermsUnresolved` (native_order.hpp:877); validation reuses `RequestRejectReason::InvalidQuantityBasis` (native_order.hpp:827).
...
- Mechanism (F): a **kernel-originated request**. At each script-bar open and after each applied fill the kernel computes the liquidation level and rests a `Reduce` / `Flatten` with `Stop{level}` bound to the live openings — the shape the adapter builds host-side today (pine_adapter.cpp:13299-13320). New hashed `RequestDefinition::origin { Host, KernelLiquidation, KernelRisk }`, new `CancelReason::Superseded` on re-price, `MarginCallEvent` joins `CommandEvent` (17 → 18, native_order.hpp:1308). O's alternative, a checkpoint on the excursion walk (`apply_excursion` native_execution_consumer.cpp:2622), is not chosen: a reduction mid-path is the entanglement O itself flags (E2).
- FX revaluation (MG9) falls out once `native_fx_curve` drives the mark (`native_fx_curve.cpp`).
- TUs: `native_run_spec.{hpp,cpp}`, `native_execution_consumer.cpp` (`admit_opening_inspect` native_execution_consumer.cpp:4684-4697 per side; scheduling next to `invoke_bar_open_callback` / `drain_after_applied`), `native_order.*`, `engine_state_hash.cpp` + the coverage checker, `c_abi.cpp` + `pineforge.h` (`pf_native_run_spec_v2` or an extension struct; `CLAUDE.md` SOP).
...
1. At one point: match and settle → `on_native_applied` per applied event, FIFO (rule already at `on_native_applied` native-engine.md:1701-1702) → with `BarCloseAndFills`, one recalculation at the fill cursor (reason `OrderFill`), driven from the existing notification drain (native_execution_consumer.cpp:6995-7007) under its re-entrancy guard, bounded by `max_recalculations_per_point`. S would run it *before* the drain; not chosen, the drain order is frozen (E4).
2. Requests born in any of these callbacks follow the existing birth rule: eligible on the unconsumed rest of the bar (`born_on_remaining_path` native_execution_consumer.cpp:5641). `born_on_remaining_path` (native_execution_consumer.cpp:5641-5645) is not touched — the adapter's COOF deferral (`pending_coof_requests_`, element type `PendingCoofRequest` pine_adapter.hpp:1641-1648) is calibrated against it.
3. `EveryModeledPoint`: a calculation at each magnifier sample / observed print, in batch as well as stream (every delivery loop asks the per-point guard native_execution_consumer.hpp:1116-1123, whose body is `recalculate_at_modeled_point` native_execution_consumer.cpp:6857-6864). `on_native_tick` stays the observation hook (S).
4. `current_partial_bar()` derives from the driver points already recorded (`record_driver` native_execution_consumer.cpp:2566); valid in bar-open, applied and tick callbacks.
5. The kernel never attempts language-state rollback; that stays Pine's (`snapshot / restore / commit_coof_script_state`, pine_scheduler_native.cpp:52-104) (O).
...
std::size_t cancel_where(std::string_view label);                    // O: optional kernel label -> live-handle index (labels exist, `Request::label` native_order.hpp:485)
...
struct ReplaceOptions { bool retain_trigger_state = false; };          // O: keeps `TrailTrack::best` (native_order.hpp:651)
// Trail::offset == 0.0 is accepted: "exit on the first adverse move past the best" (now accepted: `valid_trail_offset` native_order.cpp:496)
...
- A leg with owner `WaitForApplied` / `BindOpening` and a non-absolute anchor gets its level when the owner's fill arms it (`ArmedEvent` native_order.hpp:1246-1253 already exists) (O). Fallback with no kernel change: the toolkit materializes relative levels from the host's `on_native_applied` (F).
...
- TUs: `native_run_spec.cpp` (validation), `native_execution_consumer.cpp` (resolve path native_execution_consumer.cpp:3962-3984, `execute_current` path native_execution_consumer.cpp:3791-3801), `native_matching.hpp` (threshold form). The dead helpers were the reference arithmetic and are deleted (§2.ii e); the kernel's own step is `native_matching::apply_slippage` native_matching.hpp:259.
...
- Breach: `MatchRejectReason::RiskLimit` for openings (O; F: `RiskHalted`), an optional kernel-issued flatten through A.3's kernel-originated request (`origin = KernelRisk`, F), and a `NativeRiskEvent` in `native_events` (native_host.hpp:1335) (O) or a callback `on_native_risk_event` (S — a virtual, so only inside an epoch batch). The day key uses the existing calendar (`session_day_ordinal` native_calendar.hpp:416), not a chart clock.
- S's `directions` / `max_units` / `max_position_units` are existing spec fields (MG1-MG3), not duplicated.
- Interim (F): a C++ host can build all four today from `native_marked_equity` + `on_native_applied`, which is what the adapter does (pine_adapter.cpp:13820-13918); a toolkit helper is an acceptable stand-in until L9.
- TUs: `native_run_spec.{hpp,cpp}`, `native_execution_consumer.cpp` (`admit_opening_inspect` native_execution_consumer.cpp:4684-4697 and the per-point walk), `native_order.hpp`, the state hash.

### A.9 Single-source proposals not adopted

| Proposal | Src | Why not now |
|---|---|---|
| `NativeExcursionMode { PathExtremes, FillOnly, HostOwned }` + per-lot sample events | S P8 | The kernel sampler + A48 ownership already cover RP1 (F, O). TradingView's tapes found gaps in the host-owned half, not in the kernel's (RP1, R5 lane H-MEASURE); a mode enum would not close them, and R5 lane H-THIN lowered the Pine host onto the sampler |
