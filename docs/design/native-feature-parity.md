...
| MG6 | Maintenance margin + forced liquidation (`submit_margin_call_slice` pine_adapter.cpp:14077-14255, `submit_margin_call_units` pine_adapter.cpp:14216-14262, `schedule_margin_call_path` pine_adapter.cpp:18227-18304) | **no** — "no maintenance liquidation" `initial_margin_fraction` native_run_spec.hpp:597-598; margin is checked only when `inspect.would_open` native_execution_consumer.cpp:2936 **Closed:** `maintenance_long` / `maintenance_short` with `NativeLiquidationCheck::{PathAdverseExtreme, PathAdverseExtremeMark, CalculationOnly}`; the kernel solves the level, rests its own `Reduce`/`Flatten` bound to the live book, re-prices it with `CancelReason::Superseded` and books a `MarginCallEvent` (`tests/test_native_margin_model.cpp`, `tests/test_native_margin_hooks.cpp`). | K+A | L4 | F:D2 O:D3 S:M3 | TV parts → MG15 |
...
- Under the magnifier on an aggregated chart, the entry-bar masks and
  ...
  **Closed (R5 lane H-MEASURE):** all three entry-bar tests of a magnified
  ...
  run books exactly; the ones it does not are recorded divergences of their
  own: a stop entry already triggered at its bar's close filling a bar late).
