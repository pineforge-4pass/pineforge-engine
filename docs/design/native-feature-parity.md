...
| MG9 | FX revaluation of margin (`apply_fx_open_margin_slice` pine_adapter.cpp:2980-2996, `apply_fx_opening_margin_slice` pine_adapter.cpp:2998-3026) | **no** — staged FX curve native_host.hpp:484 but no margin re-check | K | L4 | O:D8 | (single-source) |
...
| FP6 | Account-currency FX | **partial** — scalar `account_fx` + immutable `NativeFxCurve` native_host.hpp:484 in batch; the stream refuses a non-empty curve native_execution_consumer.cpp:1251-1261, native-engine.md:415 | K | unassigned | F:D9 S:F4 | **Disagree:** F out of scope ("O7 clock" native-engine.md:739-744) / S its P10, a stream-safe FX epoch. No R5 lane → §4 U4. |
...
| OT8 | *(quirk)* Live-tail / probe-suppress harness flags (pine_strategy_host.cpp:270-272) | n/a | A | — | F:J7 S:C5 | parity-campaign tooling |
...
4. **Delivery rule (F):** a subscription bar is delivered when its last contributing input bar is accepted, immediately before that input's script calculation, never earlier.
