...
| OL6 | Ownership: legs bound to a parent / opening(s) / cohort; close the lots of a given entry, FIFO or ANY (`owner_for_close` pine_adapter.cpp:5451-5486, `cohort_for` pine_adapter.cpp:2335; FIFO prefix computed adapter-side `source_fifo_prefix_openings` pine_adapter.cpp:7484-7501) | **yes** — `Owner` native_order.hpp:464-479, `Authority` native_order.hpp:620-621, `CohortClose` native_order.hpp:617-619, `SelectedExposure` native_order.hpp:625-629, `cohort_open` native_host.hpp:1282, `CancelReason::OwnerGone` native_order.hpp:949 | K | — | F:A6 O:A8 O:A9 S:O5 | FIFO *ordering* is host-computed (O) |
| OL7 | OCA cancel / reduce groups (`group_for` pine_adapter.cpp:2609-2615) | **yes** — `Group` / `GroupEffect` native_order.hpp:466-473, `CancelReason::Group` native_order.hpp:948-953, `ReservationReducedEvent` native_order.hpp:1191-1201 (deferred form `DeferredGroupAdjustmentEvent` native_order.hpp:1211-1221) | K | — | F:A12 O:A6 O:A7 S:O7 | — |
...
| TR3 | Zero-distance trail, "ride the best, exit on any adverse tick" (adapter substitutes `tick * 0.5` pine_adapter.cpp:11817-11829) | **no** — the kernel fails the run when `offset <= 0` (`checked_trail_stop` → native_execution_consumer.cpp:4473-4478) **Closed:** a zero offset is legal and means *ride the best* — the level is the running best itself and the exit is the first print strictly past it; negative and nonfinite offsets stay rejected (`tests/test_native_order_ergonomics.cpp`, `tests/test_native_anchored_legs.cpp`). | K+A | L7 | O:B3 F:B3 S:T2 | **Disagree:** O kernel / F, S quirk. R5-1: accepting a zero offset is generic; TV zero-offset *pricing* (`zero_trail_sibling_stop` pine_adapter.cpp:11730-11867) stays A. |
...
| TR5 | *(quirk)* TV trail conventions: half-tick arm threshold against tick-quantized extremes `has_trail_request` pine_adapter.cpp:9618-9623, zero-offset pricing `zero_trail_sibling_stop` pine_adapter.cpp:11730-11867, retained best `retained_trail_best` pine_adapter.hpp:287-289, operand / restart rules `source_trail_points` pine_adapter.cpp:8635-8861, `trail_one_shot` pine_adapter.cpp:9707-9836 | n/a | A | — | F:B3 S:T2 | R5-1 |

### 1.3 Sizing bases and reservations

| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| SZ1 | Explicit units (`Transact{signed}` after a grid floor pine_adapter.cpp:5862-5866, pine_adapter.cpp:5862) | **yes** — `Transact` in `OrderIntent` native_order.hpp:229 | K | — | F:C1 O:C1 S:S1 | — |
...
| SZ3 | Percent-of-equity sizing (`default_sizing_units` pine_adapter.cpp:2458-2513) | **no** — `HostSized` without host units is `TermsUnresolved` native_execution_consumer.cpp:4987-4989 **Closed:** `Sized{Side, EquityFraction{f}}` (`OrderIntent` native_order.hpp:229) is the sixth intent — the kernel resolves the units from a fraction of marked equity at the sizing point, so a bare host needs no terms override (`tests/test_native_sizing_bases.cpp`). | K+A | L3 | F:C3 O:C2 S:S3 | TV parts stay A: rounded equity `source_money_round` pine_adapter.cpp:2403, money floor-to-lot `source_money_floor_lot` pine_adapter.cpp:378 (→ SZ10) |
| SZ4 | Cash-value sizing (`default_qty_type` pine_adapter.cpp:2412-2421) | **no** — same **Closed:** `Sized{Side, CashValue{cash}}`, the same resolution over an account-currency amount (`tests/test_native_sizing_bases.cpp`). | K | L3 | F:C2 O:C3 S:S4 | A per-call `qty_type` (cash or percent of equity on one `strategy.entry`) is not Pine — TradingView's compiler refuses it, so only generated or C++ callers reach it — and since R5 lane B-ADAPTER it converts through the kernel's `native_sized_units` exactly as the declared default does (`typed_quantity_units` pine_adapter.cpp:2471, `typed_entry_units` pine_adapter.cpp:2489; `tests/test_adapter_typed_entry_admission.cpp`) |
...
| SZ8 | Fractional reduce of the position / a cohort (`quantize_close_units` pine_adapter.cpp:12527-12555, `compute_exit_reservation` pine_adapter.cpp:4289-4408) | **no** — sizes are `ExplicitUnits` / `OwnerOpenedUnits` / `HostSized` only native_order.hpp:60-74 **Closed:** `Reduce{ScopeFraction{fraction, claim, basis}}` takes a fraction in `(0, 1]` of the scope the reduce is bound to — the book, one opening, or a cohort roster (`tests/test_native_sizing_bases.cpp`). | K | L3 | F:A7 O:A12 O:C4 S:S5 | F's "partial" counts the units-reduce of OL2. R5-4 → L3. |
...
| SZ10 | *(quirk)* Ten-significant-digit money rule (`source_money_round` pine_adapter.cpp:369-376, `source_money_floor_lot` pine_adapter.cpp:378; twin `tv_money_round` pine_policy_support.hpp:9-15); exact percent-exit rounding (`quantize_percent_exit_units` pine_adapter.cpp:4321-4336) | n/a — kernel money is plain binary64 `marked_equity` engine_execution.cpp:1638-1657 | A | — | O:C8 F:C3 S:M4 S:S5 | R5-1 |
| SZ10a | *(quirk, measured retention)* The money band's affordable price, `sig10(sig10(E) / notional per unit of price)` against the sizing price — at placement (`entry`, `affordable_price` pine_adapter.cpp:6718), in the paired-reversal whole drop (`close`, `affordable_price` pine_adapter.cpp:7924), at the terms boundary (`resolve_terms`, `affordable_price` pine_adapter.cpp:12785), in the precommit money scopes (`validate_precommit`, `affordable_price` pine_adapter.cpp:13161, pine_adapter.cpp:13214) and in the carried-money check (`apply_reversal_gap_bracket_policy`, `affordable_price` pine_adapter.cpp:15547) | n/a — not a units conversion. Every money-to-units conversion in the adapter is the kernel's `native_sized_units` under the source's floors since R5 lane B-ADAPTER: the default at placement and at the fill (`default_sizing_units` pine_adapter.cpp:2458), the typed per-call quantity at placement, at the fill and in the probe (`typed_quantity_units` pine_adapter.cpp:2471, `typed_entry_units` pine_adapter.cpp:2489), and the pending-order fill-quantity probe (`probe_fill_qty` pine_adapter.cpp:18652) | A | — | AUDIT3-opus2 H4 (G2 R2, R3) | **Measured:** the quotient is TradingView's whole-drop PRICE, pinned by 445 famr3 tapes (`tests/test_tv_money_band_l4b.cpp`) and 202 famr tapes (`tests/test_tv_money_precision_l4b.cpp`); on a seven-digit tie (E 9998.99999996, 10 000 lots at tick(close) 0.9999) the rule admits while the kernel's conversion buys 9999.99999996 lots and binary64 gives E/Q 0.999899999996 (`tests/test_adapter_fill_qty_probe.cpp`, section 2). |
...
| ID | Feature (adapter mechanism) | Native today | Class | Lane | Sources | Notes |
|---|---|---|---|---|---|---|
| MG1 | Allowed opening directions (`allowed_open_directions` pine_adapter.cpp:2195) | **yes** — `allowed_open_directions` native_run_spec.hpp:597 → `MatchRejectReason::OpeningDirection` native_execution_consumer.cpp:2956-2964 | K | — | F:D4 O:D7 S:M1 | the one risk rule the adapter delegates (O) |
| MG2 | Max absolute position size | **yes** — `max_abs_units` native_run_spec.hpp:595 tests the resulting book native_execution_consumer.cpp:2970-2973 | K+A | — | F:D5 O:D5 S:M1 S:M5 | **Disagree:** O, S:M5 partial. R5-1: the generic cap is native; the adapter's live-position `>= held` variant (`max_position_size` pine_adapter.cpp:13056-13059) is TV. |
...
| MG11 | Max consecutive losing days (`SourceDayLedger` pine_adapter.hpp:1197-1205; gate `max_cons_loss_days` pine_adapter.cpp:13145-13148) | **no** — observation only `loss_days_limit` include/pineforge/source/market_admission.hpp:49 **Closed:** `NativeRiskLimits::max_consecutive_loss_days`, settled from each day's realized result at the next day's open (`tests/test_native_risk_limits.cpp`). | K | L9 | F:D6 O:E2 S:M6 | R5-9 |
| MG12 | Max intraday loss → flatten + block (`intraday_loss_breached` pine_adapter.cpp:13720-13734, forced flatten `submit_intraday_loss_close` pine_adapter.cpp:14789-14819) | **no** — observation only `intraday_loss_limit` include/pineforge/source/market_admission.hpp:51 **Closed:** `NativeRiskLimits::max_intraday_loss` with `NativeRiskAction::FlattenAndBlock`, under which the kernel issues one `Flatten` of its own at the breach point (`RequestOrigin::KernelRisk`) and latches the block before that flatten settles (`tests/test_native_risk_limits.cpp`). | K | L9 | F:D7 O:E3 S:M6 | R5-9 |
...
| CT2 | Process orders on close (`close_execution` pine_adapter.cpp:2152-2153) | **yes** — `NativeCloseExecution::AfterCalculation` native_run_spec.hpp:35-38; `AfterCalculation` native_execution_consumer.cpp:7588-7592 | K | — | F:E2 O:F6 S:C1 | — |
| CT3 | Pre-match hook at bar open (`on_bar_open` pine_adapter.cpp:15986) | **yes** — `on_native_bar_open` native_host.hpp:889 runs before the open match native_execution_consumer.cpp:7096-7100; `execute_current` legal native_host.hpp:1104 | K | — | F:E3 O:F2 | hazard → CT4 |
...
| RP7 | Live order-action stream (runner ledger / webhook) | **yes** — `stream_observe_actions_` engine_execution.cpp:1293, enabled `stream_observe_actions_` native_execution_consumer.cpp:9359, C ABI `strategy_stream_order_actions_len` c_abi.cpp:652-669 | K | — | F:H5 | (single-source) |
| RP8 | Continuation hash | **yes** — `native_continuation_hash` native_host.hpp:1376, native_execution_consumer.cpp:10765; the spec is folded by `hash_spec` native_execution_consumer.cpp:238-260 | K | — | F:I1 O:J2 S:X3 | — |
...
- **Before L1:** one static library always compiled the source layer and installed every header, `include/pineforge/source/**` and `include/pineforge/compat/pine/**` included, and `src/compat/pine/market_admission.cpp` sat in the *main* list for historical reasons (added by `3b13183`, before the source-layer list existed — F). **Since L1:** the source layer is its own list (`PINEFORGE_SOURCE_LAYER_SOURCES` CMakeLists.txt:91, `market_admission.cpp` at :94), appended to `pineforge` only when `PINEFORGE_BUILD_SOURCE_LAYER` is ON (`add_library(pineforge STATIC` CMakeLists.txt:164-167), beside the kernel-only archive (`pineforge_kernel` CMakeLists.txt:153), whose header install excludes `source` and `compat` (`PATTERN` CMakeLists.txt:426-431).
...
| the rich-`run` paragraph | the rich `run` overload fails a native host | `run_rich` native_execution_consumer.cpp:9229-9269 shows no such refusal (still unpinned by a test; the page says so) |
...
|---|---|---|---|---|---|
| `strategy.risk.allow_entry_in` | `risk_.direction`; `project()` already declares `spec.allowed_open_directions` (pine_adapter.cpp:2195); the forbidden opposite entry is reshaped to `CloseOpposite` in the adapter's own terms branch | per-opening cap `allowed_open_directions` | (i)+(ii), **already re-lowered** | none to measure: the cap is the kernel's, the close-only reshaping is the retained TV policy | done before N12; nothing to move |
| `strategy.risk.max_position_size` | precommit refuses a flat/same-side ENTRY at its fill when the LIVE book already holds ≥ the limit (`max_position_size` pine_adapter.cpp:13056-13057; the retired legacy rule is `git show ab9714be:src/source/pine_risk.cpp`, lines 115 and 2531) | `max_abs_units`: refuses a fill whose RESULTING book would exceed it | (iv) | `PS`: two-unit entries against 3 — adapter admits the second (live 2 < 3, book 4) and refuses the third; kernel refuses the second (`MaxAbsUnits` at bars 1, 2), book 2 | retained: a pre-fill-book gate is a TV emulation fact, not a generic cap |
| `strategy.risk.max_drawdown` | `update_risk_state(bar.close)` once per script bar at the close mark (`update_risk_state(bar.close)` pine_adapter.cpp:16715, `update_risk_state` pine_adapter.cpp:13691-13718): peak / running max drawdown at close marks, `>=` exact, percent of the current peak; the latch `risk_.halted` gates flat/same-side entries at precommit (`risk_.halted` pine_adapter.cpp:13049), never an opposite entry | `max_drawdown`: current drawdown vs percent-of-peak, at all three points; `BlockOpenings` refuses every opening | (ii) cadence + (iv) scope | `MG10-a`: bar 1 opens 600 under the entry and closes 100 under — kernel breaches at the open mark and refuses bar 2's add (book 100), adapter admits it (book 200). `MG10-b`: latched at bar 1's close, the adapter still reverses twice (`L→S` −600, `S→L`), the kernel refuses both reversals (`RiskLimit` at bars 2, 3) and holds. Corpus: `ta-closedtrades-risk-introspection-01` (20 % of peak) identical 1502/1502 rows with the kernel seeded — the rule never fires there | retained: the latched-reversal exemption cannot be composed on a kernel block; a close-only cadence knob alone would not close it |
...
(pine_adapter.cpp:258), `source_level_on_price_grid` (:244) and the
...
`reset_for_run` pine_adapter.cpp:1942 — and reflected the whole admission
...
3. **A cache must know which rows changed, and nothing records it.**
   ...
   (`has_full_entry_bracket` pine_adapter.cpp:8776). Full-table scans
   rewrite rows as well (`suspend_brackets_for_reversal` pine_adapter.cpp:1529,
   `revive_brackets_after_margin` pine_adapter.cpp:1751,
   `preserved_by_close_all` pine_adapter.cpp:11518), and
...
(`retire_cancelled_exits` pine_adapter.cpp:1677), the revival skips a withdrawn
...
pine_adapter.cpp:2793). Both shapes now cost ×1.89 to ×2.12 per doubling up
...
it off (`release_closed_cohort_origins` pine_adapter.cpp:3224); a lot opened
...
3. `EveryModeledPoint`: a calculation at each magnifier sample / observed print, in batch as well as stream (every delivery loop asks the per-point guard native_execution_consumer.hpp:1083-1090, whose body is `recalculate_at_modeled_point` native_execution_consumer.cpp:6857-6864). `on_native_tick` stays the observation hook (S).
