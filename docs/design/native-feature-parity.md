...
| R5-2 | **Brackets are not a new kernel object.** `WaitForApplied{RequestHandle parent}` (native_order.hpp:385-387) and `waiting_children(parent)` (native_order.hpp:1652, *fresh*) exist; L7 delivers a builder/toolkit over them plus relative anchors and the missing offsets. Size M. |
...
| OL6 | Ownership: legs bound to a parent / opening(s) / cohort; close the lots of a given entry, FIFO or ANY (`owner_for_close` pine_adapter.cpp:3857-3892, `cohort_for` pine_adapter.cpp:1648; FIFO prefix computed adapter-side pine_adapter.cpp:5519-5536) | **yes** — `Owner` native_order.hpp:413-428, `Authority` native_order.hpp:569-570, `CohortClose` native_order.hpp:566-568, `SelectedExposure` native_order.hpp:574-578, `cohort_open` native_host.hpp:1137, `CancelReason::OwnerGone` native_order.hpp:842 | K | — | F:A6 O:A8 O:A9 S:O5 | FIFO *ordering* is host-computed (O) |
...
- Mechanism (F): a **kernel-originated request**. At each script-bar open and after each applied fill the kernel computes the liquidation level and rests a `Reduce` / `Flatten` with `Stop{level}` bound to the live openings — the shape the adapter builds host-side today (pine_adapter.cpp:11426-11447). New hashed `RequestDefinition::origin { Host, KernelLiquidation, KernelRisk }`, new `CancelReason::Superseded` on re-price, `MarginCallEvent` joins `CommandEvent` (17 → 18, native_order.hpp:1167). O's alternative, a checkpoint on the excursion walk (native_execution_consumer.cpp:1769-1782), is not chosen: a reduction mid-path is the entanglement O itself flags (E2).
