...
(pine_adapter.cpp:330), `source_level_on_price_grid` (:316) and the
...
`reset_for_run` pine_adapter.cpp:1924 — and reflected the whole admission
...
2. **The transforms need a home.** A row's transform, or a composed
   ...
   pine_adapter.hpp:364, `admission::Journal` market_admission.hpp:188 or
...
3. **A cache must know which rows changed, and nothing records it.**
   ...
   pine_adapter.hpp:500, the non-const `at` pine_adapter.hpp:516 and its
