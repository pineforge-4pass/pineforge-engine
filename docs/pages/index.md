...
This snippet calls four of the eight per-strategy exports generated by PineForge codegen.
Compile it as part of the generated strategy module and link that module with
`libpineforge`; these symbols are not provided by the archive on its own.
...
Build the generated strategy module with the repository recipe (or
`bash tutorial/run.sh`); a standalone `cc demo.c -lpineforge` link cannot provide
`strategy_create`, `run_backtest` or `report_free`.
