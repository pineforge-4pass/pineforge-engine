...
| `strategy.cancel_all()` / cancel by id | `cancel_all()`, `cancel_where(comment)`, `native_working_requests()` native_host.hpp; `native_toolkit::OrderBook<Key>` for an id → handle book | A `PendingUntilArmed` child is not in the working enumeration before its arm but `cancel_all` / `cancel_where` / `cancel(handle)` still address it. |
