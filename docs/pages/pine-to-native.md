...
| `process_orders_on_close` | `NativeCloseExecution` native_run_spec.hpp:33 | `close_execution` pineforge.h:503 | `native_open_lots_strategy.cpp` | `AfterCalculation` emits the post-calculation close point native_execution_consumer.cpp:7662-7663, so a request submitted in bar *k*'s calculation fills at bar *k*'s own close. |
...
holding exactly the objects of `PineForge::kernel` CMakeLists.txt:155, and
