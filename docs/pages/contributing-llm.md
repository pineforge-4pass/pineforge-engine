...
10. **A test row never silently disappears.** Each profile counts the rows that
    *ran* against a floor: `KERNEL_MIN_TESTS` ci_verify.py:264 and
    `RELEASE_MIN_TESTS` ci_verify.py:408. Adding rows means raising the floor
    in the same commit.
...
(`KERNEL_MIN_TESTS` ci_verify.py:264, `RELEASE_MIN_TESTS` ci_verify.py:408). It
