...
10. **A test row never silently disappears.** Each profile counts the rows that
    *ran* against a floor: `KERNEL_MIN_TESTS` ci_verify.py:122 and
    `RELEASE_MIN_TESTS` ci_verify.py:157. Adding rows means raising the floor
    in the same commit.
...
`strategy_native_*` surface native_c_api.h:1669.
...
(`KERNEL_MIN_TESTS` ci_verify.py:122, `RELEASE_MIN_TESTS` ci_verify.py:157). It
