...
10. **A test row never silently disappears.** Each profile counts the rows that
    *ran* against a floor: `KERNEL_MIN_TESTS` ci_verify.py:108 and
    `RELEASE_MIN_TESTS` ci_verify.py:137. Adding rows means raising the floor
    in the same commit.
...
through `NativeStrategyHost` native_host.hpp:778, or C through the
`strategy_native_*` surface native_c_api.h:1516.
...
(`KERNEL_MIN_TESTS` ci_verify.py:108, `RELEASE_MIN_TESTS` ci_verify.py:137). It
