...
10. **A test row never silently disappears.** Each profile counts the rows that
    *ran* against a floor: `KERNEL_MIN_TESTS` ci_verify.py:195 and
    `RELEASE_MIN_TESTS` ci_verify.py:274. Adding rows means raising the floor
    in the same commit.
...
`strategy_native_*` surface native_c_api.h:2547.
...
(`KERNEL_MIN_TESTS` ci_verify.py:195, `RELEASE_MIN_TESTS` ci_verify.py:274). It
