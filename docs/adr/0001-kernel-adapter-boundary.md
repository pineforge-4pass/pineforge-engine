...
- **Callbacks are not close-only.** `on_native_bar` is the pure-virtual script-bar calculation
  ...
  (`native_host.hpp:870`, `on_native_bar_open` `native_execution_consumer.cpp:6910-6912`) —
...
- **Matching, fills, and who sizes.** The kernel matches geometrically along the bar path and
  ...
  (`TermsUnresolved` `native_execution_consumer.cpp:4845`).
...
- **The rest of the host surface**, all on `NativeStrategyHost`:
  ...
  | Failure model | `NativeFailureCode` (`native_host.hpp:63-77`); `Failed` is latched, nothing resumes. A throwing callback latches `CallbackException` (`CallbackException` `native_execution_consumer.cpp:4818`) |
...
- **Feeding data.** Bars enter through the engine's own entry points: `run(bars, n)`
  (``sha256:1eb25aef104b9f79988d37cffef5383089cac9098a163627c4709370ca830540` `sha256:205011b38f723ee290154f642194350b4e011465ec2f419e6d947761b5f84e8c` engine.hpp:1700`), the timeframe `run` (`engine.hpp:1712`), the rich begin
  (`run` ``sha256:edf23986b5b1ea9448faea0cc9eb8f5191ccdd1b1f4adb4e2368dec82aee38d0` engine.hpp:1785`), or `stream_begin` (`engine.hpp:1755`) / `stream_push_bar`
  (`engine.hpp:1761`) / `stream_push_tick(s)` / `stream_advance_time` / `stream_end`
  (`engine.hpp:1765`). `market_driver.hpp` is driver *types* (`NativeCoordinate` `market_driver.hpp:71`), not a feed API; read
...
- **Reproducibility.** Two digests, one wrapping the other. `native_continuation_hash()`
  ...
  `broker_state_hash()` (``sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855` engine.hpp:2028`)
...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  ...
  `validate_precommit` (`pine_adapter.cpp:12809`) returns `AdmitWithHostMargin`
  (`pine_adapter.cpp:13054`) to own the opening margin decision; `owns_lot_excursions() = true`
  ...
  `NativeCloseExecution::AfterCalculation` in the projected spec (`pine_adapter.cpp:2259-2260`);
...
- **Batching and open-order priority.** Same-bar command batching and its deferred queues
  (``sha256:27e64b2cd0bbe283a8c83056538d66cd374e53cad072772c775a8529624768ac` pine_adapter.hpp:1785-1816`); the retained parent-before-child ordering of live handles — an
...
- **Trail and tick conventions.** The half-tick arm threshold measured against tick-quantized
  extremes while the raw running best is retained (`has_trail_request` `pine_adapter.cpp:9381-9386`); the half-tick
  ...
  adapter's terms seam (`resolve_terms` `pine_adapter.cpp:11446`).
...
- **Close reservations.** `strategy.close` callsite batching, two-call provenance and the entry-id
  ledger (`close_logical_units_` `pine_adapter.hpp:2011-2022`); the POOC reservation-growth population predicate
...
- **Margin.** The kernel owns the margin *mechanism* — the level solve, the check points, the
  ...
  with TradingView's: `margin_check_allowed` (`pine_adapter.cpp:13844`) for the scheduling,
  `resolve_margin_requirement` (`pine_adapter.cpp:13880`) for the ten-significant-digit money,
  ...
  (`non_pooc_commissioned_short` `pine_adapter.cpp:16578`), the pre-open admission slice and
...
- **Calculation timing.** The cadence itself is the kernel's: a `calc_on_order_fills` strategy
  ...
  (`next_source_path_waypoint` `pine_adapter.cpp:8072`), the first-open execution chain and its
  own loop guard (`kFirstOpenLoopGuard` `pine_scheduler_native.cpp:681`), and the two fills Pine refuses to
  recalculate on (`suppress_grouped_stop_recalc` `pine_adapter.cpp:5431`).
...
- **The four rules earlier drafts named — one survives as code, and it is hashed.** The
  ...
  are **gone** from the kernel — the ledger is adapter state (`close_logical_units_` `pine_adapter.hpp:2011-2022`) and
...
- **The C API trades.** Two disjoint runtime inventories, both pinned.
  ...
  **43** additive symbols of `<pineforge/native_c_api.h>`. A C host creates a host from a
  callback table (`strategy_native_host_create_v1` ``sha256:755c4cd5bc693ea9278bdecf757fe64e1094a71536641e1f4753984d572fa8e5` native_c_api.h:2607`), runs a batch
  (`strategy_native_run_v1` `native_c_api.h:2611`), and **submits, replaces, cancels and
  executes** orders — `strategy_native_submit_v1` (``sha256:9b2a62efb044f5b285a824d5fb15be418b459ffd83c5dd4e1013cf798c8a9a32` native_c_api.h:2648`), `_replace_v1`
  (``sha256:fc1927f5b4fe78ca4e8a34b3f57629d2b72e5efa5b54ca564253a0abe01acc51` native_c_api.h:2670`), `_cancel_v1` (``sha256:02a7ec6c16a711c23d56fe0879c02c77e2b694f4c2f56c252d0e157f6efdb0eb` `sha256:c0be5e206c8805cc07c683edf0327fcc3553663f77cb955bcb6daf542993ffb4` native_c_api.h:1477`), `_cancel_all_v1` (``sha256:1fa883d1460c5fdb5e3d98441af75892f0e7f91289de4b35f6660e6e1ace4775` `sha256:7086fa3aec5dcfcb337af5adbcd51cdd684ab4143d98704d419d067a9c2c331e` native_c_api.h:1481`), `_cancel_where_v1` (``sha256:638eac74d2670de4db13928dc1e8ac32c5a3589d7b3a5a01bb711fcedfc0b90b` `sha256:dd01b6a5057be1edcd756278169c05dd888365a92f55a20b677e7e443f2e5c1e` native_c_api.h:1517`),
  `_execute_current_v1` (``sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855` `sha256:772296a782ed58388867cf24770ef77ce725aafbfe6f5d0c8345af26aa13cd58` native_c_api.h:1538`) — under the kernel's own legality rule. Streaming needs no new
...
|---|---|---|
| `WaitForApplied::first_match` / `WaitForApplied::scope` | `arm_first_match` (``sha256:4c4077f5233d7b7b67042c0b124b5174397b7908b627b6a8b746ae7f59da5ab7` native_c_api.h:2166`), `pf_native_request_v1`'s fifth published layout; a Book-scoped child may be the host-sized close `on_close_units` sizes | the arm-relation scenario |
| `native_sized_units` | `strategy_native_sized_units_v1` (``sha256:3667ec92bafc720549920e9748659247599e15f35d6b34c246f3c1dcecb6c1b6` native_c_api.h:3017`), reading a SIZED request's own sizing block | the sizing-query scenario |
| `resolve_execution_terms`, price half | `on_execution_terms` (``sha256:7084e11d2a7c2d732afd6f206d83e52f93905c94f69f130f1d1ccaf6b60e4bc4` native_c_api.h:2534`): price, opening shape, grid policy, beside the units half `on_close_units` | the terms-hook scenario |
| `validate_execution_precommit` | `on_precommit` (``sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855` native_c_api.h:2543`): the plan, the inspection and the projected account, the closed rows' P&L borrowed for the call | the precommit scenario |
| `resolve_anchored_level` | `on_anchored_level` (``sha256:7bde7ad36377144882067e70e56792edbc91a3ed46391348fb7fc9c0639d3f08` native_c_api.h:2550`) | the anchored-level scenario |
