...
- **Aggregated coordinates (K-IDX, Option A).** The kernel's
  ...
  (`native_execution_consumer.cpp:1030-1044`) in the v19 continuation. The Pine
  adapter keeps its chart-source sampling index as a source-layer fact and does
  not rewrite the kernel's lot or row indices (`pine_strategy_host.cpp:478`).
