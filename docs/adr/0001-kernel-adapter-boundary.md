...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  ...
  `validate_precommit` (`pine_adapter.cpp:13076`) returns `AdmitWithHostMargin`
  (`pine_adapter.cpp:13321`) to own the opening margin decision; `owns_lot_excursions() = true`
  ...
  `NativeCloseExecution::AfterCalculation` in the projected spec (`pine_adapter.cpp:2318-2319`);
...
- **Trail and tick conventions.** The half-tick arm threshold measured against tick-quantized
  extremes while the raw running best is retained (`has_trail_request` `pine_adapter.cpp:9621-9626`); the half-tick
  trigger threshold (`source_trigger_threshold` pine_adapter.cpp:333) and raw-vs-booked fill spelling, behind the
  adapter's terms seam (`resolve_terms` `pine_adapter.cpp:11713`).
...
- **Margin.** The kernel owns the margin *mechanism* — the level solve, the check points, the
  ...
  with TradingView's: `margin_check_allowed` (`pine_adapter.cpp:14111`) for the scheduling,
  `resolve_margin_requirement` (`pine_adapter.cpp:14147`) for the ten-significant-digit money,
...
- **Calculation timing.** The cadence itself is the kernel's: a `calc_on_order_fills` strategy
  ...
  (`next_source_path_waypoint` `pine_adapter.cpp:8312`), the first-open execution chain and its
  own loop guard (`kFirstOpenLoopGuard` `pine_scheduler_native.cpp:680`), and the two fills Pine refuses to
  recalculate on (`suppress_grouped_stop_recalc` `pine_adapter.cpp:5641`).
