...
- **Callbacks are not close-only.** `on_native_bar` is the pure-virtual script-bar calculation
  ...
  (`native_host.hpp:869`, `on_native_bar_open` `native_execution_consumer.cpp:6673-6675`) —
...
- **Matching, fills, and who sizes.** The kernel matches geometrically along the bar path and
  ...
  (`TermsUnresolved` `native_execution_consumer.cpp:4820`).
...
- **The rest of the host surface**, all on `NativeStrategyHost`:
  ...
  | Failure model | `NativeFailureCode` (`native_host.hpp:63-77`); `Failed` is latched, nothing resumes. A throwing callback latches `CallbackException` (`CallbackException` `native_execution_consumer.cpp:4793`) |
...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  `resolve_execution_terms` (`pine_strategy_host.cpp:702`) for TV sizing and fill spelling;
  `validate_execution_precommit` (`pine_strategy_host.cpp:727`), whose
  `validate_precommit` (`pine_adapter.cpp:11405`) returns `AdmitWithHostMargin`
  (`pine_adapter.cpp:11650`) to own the opening margin decision; `owns_lot_excursions() = true`
  (`pine_strategy_host.hpp:284-285`) with `closed_lot_excursion` (`pine_strategy_host.cpp:812`), so
  MFE/MAE are measured on TV tick-quantized prices; and `on_native_tick`
  (`pine_strategy_host.cpp:389`) / `on_native_applied` (`pine_strategy_host.cpp:485`) for
  `calc_on_order_fills` re-entry. `process_orders_on_close` becomes
  `NativeCloseExecution::AfterCalculation` in the projected spec (`pine_adapter.cpp:1541-1542`);
...
- **Batching and open-order priority.** Same-bar command batching and its deferred queues
  ...
  `pine_adapter.cpp:831-890`). Kernel priority is acceptance/incarnation order.
...
- **Trail and tick conventions.** The half-tick arm threshold measured against tick-quantized
  extremes while the raw running best is retained (`has_trail_request` `pine_adapter.cpp:7988-7993`); the half-tick
  trigger threshold (`pine_adapter.cpp:323-335`) and raw-vs-booked fill spelling, behind the
  adapter's terms seam (`resolve_terms` `pine_adapter.cpp:10027`).
- **Money arithmetic.** Ten-significant-digit half-up money (`source_money_round`
  `pine_adapter.cpp:436-442`, twin `tv_money_round` `pine_policy_support.hpp:9-15`).
- **Close reservations.** `strategy.close` callsite batching, two-call provenance and the entry-id
  ledger (`close_logical_units_` `pine_adapter.hpp:1416-1427`); the POOC reservation-growth population predicate
...
- **Margin.** The kernel owns the margin *mechanism* — the level solve, the check points, the
  ...
  with TradingView's: `margin_check_allowed` (`pine_adapter.cpp:12402`) for the scheduling,
  `resolve_margin_requirement` (`pine_adapter.cpp:12438`) for the ten-significant-digit money,
  ...
  (`non_pooc_commissioned_short` `pine_adapter.cpp:14899`), the pre-open admission slice and
...
- **Calculation timing.** The cadence itself is the kernel's: a `calc_on_order_fills` strategy
  ...
  (`next_source_path_waypoint` `pine_adapter.cpp:6663`), the first-open execution chain and its
  own loop guard (`kFirstOpenLoopGuard` `pine_scheduler_native.cpp:680`), and the two fills Pine refuses to
  recalculate on (`suppress_grouped_stop_recalc` `pine_adapter.cpp:4040`).
- **Pine language state and harness flags.** Series, the tick-level barstate flags and
  ...
  `pine_strategy_host.hpp:850`), which `scheduler_update_session_state`
  (`pine_strategy_host.cpp:1774`) selects from the kernel's session-day facts before each source
...
- **The four rules earlier drafts named — one survives as code, and it is hashed.** The
  ...
  adapter-side (`source_money_round` `pine_adapter.cpp:436-442`). `strategy.close` batching and the entry-id ledger
  are **gone** from the kernel — the ledger is adapter state (`close_logical_units_` `pine_adapter.hpp:1416-1427`) and
  ...
  `pine_strategy_host.cpp:574`) and the kernel derives the pair — the intrabar-fill excursion
...
| `include/pineforge/engine.hpp`, lines 918-952 (the row first named lines 73-180, which were and are live code: `ClosedLotExcursionFacts`, `PyramidEntry`, `Trade`) | the ten-significant-digit money rule | adapter-side arithmetic (`source_money_round` `pine_adapter.cpp:436-442`) |
