...
- **Callbacks are not close-only.** `on_native_bar` is the pure-virtual script-bar calculation
  ...
  (`native_host.hpp:877`, `on_native_bar_open` `native_execution_consumer.cpp:7062-7064`) —
...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  `resolve_execution_terms` (`pine_strategy_host.cpp:676`) for TV sizing and fill spelling;
  `validate_execution_precommit` (`pine_strategy_host.cpp:701`), whose
  `validate_precommit` (`pine_adapter.cpp:13046`) returns `AdmitWithHostMargin`
  (`pine_adapter.cpp:13291`) to own the opening margin decision; `owns_lot_excursions() = true`
  (`pine_strategy_host.hpp:284-285`) with `closed_lot_excursion` (`pine_strategy_host.cpp:786`), so
...
- **Trail and tick conventions.** The half-tick arm threshold measured against tick-quantized
  extremes while the raw running best is retained (`has_trail_request` `pine_adapter.cpp:9591-9596`); the half-tick
  trigger threshold (`source_trigger_threshold` pine_adapter.cpp:333) and raw-vs-booked fill spelling, behind the
  adapter's terms seam (`resolve_terms` `pine_adapter.cpp:11683`).
...
- **Margin.** The kernel owns the margin *mechanism* — the level solve, the check points, the
  ...
  with TradingView's: `margin_check_allowed` (`pine_adapter.cpp:14081`) for the scheduling,
  `resolve_margin_requirement` (`pine_adapter.cpp:14117`) for the ten-significant-digit money,
  ...
  (`non_pooc_commissioned_short` `pine_adapter.cpp:16816`) and
...
- **Calculation timing.** The cadence itself is the kernel's: a `calc_on_order_fills` strategy
  ...
  (`next_source_path_waypoint` `pine_adapter.cpp:8282`), the first-open execution chain and its
...
- **Pine language state and harness flags.** Series, the tick-level barstate flags and
  ...
  (`pine_strategy_host.cpp:1764`) selects from the kernel's session-day facts before each source
...
- **The four rules earlier drafts named — one survives as code, and it is hashed.** The
  ...
  `pine_strategy_host.cpp:548`) and the kernel derives the pair — the intrabar-fill excursion
...
| `magnifier_sub_bars_total` / `magnifier_sample_ticks_total` (`engine_report.cpp:45-46`) | **kernel-owned report facts** | The generic intrabar driver already increments `NativeDriverStatistics::sub_bars_processed` / `sample_ticks_processed` for every retained path (`native_execution_consumer.cpp:7626-7638`); the kernel now publishes those same counters for bare hosts. They are report fields, not new durable state: the existing `native_continuation_hash` fold of `NativeDriverStatistics` (`native_execution_consumer.cpp:1085-1090`) sees the same values, so no hash contract moves. |
