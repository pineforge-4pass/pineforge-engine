...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  ...
  `validate_precommit` (`pine_adapter.cpp:13109`) returns `AdmitWithHostMargin`
...
- **Trail and tick conventions.** The half-tick arm threshold measured against tick-quantized
  ...
  adapter's terms seam (`resolve_terms` `pine_adapter.cpp:11736`).
...
- **Margin.** The kernel owns the margin *mechanism* — the level solve, the check points, the
  ...
  with TradingView's: `margin_check_allowed` (`pine_adapter.cpp:14149`) for the scheduling,
  `resolve_margin_requirement` (`pine_adapter.cpp:14218`) for the ten-significant-digit money,
