...
1. **Generic kernel — Pine-agnostic (the reusable core).** The public host is
   ...
   `pending_order_mirror` (`CMakeLists.txt:129`); headers under `include/pineforge/`. One file
...
- **Callbacks are not close-only.** `on_native_bar` is the pure-virtual script-bar calculation
  (`native_host.hpp:450`); also `on_native_input` (`native_host.hpp:673`), `on_native_tick`
  (`native_host.hpp:676`), `on_native_bar_open` — at the modeled opening, before its matching pass
  (`native_host.hpp:686`, `native_execution_consumer.cpp:4394-4396`) — and post-fill
...
- **Requests.** `submit` / `replace` / `cancel` / `submit_market` / `replace_market`
  (`native_host.hpp:880-886`). A request carries one of five triggers — Market, Limit (with a
  generic market-if-touched `fill_through`, `native_order.hpp:203-209`), Stop, StopLimit, Trail
  (`native_order.hpp:77-96`); one of five intents — `Flatten`, `Reduce`, `Transact`, `ReverseTo`,
  `HostSized` (`native_order.hpp:62`); one of five owner kinds — `Independent`, `WaitForApplied`,
  `BindOpening`, `BindOpenings`, `BindCohort` (`native_order.hpp:341-356`); an OCA-style group with
  cancel-or-reduce effect (`native_order.hpp:121-128`); a capacity, `ImmediateRemaining` or
  `PointBudget` (`native_order.hpp:272-276`).
- **Matching, fills, and who sizes.** The kernel matches geometrically along the bar path
  ...
  `resolve_execution_terms` (`native_host.hpp:732-735`) returns no units is a run-terminating
...
- **Observing executions.** Implement `on_native_applied` (`native_host.hpp:729`) — notifications
  ...
  poll `native_events(after_ordinal)` (`native_host.hpp:938`). Do *not* implement
...
- **The rest of the host surface**, all on `NativeStrategyHost`:
  ...
  | Cohorts | `cohort_open` / `cohort_add` / `cohort_remove` (`native_host.hpp:905-907`) — the handle a `BindCohort` owner names |
  | Mid-callback execution | `current_execution_point` (`native_host.hpp:822`), `inspect_current_execution` / `execute_current` (`:826-827`) |
  | Margin seam | `validate_execution_precommit` (`native_host.hpp:737-740`); returning `AdmitWithHostMargin` (`:366-371`) hands that one check to the host — the only margin seam, since spec margin is admission-only |
  | State reads | `physical_position()` / `native_marked_equity(mark)` (`native_host.hpp:913-914`), `trail_state(handle)` (`:823-824`) |
  | Lookahead hazard | the `Bar` given to `on_native_bar_open` is the *complete* script bar (`native_execution_consumer.cpp:6083`); open-only decisions must use `bar.open` (`docs/pages/native-engine.md`) |
...
- **Feeding data.** Bars enter through the engine's own entry points: `run(bars, n)`
  ...
  (`engine.hpp:2310-2320`). `market_driver.hpp:46-101` is driver *types*, not a feed API; read
...
- **Data / HTF.** The magnifier (`magnifier.hpp`) reconstructs intrabar fills.
  ...
  `prepare_native_security_feeds` is protected (`engine.hpp:2164`) with one caller, the adapter
  (`pine_strategy_host.cpp:1325`); `configure_security_evaluators` is an empty virtual
  (`engine.hpp:1813`) whose one caller is likewise the adapter (`pine_strategy_host.cpp:1305`); the
  public `set_native_security_feed` (`engine.hpp:2275`) installs bars nothing evaluates pre-run and
...
- **Reporting caveat.** `fill_report` (`engine.hpp:2388`, `src/engine_report.cpp:40`) gives a bare
  host correct **trade stats**, but the equity curve is recorded by the *host*:
  `record_equity_point` / `update_equity_extremes` are protected (`engine.hpp:1853-1879`) and only
  ...
  (`src/engine_metrics.cpp:153`, consumed at `src/engine_report.cpp:119-139`). The range-end row
...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  ...
  `validate_execution_precommit` (`pine_strategy_host.cpp:634`), returning `AdmitWithHostMargin`
...
- **The real coupling inventory is wider than those four.** `docs/design/native-feature-parity.md`
  ...
  `src/engine_aux_security.cpp:89`, `SecurityEvalState` `engine.hpp:1654`); Pine's comparison band
  ...
  (`pending_order_mirror.hpp:421`, `pending_order_mirror.hpp:135`); and further hashed Pine-shaped
...
2. The kernel changes only for a *generic* capability with a recorded ruling (e.g. per-lot
   ...
   `native_order.hpp:203-209`). Test: if justifying it needs the word "TradingView," it belongs in
   the adapter.
