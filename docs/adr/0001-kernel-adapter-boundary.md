...
- **Callbacks are not close-only.** `on_native_bar` is the pure-virtual script-bar calculation
  ...
  (`native_host.hpp:869`, `on_native_bar_open` `native_execution_consumer.cpp:6570-6572`) —
...
- **Matching, fills, and who sizes.** The kernel matches geometrically along the bar path and
  ...
  (`TermsUnresolved` `native_execution_consumer.cpp:4591`).
- **Observing executions.** Implement `on_native_applied` (`native_host.hpp:917`) — notifications
  ...
  (`src/native_execution_consumer.hpp:38`) and bound by the host.
- **The rest of the host surface**, all on `NativeStrategyHost`:
  ...
  | Failure model | `NativeFailureCode` (`native_host.hpp:63-77`); `Failed` is latched, nothing resumes. A throwing callback latches `CallbackException` (`CallbackException` `native_execution_consumer.cpp:4564`) |
...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  ...
  `validate_precommit` (`pine_adapter.cpp:11780`) returns `AdmitWithHostMargin`
  (`pine_adapter.cpp:12025`) to own the opening margin decision; `owns_lot_excursions() = true`
...
- **Batching and open-order priority.** Same-bar command batching and its deferred queues
  ...
  `pine_adapter.cpp:862-941`). Kernel priority is acceptance/incarnation order.
...
- **Trail and tick conventions.** The half-tick arm threshold measured against tick-quantized
  extremes while the raw running best is retained (`has_trail_request` `pine_adapter.cpp:8353-8358`); the half-tick
  trigger threshold (`pine_adapter.cpp:327-339`) and raw-vs-booked fill spelling, behind the
  adapter's terms seam (`resolve_terms` `pine_adapter.cpp:10402`).
...
- **Margin.** The kernel owns the margin *mechanism* — the level solve, the check points, the
  ...
  with TradingView's: `margin_check_allowed` (`pine_adapter.cpp:12777`) for the scheduling,
  `resolve_margin_requirement` (`pine_adapter.cpp:12813`) for the ten-significant-digit money,
  ...
  (`non_pooc_commissioned_short` `pine_adapter.cpp:15483`), the pre-open admission slice and
...
- **Calculation timing.** The cadence itself is the kernel's: a `calc_on_order_fills` strategy
  ...
  (`next_source_path_waypoint` `pine_adapter.cpp:7048`), the first-open execution chain and its
  own loop guard (`kFirstOpenLoopGuard` `pine_scheduler_native.cpp:680`), and the two fills Pine refuses to
  recalculate on (`suppress_grouped_stop_recalc` `pine_adapter.cpp:4425`).
- **Pine language state and harness flags.** Series, the tick-level barstate flags and
  ...
  (`pine_strategy_host.cpp:1762`) selects from the kernel's session-day facts before each source
...
| `include/pineforge/engine.hpp`, lines 918-952 (the row first named lines 73-180, which were and are live code: `ClosedLotExcursionFacts`, `PyramidEntry`, `Trade`) | the ten-significant-digit money rule | adapter-side arithmetic (`source_money_round` `pine_adapter.cpp:440-446`) |
...
| the two delivery rules, `projected_first_index` `src/native_execution_consumer.cpp:8051-8054` (lookahead) and `clear_if_gapped` `:8045-8048` (gaps) | deliver a completed bucket at its **first** contributing bar (lookahead), and **clear** the series on a bar it publishes nothing on (gaps) | both are delivery rules over a bucket the aggregator already closed; neither reads a platform. They are spec fields: `NativeTimeframeSubscription::lookahead` / `::gaps` | already spelled, and both default to the rule that moves no identity | `tests/test_native_htf_subscriptions.cpp` |
| `pf_native_subscription_v1::lookahead` `include/pineforge/native_c_api.h:2129`, `::gaps` `:2133` | the C spelling of those two rules | since R5 lane E7 both words are typed and named after the kernel's own delivery rules (`pf_native_lookahead_e` `include/pineforge/native_c_api.h:745`, `pf_native_gaps_e` `:756`); the C header no longer names `barmerge` at all, and the migration page maps Pine's words onto them — a *pointer to the reader's vocabulary*, not a justification | — | `tests/test_native_c_api.c` |
