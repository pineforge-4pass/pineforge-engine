...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  ...
  `validate_precommit` (`pine_adapter.cpp:12907`) returns `AdmitWithHostMargin`
  (`pine_adapter.cpp:13079`) to own the opening margin decision; `on_native_tick`
...
- **Batching and open-order priority.** Same-bar command batching and its deferred queues
  (`PendingSameBarCommand` pine_adapter.hpp:1584, `pending_bracket_legs_` pine_adapter.hpp:1996-2014); the retained parent-before-child ordering of live handles — an
...
- **Trail and tick conventions.** The half-tick arm threshold measured against tick-quantized
  extremes while the raw running best is retained (`has_trail_request` `pine_adapter.cpp:9588-9593`); the half-tick
  trigger threshold (`source_trigger_threshold` pine_adapter.cpp:258) and raw-vs-booked fill spelling, behind the
  adapter's terms seam (`resolve_terms` `pine_adapter.cpp:11679`).
...
- **Margin.** The kernel owns the margin *mechanism* — the level solve, the check points, the
  ...
  with TradingView's: `margin_check_allowed` (`pine_adapter.cpp:13868`) for the scheduling,
  `resolve_margin_requirement` (`pine_adapter.cpp:13937`) for the ten-significant-digit money,
  ...
  (`non_pooc_commissioned_short` `pine_adapter.cpp:16715`) and
...
- **Calculation timing.** The cadence itself is the kernel's: a `calc_on_order_fills` strategy
  ...
  (`next_source_path_waypoint` `pine_adapter.cpp:8272`), the first-open execution chain and its
  own loop guard (`kFirstOpenLoopGuard` `pine_scheduler_native.cpp:663`), and the two fills Pine refuses to
  recalculate on (`suppress_grouped_stop_recalc` `pine_adapter.cpp:5552`).
