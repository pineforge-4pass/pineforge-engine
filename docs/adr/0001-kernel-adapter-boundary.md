...
- **Callbacks are not close-only.** `on_native_bar` is the pure-virtual script-bar calculation
  ...
  (`native_host.hpp:869`, `on_native_bar_open` `native_execution_consumer.cpp:6827-6829`) —
...
- **Matching, fills, and who sizes.** The kernel matches geometrically along the bar path and
  ...
  (`TermsUnresolved` `native_execution_consumer.cpp:4866`).
- **Observing executions.** Implement `on_native_applied` (`native_host.hpp:917`) — notifications
  ...
  (`src/native_execution_consumer.hpp:20`) and bound by the host.
- **The rest of the host surface**, all on `NativeStrategyHost`:
  ...
  | Failure model | `NativeFailureCode` (`native_host.hpp:63-77`); `Failed` is latched, nothing resumes. A throwing callback latches `CallbackException` (`CallbackException` `native_execution_consumer.cpp:4839`) |
...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  `resolve_execution_terms` (`pine_strategy_host.cpp:687`) for TV sizing and fill spelling;
  `validate_execution_precommit` (`pine_strategy_host.cpp:712`), whose
  `validate_precommit` (`pine_adapter.cpp:11495`) returns `AdmitWithHostMargin`
  (`pine_adapter.cpp:11740`) to own the opening margin decision; `owns_lot_excursions() = true`
  (`pine_strategy_host.hpp:284-285`) with `closed_lot_excursion` (`pine_strategy_host.cpp:797`), so
  MFE/MAE are measured on TV tick-quantized prices; and `on_native_tick`
  (`pine_strategy_host.cpp:374`) / `on_native_applied` (`pine_strategy_host.cpp:470`) for
  `calc_on_order_fills` re-entry. `process_orders_on_close` becomes
  `NativeCloseExecution::AfterCalculation` in the projected spec (`pine_adapter.cpp:1582-1583`);
...
- **Batching and open-order priority.** Same-bar command batching and its deferred queues
  ...
  `pine_adapter.cpp:859-938`). Kernel priority is acceptance/incarnation order.
...
- **Trail and tick conventions.** The half-tick arm threshold measured against tick-quantized
  extremes while the raw running best is retained (`has_trail_request` `pine_adapter.cpp:8068-8073`); the half-tick
  trigger threshold (`pine_adapter.cpp:324-336`) and raw-vs-booked fill spelling, behind the
  adapter's terms seam (`resolve_terms` `pine_adapter.cpp:10117`).
- **Money arithmetic.** Ten-significant-digit half-up money (`source_money_round`
  `pine_adapter.cpp:437-443`, twin `tv_money_round` `pine_policy_support.hpp:9-15`).
- **Close reservations.** `strategy.close` callsite batching, two-call provenance and the entry-id
  ledger (`close_logical_units_` `pine_adapter.hpp:1420-1431`); the POOC reservation-growth population predicate
...
- **Margin.** The kernel owns the margin *mechanism* — the level solve, the check points, the
  ...
  with TradingView's: `margin_check_allowed` (`pine_adapter.cpp:12492`) for the scheduling,
  `resolve_margin_requirement` (`pine_adapter.cpp:12528`) for the ten-significant-digit money,
  ...
  (`non_pooc_commissioned_short` `pine_adapter.cpp:15198`), the pre-open admission slice and
...
- **Calculation timing.** The cadence itself is the kernel's: a `calc_on_order_fills` strategy
  ...
  (`next_source_path_waypoint` `pine_adapter.cpp:6743`), the first-open execution chain and its
  own loop guard (`kFirstOpenLoopGuard` `pine_scheduler_native.cpp:680`), and the two fills Pine refuses to
  recalculate on (`suppress_grouped_stop_recalc` `pine_adapter.cpp:4120`).
- **Pine language state and harness flags.** Series, the tick-level barstate flags and
  ...
  `pine_strategy_host.hpp:848`), which `scheduler_update_session_state`
  (`pine_strategy_host.cpp:1759`) selects from the kernel's session-day facts before each source
...
- **The four rules earlier drafts named — one survives as code, and it is hashed.** The
  ...
  adapter-side (`source_money_round` `pine_adapter.cpp:437-443`). `strategy.close` batching and the entry-id ledger
  are **gone** from the kernel — the ledger is adapter state (`close_logical_units_` `pine_adapter.hpp:1420-1431`) and
  ...
  `pine_strategy_host.cpp:559`) and the kernel derives the pair — the intrabar-fill excursion
...
| the latched continuation — `BacktestEngine::last_script_continuation_hash_` / `last_script_continuation_valid_` (`include/pineforge/engine.hpp`), the continuation a run's final `broker_state_hash()` folds, and since R5 lane PERF-P1 the consumer's continuation view beside it (`NativeExecutionConsumer::capture_continuation_view`, `src/native_execution_consumer.cpp`) | the Pine host at its last script point (`PineStrategyHost::capture_script_continuation_hash`: the last batch bar, each leftover input of an aggregated chart, every realtime bar), as a view unless a recorded row needs the value at once; the consumer at every `KernelRecorded` report point (`record_script_report_point`), at once; `begin_ready` drops both | **a view of the fold, folded on first read** (R5 lane PERF-P1; a supervisor ruling that revisits E24's STOP bucket and design §3.8 C, which were campaign design notes, not owner rulings: the owner's red line is hash values). Rule 2 (a), mechanism: any host that latches its continuation may latch a view, and nothing in it knows Pine. Rule 2 (b), knob: none -- every value is the eager capture's by construction, because the view is the fold's own bytes, recorded by the same `continuation_hash()` fold into a second sink, with the command history's and the driver log's digests left as holes at the logs' lengths; `defer_continuation_views` is an internal consumer member on no public surface. It stays exact because both logs only grow within a run, `continuation_hash()` folds a pending view before it advances the digests, and `begin_ready` drops a view before it clears the logs. Pinned by `tests/test_native_continuation_view.cpp` (kernel-only) and `tests/test_adapter_continuation_view.cpp`: eager == deferred at every read, and the values harvested on `fc7aad62`. |
...
| `NativeReportPolicy::KernelRecordedAtHostMarks` (`native_run_spec.hpp:63`) | **C++-only** (lane E22's ruling, restated) | Under it the host names each report point from inside its own callbacks, and the C table has no call that marks one: a C host could only declare a series nobody records. `pf_native_report_policy_e` (`native_c_api.h:640`) leaves its value unnamed, and the enum guard holds that exclusion. |
| `strategy_set_path_order` (`pineforge.h:917`) clamps an out-of-range mode to AUTO | **retained** | A `void` ABI v4 setter cannot answer a refusal. Its values are now documented as `pf_native_path_order_e`'s (`native_c_api.h:848`); a C host that wants an unknown word refused declares `path_order` in `pf_native_run_spec_ext_v1`, which answers `PF_NATIVE_E_TAG`. |
...
| `include/pineforge/engine.hpp`, lines 918-952 (the row first named lines 73-180, which were and are live code: `ClosedLotExcursionFacts`, `PyramidEntry`, `Trade`) | the ten-significant-digit money rule | adapter-side arithmetic (`source_money_round` `pine_adapter.cpp:437-443`) |
...
| `bar_path_uses_high_first` `src/engine_path_resolve.cpp:40` (the `TradingView` comment `:43`), `compute_ohlc_path_legs` `src/magnifier.cpp:53` (the `TradingView` comment `:49`), `entry_stop_first_touch` `src/engine_internal.hpp:116-124` | with only OHLC the intrabar walk order is unknowable, so the open-proximity heuristic picks the first leg | **and the knob exists**: `NativeRunSpec::path_order` (`NativePathOrder` `native_run_spec.hpp:334`) states `HighFirst` / `LowFirst` explicitly, so a replay host does not depend on the inference | already spelled, and `Auto` is the default that moves no identity | `tests/test_native_run_spec.cpp`, `tests/test_native_price_grid.cpp` |
