...
1. **Generic kernel — Pine-agnostic (the reusable core).** The public host is
   ...
   thirty-five members of `PINEFORGE_KERNEL_SOURCES` (`CMakeLists.txt:114`) — the `engine_*`,
   ...
   (`CMakeLists.txt:124`) and `pending_order_mirror` (`CMakeLists.txt:130`) units — plus the
   headers directly under `include/pineforge/`. That list is what `add_library`
   (`CMakeLists.txt:154`) compiles into `pineforge_kernel` / `PineForge::kernel`, and it and
...
- **Aggregated coordinates (K-IDX, Option A).** The kernel's
  ...
  (`pine_adapter.cpp:679-685`) where Pine still needs input cadence, and
...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  `resolve_execution_terms` (`pine_strategy_host.cpp:535`) for TV sizing and fill spelling;
  `validate_execution_precommit` (`pine_strategy_host.cpp:560`), whose
  `validate_precommit` (`pine_adapter.cpp:13011`) returns `AdmitWithHostMargin`
  (`pine_adapter.cpp:13183`) to own the opening margin decision; `on_native_tick`
  (`pine_strategy_host.cpp:299`) / `on_native_applied` (`pine_strategy_host.cpp:356`) for
  ...
  `NativeCloseExecution::AfterCalculation` in the projected spec (`pine_adapter.cpp:2153-2154`);
...
- **Batching and open-order priority.** Same-bar command batching and its deferred queues
  (`PendingSameBarCommand` pine_adapter.hpp:1584, `pending_bracket_legs_` pine_adapter.hpp:2000-2018); the retained parent-before-child ordering of live handles — an
  ...
  `update_l4c_priority` `pine_adapter.cpp:976-1046`). Kernel priority is acceptance/incarnation order.
...
- **Trail and tick conventions.** The half-tick arm threshold measured against tick-quantized
  extremes while the raw running best is retained (`has_trail_request` `pine_adapter.cpp:9692-9697`); the half-tick
  trigger threshold (`source_trigger_threshold` pine_adapter.cpp:259) and raw-vs-booked fill spelling, behind the
  adapter's terms seam (`resolve_terms` `pine_adapter.cpp:11783`).
- **Money arithmetic.** Ten-significant-digit half-up money (`source_money_round`
  `pine_adapter.cpp:371-377`, twin `tv_money_round` `pine_policy_support.hpp:9-15`).
- **Quantity dust.** After every applied execution the Pine host erases any lot of at most
  ...
  (`on_native_applied` `pine_strategy_host.cpp:356`), inside a live book as well as at a flat
...
- **Margin.** The kernel owns the margin *mechanism* — the level solve, the check points, the
  ...
  with TradingView's: `margin_check_allowed` (`pine_adapter.cpp:13972`) for the scheduling,
  `resolve_margin_requirement` (`pine_adapter.cpp:14041`) for the ten-significant-digit money,
  ...
  (`non_pooc_commissioned_short` `pine_adapter.cpp:16824`) and
  ...
  them, each owned by the chart bar holding its last minute), and since R5 lane MAG-INTRABAR so
  does the adapter: a magnified run's lower path is TradingView's intrabars, built from the
  host's finer feed at the table's timeframe with that ownership, between the chart bar's own
  open and close (`tradingview_magnifier_bars`, `src/source/magnifier_intrabars.cpp`), and the
  adapter books all 20 tapes' calls (walking the feed's own minutes, it parted on six).
  A `calc_on_order_fills` refill on that path fills at TradingView's next fill point -- the
  chart bar's open, then each intrabar's open twice and its two extremes, never its close
  (`next_input_waypoint`, `coof_waits_for_next_sub_bar`; `tests/fixtures/magnifier_intrabars`,
  72 of 72 cascades). A full-margin long (its one-contract money call) and a short under
...
- **Calculation timing.** The cadence itself is the kernel's: a `calc_on_order_fills` strategy
  ...
  (`next_source_path_waypoint` `pine_adapter.cpp:8344`), the first-open execution chain and its
  own loop guard (`kFirstOpenLoopGuard` `pine_scheduler_native.cpp:729`), and the two fills Pine refuses to
  recalculate on (`suppress_grouped_stop_recalc` `pine_adapter.cpp:5593`).
- **Pine language state and harness flags.** Series, the tick-level barstate flags and
  ...
  `pine_strategy_host.hpp:867`), which `scheduler_update_session_state`
  (`pine_strategy_host.cpp:1391`) selects from the kernel's session-day facts before each source
  ...
  (`pine_session_ismarket` `pine_strategy_host.hpp:617`), which on six `lab tv` tapes misses every
  ...
  probe-suppress overrides (`set_realtime_tail` `pine_strategy_host.hpp:457`,
  `set_probe_suppress_tail_logic` `pine_strategy_host.hpp:483`), whose live-probe protocol is
...
- **The kernel is a separately linkable artifact.** `add_library`
  (`CMakeLists.txt:154`) builds `pineforge_kernel` / `PineForge::kernel` from
...
- **The four rules earlier drafts named — one survives as code, and it is hashed.** The
  ...
  adapter-side (`source_money_round` `pine_adapter.cpp:371-377`). `strategy.close` batching and the entry-id ledger
  ...
  `source::PineStrategyHost::set_margin_call_enabled` (`pine_strategy_host.hpp:435`);
...
- **The examples are a gated target with executed assertions.** Eighteen Pine-free hosts ship
  ...
  (`CMakeLists.txt:37`, default OFF) builds them (`add_subdirectory` `CMakeLists.txt:343`) and
...
| the samples of a retained lower bar -- `NativeExecutionConsumer::deliver_intrabar_script` (`src/native_execution_consumer.cpp`): a bar the direct four-sample walk declines (a leg of zero length, an open at its own low) is walked through its own four turning points, the repeated one included, where the general `ENDPOINTS` sampler filled the missing point uniformly; a bar that traded nothing at one price (volume 0, open = high = low = close) is one discrete point, walked once | every host with a `lower_tf` path; the Pine adapter's magnified path marks a chart bar's own open and close this way (R5 lane MAG-INTRABAR, `source::tradingview_magnifier_bars`, `src/source/magnifier_intrabars.cpp`) | **a mechanism correction** (R5 lane MAG-INTRABAR). Rule 2 (a), mechanism: a retained bar supplies its own four ticks, and every sample is a check point of the margin model since R5 lane PAR-MARGIN, so a sample between them measured -- and a request matched at -- a price the bar did not print at that point of its path; a bar that traded nothing is one quote with no path, and its four coincident samples were three path points, three phases and three margin checks that are not there. Nothing in it knows a venue: the rule reads the bar's own prices and volume. Rule 2 (b), knob: none. TradingView's magnified broker walks an intrabar as its four ticks whatever their values (`tests/fixtures/intrabar_margin/pm2-i3-pooc-mag-0402-2000`: its first call is booked at an intrabar that opens at its low). What moved: an open-at-its-low bar's second mark is its low, 60, where the sampler put 77.5, and the call armed at that open is booked there; a zero-volume one-price bar gives one mark where it gave four (`tests/test_native_intrabar_turning_points.cpp`, kernel-only), and the sampler test's lower-path expectation moved with it (`tests/test_magnifier_ordered_sampler.cpp`: a sub-bar with a zero-length leg walks its corners). No witness value moved; the corpus runs no lower path (312/312, moved 0). |
| the presented bar -- `current_bar_`, `bar_index_`, `prev_bar_timestamp_` (`include/pineforge/engine.hpp`) | the kernel at every callback point: the consumer presents the bar at the point's instant (`src/native_execution_consumer.cpp`). For the body it runs, the Pine host then presents its own script bar. `scheduler_publish_source_bar` and `scheduler_publish_suppressed_tail` write the whole `current_bar_`, stamped at the bar's label (`current_bar_ = bar` `pine_strategy_host.cpp:1422` and `:1531`). They also write the slot's `bar_index_`, restored after a temporary publication, and each published slot's `prev_bar_timestamp_`. `PineScheduler::input` holds `bar_index_` at the input's own index for one input (`InputBarIndexScope` `pine_scheduler_native.cpp:352`) | **retained, the host's view; the fourth audit's E21 check e**. Execution reads its cursor, not this bar: `execute_current` converts at its cursor's rate and neither writes nor restores the host's clock (Section B's row). The broker-state hash folds none of the three, and `scripts/broker_state_hash_waivers.txt` waives `current_bar_` (a convenience copy) and `bar_index_` (a reconstructed cursor). While the Pine body runs, the host's bar is what these read: generated code's `time` and OHLC reads, the presented-clock readers `open_trade_profit` and `active_account_currency_fx`, and `strategy_stream_state_hash`, which folds `current_bar_` and `prev_bar_timestamp_` (`src/engine_stream.cpp`) |
| the run's words on the codegen-ABI class -- `input_tf_`, `script_tf_`, `script_tf_seconds_`, `last_bar_index_`, `last_bar_time_`, `qty_step_` (`include/pineforge/engine.hpp`) | the Pine host alone. `scheduler_prepare_script_run` sets the timeframe words and the feed's last bar from its scheduler's run spec (`script_tf_seconds_` `pine_strategy_host.cpp:1077`). `apply_realtime_tail_horizon` narrows the last bar to a live probe's horizon, and `prepare_native_begin` copies the symbol's `qty_step` | **retained as the Pine host's run facts**. Generated code reads the timeframe words. The kernel reads them for the report (`src/engine_report.cpp`), the implicit chart feed and the chart-day partition (`src/engine_aux_security.cpp`), all of which a bare host reaches with them empty. A bare host's timeframes are its `NativeRunSpec`'s, which the consumer parses into its own. `last_bar_index_`, `last_bar_time_` and `qty_step_` sit in the hashed region and are waived as feed and symbol metadata; the timeframe words sit outside it. `script_tf_seconds_`'s comment used to place `script_tf_`'s writers in `engine_run.cpp`; it now names the one writer (R5 lane H-DOCGATES) |
| the security-evaluator registry the Pine host fills -- `security_eval_states_`, `security_input_tf_`, `security_next_input_ms_` (`include/pineforge/engine.hpp`) | the Pine host. `declare_security_sites_to_kernel` hands a routable run's sites to the kernel and empties the registry (`security_eval_states_` `pine_strategy_host.cpp:1215`). `scheduler_prepare_security_sequence` sets the input timeframe and resets the next-input cursor, and the auxiliary drive and the scheduler advance that cursor (`src/source/pine_aux_security.cpp`, `src/source/pine_scheduler.cpp`) | **retained by design §2.ii row m**: the kernel keeps the registry, the aggregator and one generic step, and the Pine semantics run in `src/source/pine_security_eval.cpp`. `security_next_input_ms_` is waived: the source scheduler's deferred-boundary state is hashed on the source side. `strategy_stream_state_hash` folds the registry |
...
| `include/pineforge/engine.hpp`, lines 918-952 (the row first named lines 73-180, which were and are live code: `ClosedLotExcursionFacts`, `PyramidEntry`, `Trade`) | the ten-significant-digit money rule | adapter-side arithmetic (`source_money_round` `pine_adapter.cpp:371-377`) |
