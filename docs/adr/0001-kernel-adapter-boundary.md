...
- **Seams the adapter overrides.** The kernel is driven, not patched. `PineStrategyHost` overrides
  ...
  `validate_precommit` (`pine_adapter.cpp:13139`) returns `AdmitWithHostMargin`
...
- **Trail and tick conventions.** The half-tick arm threshold measured against tick-quantized
  extremes while the raw running best is retained (`has_trail_request` `pine_adapter.cpp:9684-9689`); the half-tick
  trigger threshold (`source_trigger_threshold` pine_adapter.cpp:258) and raw-vs-booked fill spelling, behind the
  adapter's terms seam (`resolve_terms` `pine_adapter.cpp:11776`).
...
- **Margin.** The kernel owns the margin *mechanism* — the level solve, the check points, the
  ...
  with TradingView's: `margin_check_allowed` (`pine_adapter.cpp:14174`) for the scheduling,
  `resolve_margin_requirement` (`pine_adapter.cpp:14210`) for the ten-significant-digit money,
