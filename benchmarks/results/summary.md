...
- **excellent**: PineForge 50/50, PyneCore 47/50
...
- **Slot 13 swap (r5):** Replaced `13-parabolic-asr` (PF + PC both tier "strong" — engines bit-perfect with each other; TV-side parabolic SAR semantic divergence ~2.8% extra trades) with corpus probe `ta-stoch-slow-k-d-cross-01` (corpus tier excellent, 7585 trades). Bench composition now exercises Stoch slow K/D cross. Original parabolic-asr probe remains in corpus.
- **Cloud-compiler drift:** 41 of 50 strategies' `strategy_pyne.py` regenerated by PyneSys 6.4.6 (see `cloud_compile_drift.md`).
- **Known issue:** `10-market-shift` PyneCore 6.4.6 `input.string()` API rejection (`takes from 2 to 3 positional arguments but 4 positional arguments (and 1 keyword-only argument) were given`); reflects stale CSV from prior refresh, not a live 6.4.6 run.
- **Tier shifts vs prior (2025-05 baseline):** 17-bos-curv promoted strong → excellent (canonical align-then-trim window eliminated edge-trade count inflation). Slot 13 swap promotes PF 49/50 → **50/50 excellent**.
