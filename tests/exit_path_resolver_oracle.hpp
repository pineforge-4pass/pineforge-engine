#pragma once
/*
 * exit_path_resolver_oracle.hpp — the TradingView exit-path resolver, as a
 * TEST ORACLE.
 *
 * This is `internal::resolve_exit_path_fill` and the tick-quantized trail
 * machinery it owns, moved verbatim out of src/source/pine_path_resolve.cpp
 * by R5 lane N10.  On that tree the resolver was a second fill simulation
 * beside the kernel matcher (src/native_matching.hpp +
 * NativeExecutionConsumer): no production caller reached it any more — its
 * only remaining references were its own declaration in engine_internal.hpp,
 * comments, and the resolver-level pins of the trail test suites.  Shipping
 * it inside the adapter was the duplicate mechanism G2 names, so the
 * implementation left `src/` and the pins that need it carry it here.
 *
 * It is therefore NOT engine behaviour and nothing in src/ may call it.  The
 * rules it still encodes (open-arming, the sub-tick offset floor, the
 * carried best, activation on the tick-quantized bar) are each pinned a
 * second time end to end by the `test_engine_*` / host-level fixtures in the
 * same suites; these resolver-level rows are the finer-grained twin of those.
 *
 * Bodies, comments and file-internal order are unchanged from the move,
 * except that the four externally-linked entry points became `inline` (a
 * header now) and the symbols that were dead even for the tests
 * (`price_path_priority`, `fill_bar_path_points`, `collect_cross_events`,
 * `resolve_entry_stop_limit_fill`, `cascade_exit_inflight_fires`,
 * `no_trail_exit_gaps_at_open`, `select_no_trail_exit_segment_levels`) were
...
// Intra-bar path-position comparisons (segment + [0..1]).  Moved with the
// resolver by N10: after the move nothing in src/ compares a path position.
inline constexpr double kPathPosEps = 1e-12;

// Forward declarations (the raw-only form forwards to the tick_bar form).
// The default arguments live here, exactly as they did in engine_internal.hpp.
inline ExitPathFill resolve_exit_path_fill(const Bar& bar,
                                           const Bar& tick_bar,
...
inline int path_cross_kind_priority(PathCrossKind kind) {
    switch (kind) {
...
inline void append_cross_event(CrossEventList* events,
                               double from_price,
                               double to_price,
                               double level,
                               PathCrossKind kind) {
...
inline CrossEventList collect_cross_events_split(double from_price,
                                          double to_price,
