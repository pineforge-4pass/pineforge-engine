...
public:
    ...
    using pineforge::source::PineStrategyHost::fixture_configuration;
    std::vector<Trade> all_trades() const {
        std::vector<Trade> rows;
        rows.reserve(static_cast<std::size_t>(trade_count()));
        for (int i = 0; i < trade_count(); ++i) rows.push_back(get_trade(i));
        return rows;
    }
};
