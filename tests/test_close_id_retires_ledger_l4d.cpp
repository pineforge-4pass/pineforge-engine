// A29 native-route twin for test_close_id_retires_ledger.cpp.
//
...
#define CHECK_NEAR(actual, expected, tolerance) do { if (!((actual) >= (expected) - (tolerance) && (actual) <= (expected) + (tolerance))) { ++failures; } } while (0)
