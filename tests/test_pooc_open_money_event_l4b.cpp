...
public:
    ...
    void on_source_bar(const Bar& bar) override {
        ...
        if (bar_index_ == 1 && position_side_ != PositionSide::FLAT) {
            ...
            const bool recalc = point && point->decision.coordinate.path_phase == NativePathPhase::Open
                && is_first_tick();
            const double mark = point ? point->price : bar.close;
            seen.push_back({signed_position_size(),
                current_equity() + open_profit(mark), mark,
                recalc, recalc, point.has_value()});
