...
void fold_coordinate(Fnv& f, const NativeCoordinate& c) {
    f.v(c.ordinal); f.v(c.interval_index); f.v(c.input_interval_index);
    f.v(c.open_ms); f.v(c.eligible_open_ms);
...
// expectation corrected (6 values), because v19-B folds the FX-roll check's last two driver-point instants into the continuation under a staged FX curve, now that no driver log is kept -- margin_fx_roll's fold moved only through the continuation and broker-state values it reads; with that one fold masked the rows are main's again; every hook's call count and the trades did not move; harvested with -DPF_L1_HARVEST and PF_LEAN_PATH_DUMP=1 against main 3eb2cb84 (reproduces every old pin), this tree and V19-B's tip bc749095 (byte-identical rows, identical hook counts):
//   margin_fx_roll/canonical: fold 2789533630001178538ULL -> 14184613073071204061ULL, continuation 17178754079582823760ULL -> 1188032297556811944ULL, broker 209420367109385368ULL -> 5438877000245170716ULL
//   margin_fx_roll/tolerant: fold 14338402113188947682ULL -> 6141208888695716657ULL, continuation 1793934981347633737ULL -> 13394277230948639587ULL, broker 12065124535081589535ULL -> 15614434516016031356ULL
// K-IDX Option A re-pins the v19 coordinate and input-coordinate folds once.
constexpr Pin kPins[] = {
    {"batch/canonical", 17117662984031888860ULL, 17780913817525129464ULL, 2068800667747861245ULL, 6ULL},
    {"batch/tolerant", 7159587779173285726ULL, 7106716373028264855ULL, 979090476763592675ULL, 7ULL},
    {"margin/canonical", 15249851553654389325ULL, 15704940793391485011ULL, 2547355677285145902ULL, 0ULL},
    {"margin/tolerant", 12102871564077229025ULL, 13946083138301348255ULL, 7326839878467814545ULL, 4ULL},
    {"margin_calculation/canonical", 1763085069446177047ULL, 15220880405934724875ULL, 4672034550176332852ULL, 4ULL},
    {"margin_calculation/tolerant", 7785445382470718173ULL, 10902805914158279185ULL, 7522617531502372512ULL, 1ULL},
    {"margin_fx_roll/canonical", 16450529000057841155ULL, 13234685573085315365ULL, 6826822976864983079ULL, 0ULL},
    {"margin_fx_roll/tolerant", 12588418184566633375ULL, 2680628296643020618ULL, 2266859119956126744ULL, 5ULL},
    {"every_point/canonical", 10723811937494878803ULL, 4491187202482505889ULL, 11471363269271551172ULL, 9ULL},
    {"every_point/tolerant", 14679941050025003028ULL, 6574169214787712326ULL, 8952049994176171904ULL, 8ULL},
    {"synthesized/canonical", 6703115050510603746ULL, 535930656237195088ULL, 13434174737889010896ULL, 9ULL},
    {"synthesized/tolerant", 7780316339256592953ULL, 16964107334250799118ULL, 14901055835935455905ULL, 13ULL},
    {"synthesized_every_point/canonical", 9594243673180640049ULL, 178344748142130161ULL, 13681213038111961167ULL, 10ULL},
    {"synthesized_every_point/tolerant", 156957421120367645ULL, 10081957441325511405ULL, 7599415587229105335ULL, 9ULL},
    {"volume_weighted/canonical", 3445637731839009940ULL, 10263238676578182104ULL, 10666024533260247959ULL, 6ULL},
    {"volume_weighted/tolerant", 16738158308980766384ULL, 15126045393606532712ULL, 3816479336606218160ULL, 7ULL},
    {"lower/canonical", 16457686788008518293ULL, 11357011057993872829ULL, 6569078669742286805ULL, 6ULL},
    {"lower/tolerant", 14962791123507705639ULL, 16635376154655679104ULL, 3158821142657662539ULL, 8ULL},
    {"aggregated/canonical", 6196042442387675404ULL, 9322310899630531166ULL, 522501888358458445ULL, 2ULL},
    {"aggregated/tolerant", 16129624093897181696ULL, 5050819594448951837ULL, 8156482062715705622ULL, 0ULL},
    {"subscribed/canonical", 9354174218907517759ULL, 10755432213466760236ULL, 5910993035115172010ULL, 10ULL},
    {"subscribed/tolerant", 1683434358667330146ULL, 17773566295771860004ULL, 1407911890106154906ULL, 10ULL},
    {"anchored/canonical", 5612726195706972414ULL, 12008230362801453845ULL, 16162400829133148863ULL, 7ULL},
    {"anchored/tolerant", 16074674844732067849ULL, 17818307101311643851ULL, 16691127111077318243ULL, 9ULL},
    {"risk/canonical", 6787970345605368602ULL, 12195288227410166402ULL, 8314083083443106458ULL, 4ULL},
    {"risk/tolerant", 10695848571822133801ULL, 3646571594064542412ULL, 17592057317824588303ULL, 3ULL},
    {"ticks/canonical", 14757823880729403954ULL, 4258680971556760714ULL, 13717708224844676838ULL, 10ULL},
    {"ticks/tolerant", 7992845295774848222ULL, 15797592512778199126ULL, 2657993891625051114ULL, 5ULL},
    {"stream_bars/canonical", 11953185860877643017ULL, 10711069245673339558ULL, 8891908041682500906ULL, 8ULL},
    {"stream_bars/tolerant", 11934789529351532745ULL, 13544793672722675478ULL, 9748293825336591548ULL, 11ULL},
};
