...
public:
    enum class Mode { Partial, Reversal, Holding, Margin };

    explicit FeeHost(Mode mode, CommissionType commission = CommissionType::PERCENT)
        : mode_(mode) {
        source::PineStrategyConfig config;
        config.initial_capital = mode == Mode::Reversal ? 10000.0 : 1000.0;
        config.default_qty_type = static_cast<int>(QtyType::PERCENT_OF_EQUITY);
        config.default_qty_value = mode == Mode::Holding ? 50.0 : 100.0;
        config.commission_type = static_cast<int>(commission);
        config.commission_value = commission == CommissionType::PERCENT ? 1.0 : 0.0;
...
void test_adverse_margin_ledger_debits_entry_fee() {
    ...
    const Bar tape[] = {bar(100.0, 2000), {100.0, 105.0, 100.0, 105.0, 1.0, 62000}};
    probe.run(tape, 2);
...
void test_ki56_clean_room_tv_quantities() {
    FeeHost holding(FeeHost::Mode::Holding);
    const Bar holding_tape[] = {bar(1896.99, 2000), bar(1896.99, 62000)};
    holding.run(holding_tape, 2);
    CHECK(near(std::abs(holding.position()), 2.3498, 1e-9));

    FeeHost margin(FeeHost::Mode::Margin);
