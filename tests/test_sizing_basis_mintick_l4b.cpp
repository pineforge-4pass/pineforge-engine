...
public:
    ...
    void on_source_bar(const Bar& /*bar*/) override {
        ...
        if (a == 'L' || a == 'S') {
            const bool is_long = a == 'L';
            const char* id = is_long ? "L" : "S";
            strategy_entry(id, is_long);
            for (const auto& row : source_pending_view()) {
                if (row.id != id) continue;
                if (is_long) basis_buy.push_back(row.default_stop_sizing_price);
                else basis_sell.push_back(row.default_stop_sizing_price);
                // The opposite directional basis is a public tick projection
                // of this source callback; only the placed side's quantity is
                // read from the adapter's immutable pending projection.
                const double opposite = nearest(nearest(current_bar_.close)
                    + (is_long ? -1.0 : 1.0) * slippage_ * syminfo_mintick_);
                if (is_long) basis_sell.push_back(opposite);
                else basis_buy.push_back(opposite);
                frozen_qty.push_back(row.frozen_default_qty);
                break;
            }
            return;
        }
