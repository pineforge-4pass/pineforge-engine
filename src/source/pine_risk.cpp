...
PineStrategyHost::BarTime PineStrategyHost::_decompose_bar_time_chart_tz() const {
...
execution::Status PineStrategyHost::preflight_source_close_observation(
...
void PineStrategyHost::observe_source_close_rows(
...
bool PineStrategyHost::check_risk_allow_entry(bool is_long) const {
...
void PineStrategyHost::update_risk_state() {
...
int PineStrategyHost::intraday_loss_day_key() const {
    BarTime bt = _decompose_bar_time_chart_tz();
    return bt.dayofmonth * 100 + bt.month;
}

void PineStrategyHost::intraday_loss_begin_bar(const Bar& bar) {
...
bool PineStrategyHost::intraday_loss_orders_blocked() const {
...
bool PineStrategyHost::evaluate_max_intraday_loss(double mark_price,
...
void PineStrategyHost::finish_intraday_loss_cancel() {
...
void PineStrategyHost::evaluate_max_intraday_loss_over_path(const Bar& bar) {
    if (risk_max_intraday_loss_ <= 0.0) return;
    double path[4];
...
// sampled price.
void PineStrategyHost::update_per_trade_extremes() {
