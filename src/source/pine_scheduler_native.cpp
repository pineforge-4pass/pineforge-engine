...
void PineScheduler::bar_open(const Bar& value, const NativeDecisionContext& context,
    ...
    if (host.scheduler_coof_enabled()) snapshot_coof_state(host);
}
...
void PineScheduler::bar(const Bar& value, const NativeDecisionContext& context, PineStrategyHost& host) {
    ...
    const bool coof_recalculated = host.scheduler_coof_enabled()
    ...
    // ab9714be pine_scheduler.cpp:1661-1675, 1677-1756: publish the Pine
    // session flags from the chart bar before generated code runs.  Daily or
    // higher bars are their whole session; intraday last-bar uses the next
    // retained chart label (or the forming stream bucket width).
    const bool in_session = host.chart_bar_ismarket(script_bar.timestamp);
    bool next_in_session = false;
    if (in_session && chart_index >= 0
        && chart_index + 1 < static_cast<int>(retained_.bars.size())) {
        next_in_session = host.chart_bar_ismarket(
            retained_.bars[static_cast<std::size_t>(chart_index + 1)].timestamp);
    } else if (in_session && retained_.is_stream) {
        const int seconds = tf_to_seconds(host.script_tf_);
        next_in_session = seconds > 0
            ? host.chart_bar_ismarket(script_bar.timestamp
                + static_cast<std::int64_t>(seconds) * 1000)
            : true;
    }
    host.scheduler_set_session_bar_state(in_session, in_session && !next_in_session);
    if (host.scheduler_coof_enabled()) {
        restore_coof_state(host);
        language_.is_first_tick_ = true;
        language_.is_last_tick_ = true;
        language_.history_slot_is_new_ = !language_.coof_checkpoint_contains_current_bar_;
    }
    ...
    else host.scheduler_publish_source_bar(script_bar, true, !coof_recalculated);
    if (host.scheduler_coof_enabled()) {
        commit_coof_state(host);
        restore_coof_state(host);
        language_.coof_checkpoint_contains_current_bar_ = false;
        language_.history_slot_is_new_ = true;
    }
    host.prev_in_session_ = host.session_ismarket_;
...
void PineScheduler::applied(const native_order::ExecutionAppliedEvent& event,
    ...
    const bool first_recalc = coof_callback_script_open_ != context.script_bar_open_ms;
    restore_coof_state(host);
    language_.is_first_tick_ = true;
    language_.is_last_tick_ = !retained_.is_stream;
    language_.history_slot_is_new_ = !language_.coof_checkpoint_contains_current_bar_;
    publish_series(callback_bar, host);
    host.adapter_.begin_coof_recalc(context, first_open);
    try {
        host.scheduler_publish_source_bar(callback_bar, true, first_recalc);
    } catch (...) {
