...
bool BacktestEngine::NativeSettlementStage::OneLot::admit(
        const BacktestEngine& engine, internal::SettlementEntry entry,
...
void BacktestEngine::NativeSettlementStage::OneLot::count(
        internal::SettlementEntry entry, bool fused) noexcept {
...
execution::Status BacktestEngine::preview_native_settlement_commit(
    ...
    return NativeSettlementStage::OneLot::preview_keeping(
        *this, action, fill, context, scope, selected, account, row_pnl, nullptr);
}
...
execution::Status BacktestEngine::preview_native_settlement_commit(
        ...
        execution::AccountEffectProjection& account, std::vector<double>& row_pnl) const {
    return NativeSettlementStage::OneLot::preview_keeping(
        *this, reversal, fill, context, account, row_pnl, nullptr);
}

execution::Status BacktestEngine::NativeSettlementStage::OneLot::preview_keeping(
        const BacktestEngine& engine, const execution::Action& action,
    ...
    row_pnl.clear();
    if (readiness == execution::Status::Applied) {
        row_pnl.reserve(rows.closed_trades.size());
...
std::optional<execution::Result> BacktestEngine::NativeSettlementStage::OneLot::settle_kept(
        BacktestEngine& engine, const execution::Fill& fill,
        const execution::PhysicalExecutionContext& context) const {
    const unsigned mode = settlement_path_mode.load(std::memory_order_relaxed);
    if (mode & kSettlementStagedOnly) return std::nullopt;
    if (mode & kSettlementCounting) count(internal::SettlementEntry::Settle, true);
    return settle(engine, fill, context);
}
