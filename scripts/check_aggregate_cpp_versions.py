...
def check_texts(files):
    ...
    if not re.match(r'\s*BrokerStateHashSink\s+f;\s*f\.s\("pineforge-broker-state/v17"\);', broker):
        raise ValueError("broker entry requires v17 domain")
    if 'kSourceAdapterDomain[] = "pineforge-source-adapter/v2"' not in files[FILES[9]]:
    ...
    if compact.count(fold) != 1:
        raise ValueError("stream entry requires v17 then broker hash")
...
if __name__ == "__main__":
    check()
    print("aggregate v17/source-adapter, standalone admission v2 and lifecycle/cancellation v1 ownership verified")
